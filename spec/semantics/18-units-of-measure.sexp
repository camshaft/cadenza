...
(case
  ...
  (doc
    "`(Qty.of 5.0 (Unit.base #\"meter\"))` attaches the base dimension `meter` to the Float64
...
(case
  "a RUNTIME quantity is returned WITH its unit (the unit label is injected at compile time)"
  (doc
    "A parameterized `(def (main (: v Int64)) (Qty.of v (Unit.base #\"meter\")))` returns a RUNTIME
  ...
  (call main (: 7 Int64))
  (output (: (Qty.of 7 (Unit.base #"meter")) (Qty Int64 (Unit.base #"meter"))))
  (call main (: 42 Int64))
...
; COMMUTATIVE Qty.+ overflow rejection under NO-PROMOTION (breaker; operator-corrected semantics, #8297
; reverted the #8292 widen + #8300 landed the no-promotion reimpl): Qty is GENERIC over its inner numeric
; type — an Int8 inner is NOT promoted to Int64. So `Qty.+` of an `Int8`-inner magnitude and a bare-default
; call-result magnitude keeps the fixed Int8 inner; a sum that overflows Int8 (50 + 100 = 150 > 127) is a
; compile-provable overflow → CDZ0304. Crucially this is COMMUTATIVE: BOTH orders reject (Int8-inner first
; here; the call-result-first twin below) — no operand-order-dependent accept/reject. (History: an earlier
; widen (#8278/#8292) folded the call-result-first order to 150 Int64; the breaker caught the resulting
; order-dependence, the operator corrected the intent to generic-inner/no-promotion, and #8297+#8300 re-based
; it here. Triangulation that pinned it as Qty-specific vs a general width rule stands: the PLAIN-int analog
; `(+ (: 50 Int8) ((fn () 100) 3))` also rejects CDZ0304 both orders.) Regression guard for commutative
; no-promotion overflow rejection — a return of the widen, or a re-introduced order-dependence, breaks it.
(case
  "a Qty.+ Int8-inner sum that overflows is rejected CDZ0304 (Int8-inner magnitude first; generic inner, no promotion)"
  (input
    (do
      (def (main)
        (Qty.value
...
(case
  "a Qty.+ Int8-inner sum that overflows is rejected CDZ0304 COMMUTATIVELY (call-result magnitude first)"
  (input
    (do
      (def (main)
        (Qty.value
...
(case
  "a bare lowercase name in the Qty unit position names it a unit misuse, not a type variable"
  (input (do (def (g (: q (Qty Int64 meter))) q) (export g)))
  (error
    CDZ0101
    (message "`Qty`'s second argument is a UNIT")
    (message "(Unit.base")
    (not "type variable")
    (fix (kind replace) (replacement "(Unit.base #\"meter\")") (unverified))))

(case
  "a bare uppercase name in the Qty unit position names it a unit misuse, not an unknown type"
  (input (do (def (g (: q (Qty Int64 Meter))) q) (export g)))
  (error
    CDZ0101
    (message "`Qty`'s second argument is a UNIT")
    (not "declare it with `(type")
    (fix (kind replace) (replacement "(Unit.base #\"Meter\")") (unverified))))

(case
  "the Qty unit-position misuse fires at a value-annotation site too"
...
(case
  "a Qty with a bad inner TYPE and a bad unit gets position-aware guidance — type guidance inner, unit guidance outer"
  (doc
    "The cross-diagnostic position-awareness (migrated from rcdzc
...
(case
  "a bare SYMBOL in the Qty unit position names a unit expression, not a bare symbol, with a wrap fix"
  (input (do (def (g (: q (Qty Float64 #"meter"))) q) (def (main) 0) (export main)))
  (error
    CDZ0201
    (message "is a UNIT expression, not a bare symbol")
    (not "requires a type, but found a non-type")
    (fix (kind replace) (replacement-contains "(Unit.base") (unverified))))

(case
  "the bare-symbol Qty unit misuse fires at a value-annotation site"
...
(case
  "the bare-symbol Qty unit misuse fires nested inside a List type"
...
(case
  "a bare name in Unit.base names the symbol requirement with a hash-quote fix"
  (input (do (def (main) (Qty.of 1.0 (Unit.base foot))) (export main)))
  (error
    CDZ0201
    (message "is not a unit name here")
    (message "names its unit with a SYMBOL")
    (fix (kind replace) (replacement "#\"foot\"") (unverified))))

(case
  "a bare name in Unit.of names the symbol requirement with a hash-quote fix"
...
(case
  "a bare name in Unit.define names the symbol requirement with a hash-quote fix"
  (input (do (def (main) (Qty.of 1.0 (Unit.define furlong (Unit.base #"m") 201 1))) (export main)))
  (error
    CDZ0201
    (message "is not a unit name here")
    (fix (kind replace) (replacement "#\"furlong\"") (unverified))))

(case
  "an ordinary unbound name (not a unit-builder arg) keeps the plain unbound message, not the redirect"
  (input (do (def (main) (bar 5)) (export main)))
  (error CDZ0101 (message "unbound") (not "is not a unit name here")))

(case
  "Qty.value recovers the underlying numeric value, discarding the unit"
  (doc
    "`(Qty.value (Qty.of 5.0 (Unit.base #\"meter\")))` = 5.0 : Float64 — the explicit exit from
...
(case
  ...
  (doc
    "`(Qty.of 3.0 Unit.one)` is a dimensionless quantity — `Unit.one` is the identity of the unit
...
(case
  "a Unit.of named with a non-symbol value is rejected"
  (doc
    "`(Qty.of 5 (Unit.of 42))` — a unit builder names its unit with a `#\"…\"` SYMBOL, but `42` is an
...
(case
  "a Unit.^ at the wrong arity is rejected"
  (doc
    "`(Qty.of 5 (Unit.^ (Unit.base #\"m\")))` applies `Unit.^` (which raises a unit to an integer
...
(case
  "a Unit.* at the wrong arity is rejected"
  (doc
    "`(Qty.of 5 (Unit.* (Unit.base #\"m\")))` composes with `Unit.*` (a binary product of two units)
...
(case
  "a Qty.of second argument that is a bare integer is rejected as not a unit"
  (input (do (def (main) (Qty.of 5 5)) (export main)))
  (error CDZ0201 (message "`Qty.of`'s second argument must be a UNIT")))

(case
  "a Qty.of second argument that is a string is rejected as not a unit"
  (input (do (def (main) (Qty.of 5 "s")) (export main)))
  (error CDZ0201 (message "`Qty.of`'s second argument must be a UNIT")))

(case
  "a Qty.of second argument that is a tuple is rejected as not a unit"
  (input (do (def (main) (Qty.value (Qty.of 5 #tuple(1 2)))) (export main)))
  (error CDZ0201 (message "`Qty.of`'s second argument must be a UNIT")))

(case
  "an UNBOUND unit name in Qty.of's second-arg position surfaces its own unbound error, not ALSO the not-a-unit reject"
  (doc
    "The no-double contrast (migrated from rcdzc a_non_unit_qty_of_arg_unbound_unit_is_not_a_double_report):
...
(case
  "a non-unit factor in a Unit.* product is named, not silently shipped"
...
(case
  "a non-unit factor in a Unit./ quotient is named, not silently shipped"
  (input (do (def (main) (Qty.value (Qty.of 1.0 (Unit./ (Unit.base #"m") 5)))) (export main)))
  (error CDZ0201 (message "`Unit./` composes two UNITS, but this operand is not a unit")))

(case
  "a non-integer exponent in a Unit.^ power is named, not silently shipped"
...
(case
  "a non-unit base in a Unit.^ power is named, not silently shipped"
...
(case
  "a valid base-unit Qty.of is accepted and Qty.value recovers the magnitude (no over-rejection)"
  (input (do (def (main) (Qty.value (Qty.of 5 (Unit.base #"meter")))) (export main)))
  (call main)
  (output (: 5 Int64)))

(case
  "a valid Unit.one Qty.of is accepted and Qty.value recovers the magnitude"
...
(case
  "a Qty.of magnitude that is a tuple is rejected as not numeric"
  (input
    (do
      (def (main) (Qty.value (Qty.pow (Qty.of #tuple(true) (Unit.base #"gram")) 2)))
      (export main)))
  (error CDZ0201 (message "a quantity's magnitude must be a numeric value")))

(case
  "a Qty.of magnitude that is a string is rejected as not numeric"
  (input (do (def (main) (Qty.value (Qty.of "x" (Unit.base #"gram")))) (export main)))
  (error CDZ0201 (message "a quantity's magnitude must be a numeric value")))

(case
  "a Qty.of magnitude that is a bool is rejected as not numeric"
  (input (do (def (main) (Qty.value (Qty.of true (Unit.base #"gram")))) (export main)))
  (error CDZ0201 (message "a quantity's magnitude must be a numeric value")))

(case
  "a valid Float magnitude Qty.of is accepted (no over-rejection of the numeric case)"
...
(case
  "a bare number to a Qty parameter offers the Qty.of wrap fix"
...
(case
  "a bare number bound to a Qty let-binder offers the Qty.of wrap fix"
  (input (do (def (main) (let (((: x (Qty Int64 (Unit.base #"meter"))) 5)) x)) (export main)))
  (error CDZ0203 (fix (kind wrap) (replacement "(Qty.of … (Unit.base #\"meter\"))"))))

(case
  "a bare number directly annotated a Qty names the Qty.of repair (message-only)"
...
(case
  "a numeric-mix bare value into a Qty Int64 names the arg-type mismatch, not the Qty.of wrap"
...
(case
  "a prefixed quantity displays scaled to its reference unit (Float)"
  (doc
    "`5 kilometer` = `(Qty.of 5.0 (Unit.prefix kilo meter))` DISPLAYS as `5000.0 meter`: the render
...
(case
  "a standard unit abbreviation resolves to its canonical unit in a converting sum"
  (doc
    "A terse SI/metric abbreviation resolves to the SAME family unit as its canonical spelling —
...
(case
  "a quantity whose reference-scaled magnitude overflows its inner Int is rejected"
  (doc
    "`(Qty.of 9223372036854776 kilometer)` : `(Qty Int64 …)` scales to `9223372036854776 × 1000` at
  ...
  (input
    (do
      (def (main) (Qty.of 9223372036854776 (Unit.prefix kilo (Unit.base #"meter"))))
      (export main)))
  (error CDZ0304))
...
(case
  "a prefixed-unit magnitude whose reference-scaled value JUST fits Int64 is not rejected"
  (doc
    "The just-under-overflow control of the CDZ0304 scaled-display reject above: `9223372036854775 km`
...
(case
  "a reference-unit magnitude at Int64 max is not rejected (no scale to overflow)"
  (doc
    "The no-scale control: `(Qty.of 9223372036854775807 meter)` is already in the dimension's REFERENCE
...
(case
  "a narrow-width quantity whose reference-scaled magnitude overflows its inner type is rejected"
  (doc
    "The NARROW-WIDTH twin of the Int64 scaled-display overflow above: `(Qty.of (Int8.of 5) kilometer)`
  ...
  (input
    (do (def (main) (Qty.of (Int8.of 5) (Unit.prefix kilo (Unit.base #"meter")))) (export main)))
  (error CDZ0304))
...
(case
  "a narrow-width quantity ADD whose sum overflows the inner type is rejected (same-unit arith path)"
  (input
    (do
      (def
        (main)
...
(case
  "a mixed-scale narrow-width quantity combine whose reference-converted sum overflows is rejected"
  (doc
    "The reference-CONVERTING arith path honors the inner width too: `1 km` → `1000 m`, then
...
(case
  "a narrow-width quantity add whose sum FITS the inner type runs normally (no spurious overflow trap)"
  (doc
    "The control of the narrow-width arith overflow pair: `50 + 50` = 100 fits Int8 (max 127), so the
  ...
  (input
    (do
      (def
        (main)
        (Qty.value
          (+ (Qty.of (Int8.of 50) (Unit.base #"meter")) (Qty.of (Int8.of 50) (Unit.base #"meter")))))
      (export main)))
  (call main)
  (output (: 100 Int64)))

(case
  "a magnitude literal that overflows the annotated Qty INNER width is rejected"
  (doc
    "The ANNOTATION face of Qty magnitude width (distinct from the scaled-display CDZ0304 pair above —
...
(case
  "a Float32-overflowing magnitude literal under a Float32 Qty inner type is rejected"
  (doc
    "The float twin: `(: (Qty.of 1.0e300 meter) (Qty Float32 meter))` — `1.0e300` is finite at Float64
  ...
  (input
    (:
      (Qty.of
        ...
        (Unit.base #"meter"))
      (Qty Float32 (Unit.base #"meter"))))
  (error CDZ0302))

(case
  "a fitting Float32 Qty magnitude computes at the narrow inner width"
  (doc
    "The no-over-reject control: `(Qty.of 1.5 meter)` under `(Qty Float32 meter)` — 1.5 is exactly
...
(case
  "a quantity in a TUPLE inside a LIST element renders scaled (nested composition)"
  (doc
    "The DEPTH-2 composition of the collection-element scale notes: the Qty sits in a tuple which
  ...
  (output
    (:
      #list(#tuple(1 (Qty.of 5000.0 (Unit.base #"meter"))))
      (List (Tuple Int64 (Qty Float64 (Unit.base #"meter")))))))

(case
  "a quantity in an OPTION inside a MAP value renders scaled (nested composition)"
  (doc
    "The sum-wrapper composition: the Qty sits in an Option payload which itself is a MAP value —
  ...
  (output
    (:
      #map((= 1 (Some (Qty.of 5000.0 (Unit.base #"meter")))))
      (Map Int64 (Option (Qty Float64 (Unit.base #"meter")))))))

(case
  "a family quantity displays scaled exactly to its reference (Rational)"
  (doc
    "`5 mile` = `(Qty.of (Rational.of 5 1) (Unit.of #\"mile\"))` DISPLAYS as `201168/25 meter`
...
(case
  "an IEC-prefixed quantity displays scaled to its reference over Int64"
  (doc
    "`1 KiB` = `(Qty.of 1 (Unit.prefix kibi byte))` DISPLAYS as `1024 byte`: the kibi scale (1024,
...
(case
  "a prefixed quantity displays scaled to its reference over Float32"
  (doc
    "The Float32-inner companion of the Float64/Rational/Int64 scaled-display pins: `5 kilometer`
...
(case
  "a prefixed quantity displays scaled to its reference over BigInt (exact, no truncation)"
  (doc
    "The BigInt-inner companion: `5 kilometer` = `(Qty.of (BigInt.of 5) (Unit.prefix kilo meter))`
...
(case
  "a family quantity displays scaled to its reference over Int64, truncating a non-whole ratio"
  (doc
    "`5 foot` = `(Qty.of 5 (Unit.of #\"foot\"))` DISPLAYS as `1 meter` over Int64: foot = 381/1250 m,
...
(case
  "the display truncation does not lose the stored magnitude (Qty.value reads it back)"
  (doc
    "`(Qty.value (Qty.of 5 (Unit.of #\"foot\")))` = 5 : Int64 — the value recovered is the stored
...
(case
  "an explicit conversion off the truncating-display quantity is still exact (5 foot in inches = 60)"
  (doc
    "`(Unit.in (Unit.of #\"inch\") (Qty.of 5 (Unit.of #\"foot\")))` = 60 : Int64 — 5 foot is exactly
...
(case
  "a same-dimension quantity annotation does not rebrand: annotated 1 km converts to 1000 m"
  (doc
    "`(: (Qty.of 1 kilometer) (Qty Int64 meter))` is 1 km annotated at the meter dimension; converting
...
(case
  "a same-dimension-annotated quantity converts back to its own unit as the identity"
  (doc
    "The sharpest no-rebrand witness: `(: (Qty.of 1 kilometer) (Qty Int64 meter))` converted BACK to
...
(case
  "a same-dimension-annotated quantity combines at its own scale: 1 km + 2 km = 3000 m"
  (doc
    "The annotated `(: (Qty.of 1 kilometer) (Qty Int64 meter))` combined with a real `2 km` sums at km
...
(case
  ...
  (doc
    "`%` (remainder) is not defined on quantity operands (the units surface has no `%` rule; it
  ...
  (input
    (% (Qty.value (Qty.of 7 (Unit.base #"meter"))) (Qty.value (Qty.of 3 (Unit.base #"meter")))))
  (output (: 1 Int64)))
...
(case
  "Qty.unit recovers a quantity's unit to build another quantity of the same unit"
  (doc
    "`(Qty.of 9.0 (Qty.unit y))` where `y` is a `(Qty Float64 meter)` builds a NEW quantity in the
  ...
  (input (let ((y (Qty.of 3.0 (Unit.base #"meter")))) (Qty.of 9.0 (Qty.unit y))))
  (output (: (Qty.of 9.0 (Unit.base #"meter")) (Qty Float64 (Unit.base #"meter")))))

(case
  "Qty.unit recovers a derived unit"
  (doc
    "`(Qty.unit (speed))` where `speed` derives meter/second recovers the DERIVED unit — the whole
  ...
  (input
    (do
      (def (speed) (/ (Qty.of 6.0 (Unit.base #"meter")) (Qty.of 2.0 (Unit.base #"second"))))
      (def (main) (Qty.value (Qty.of 10.0 (Qty.unit (speed)))))
      (export main)))
  (output (: 10.0 Float64)))

(case
  "a quantity built from an extracted unit is dimensionally checked"
  (doc
    "The unit `Qty.unit` yields is a REAL unit, checked like any other: `(+ (Qty.of 1.0 (Qty.unit
...
(case
  "adding a length and a time is a dimensional mismatch naming the operation and bare units"
  (doc
    "L1-2: adding a length directly to a time is a DIMENSIONAL mismatch — CDZ0501 (units-of-measure.md
  ...
  (input
    (do
      (def (main) (+ (Qty.of 1.0 (Unit.base #"meter")) (Qty.of 1.0 (Unit.base #"second"))))
      (export main)))
...
(case
  "a bare literal magnitude adopts its arith sibling's fixed integer width (unify with construction)"
  (doc
    "`(+ (Qty.of 5 meter) (Qty.of v0 meter))` over `v0 : UInt32`: the bare `5` adopts the sibling
...
(case
  ...
  (doc
    "`(+ (Qty.of (: 5 Int64) meter) (Qty.of v0 meter))` over `v0 : UInt32`: the LEFT magnitude is an
  ...
  (input
    (do
      (def
        (main (: v0 UInt32))
        (+ (Qty.of (: 5 Int64) (Unit.base #"meter")) (Qty.of v0 (Unit.base #"meter"))))
      (export main)))
  (error CDZ0301))
...
(case
  "adding a quantity and a bare number offers the same-unit Qty.of wrap fix"
  (input (do (def (g) (+ (Qty.of 5 (Unit.of #"meter")) 3)) (export g)))
  (error CDZ0501 (fix (kind wrap) (replacement "(Qty.of … (Unit.base #\"meter\"))"))))

(case
  "a bare number on the LEFT of a quantity add is wrapped in the quantity's unit"
  (input (do (def (g) (+ 3 (Qty.of 5 (Unit.of #"meter")))) (export g)))
  (error CDZ0501 (fix (kind wrap) (replacement "(Qty.of … (Unit.base #\"meter\"))"))))

(case
  "the same-unit wrap resolves the dimension fault — the wrapped quantity sum runs"
  (input
    (do
      (def (main) (Qty.value (+ (Qty.of 5 (Unit.of #"meter")) (Qty.of 3 (Unit.base #"meter")))))
      (export main)))
  (call main)
  (output (: 8 Int64)))
...
(case
  "adding a quantity to a non-numeric Option operand is a plain CDZ0203 type mismatch, not a dimension slip"
  (doc
    "A quantity added to a non-numeric `(Option (Qty …))` (a `List.at`/`Map.get` result) is the plain
  ...
  (input
    (do
      (def
        (main)
        (Qty.value
          (+ (Qty.of 5 (Unit.base #"meter")) (List.at #list((Qty.of 1 (Unit.base #"meter"))) 0))))
      (export main)))
  (error CDZ0203)
  (no-other-errors))

(case
  "with the Option operand on the LEFT, the quantity-add clash names the (Option (Qty …)) type"
  (input
    (do
      (def
        (main)
        (Qty.value
          (+ (List.at #list((Qty.of 1 (Unit.base #"meter"))) 0) (Qty.of 5 (Unit.base #"meter")))))
      (export main)))
...
(case
  "a Float64 quantity scaled by a bare Int64 is a numeric mismatch with the widening fix"
  (input (do (def (g) (* (Qty.of 5.0 (Unit.base #"meter")) 1)) (export g)))
  (error CDZ0301 (fix (kind replace) (replacement "1.0"))))

(case
  "a Float64 quantity scaled by a bare Float64 is well-formed and runs"
  (input (do (def (main) (Qty.value (* (Qty.of 5.0 (Unit.base #"meter")) 2.0))) (export main)))
  (call main)
  (output (: 10.0 Float64)))

(case
  "two same-dimension quantities with differing inner numerics retype the inner literal"
  (input
    (do (def (g) (+ (Qty.of 5 (Unit.of #"meter")) (Qty.of 3.0 (Unit.of #"meter")))) (export g)))
  (error CDZ0301 (fix (kind replace) (replacement "5.0"))))

(case
  "same-inner-type quantities of one dimension add cleanly and run"
  (input
    (do
      (def (main) (Qty.value (+ (Qty.of 5 (Unit.of #"meter")) (Qty.of 3 (Unit.of #"meter")))))
      (export main)))
  (call main)
  (output (: 8 Int64)))
...
(case
  "an unknown unit near a real one suggests it with a delimiter-preserving symbol rename fix"
  (input (do (def (main) (Qty.of 5 (Unit.of #"metre"))) (export main)))
  (error
    CDZ0201
    (message "unknown unit `metre`")
    (message "did you mean `meter`?")
    (fix (kind replace) (replacement "#\"meter\"") (unverified))))

(case
  "another unknown-unit near-miss suggests the near unit"
  (input (do (def (main) (Qty.of 5 (Unit.of #"secnd"))) (export main)))
  (error
    CDZ0201
    (message "did you mean `second`?")
    (fix (kind replace) (replacement "#\"second\"") (unverified))))

(case
  "an unknown unit with no confident neighbour gets compose/declare guidance, not closest-matches"
  (input (do (def (main) (Qty.of 45 (Unit.of #"mph"))) (export main)))
  (error CDZ0201 (message "compose a compound unit") (message "(Unit.define #\"mph\"")))

(case
  "applying the unknown-unit rename fix clears the fault — the corrected unit runs"
  (input (do (def (main) (Qty.value (Qty.of 5 (Unit.of #"meter")))) (export main)))
  (call main)
  (output (: 5 Int64)))

(case
  "an unknown BASE unit inside a Unit.define is caught and named"
  (input (do (Unit.define #"furlong" (Unit.of #"zorks") 660 1) (def (main) 1) (export main)))
  (error CDZ0201 (message "`zorks`")))

(case
  "a prefixed unit of a different dimension still rejects CDZ0501 (prefix scales within a dimension, never across)"
  (doc
    "A PREFIX scales WITHIN a dimension, never across: `km + second` is still CDZ0501. Pins that the
  ...
  (input
    (do
      (def
        (main)
        (+ (Qty.of 1.0 (Unit.prefix kilo (Unit.base #"meter"))) (Qty.of 1.0 (Unit.base #"second"))))
      (export main)))
  (error CDZ0501))
...
(case
  "a Unit.define with the wrong arity is rejected"
  (input (do (Unit.define #"furlong" (Unit.of #"foot") 660) (def (main) 1) (export main)))
  (error CDZ0201 (message "a `Unit.define` is")))

(case
  "a Unit.define with a non-integer scale is rejected"
  (input (do (Unit.define #"furlong" (Unit.of #"foot") 660.5 1) (def (main) 1) (export main)))
  (error CDZ0201 (message "a `Unit.define` is")))

(case
  "Qty.unit recovers the unit of a quantity whose magnitude is a runtime value"
  (doc
    "`(Qty.unit (Qty.of n meter))` with `n` a boundary parameter recovers the unit — meter — from the
  ...
  (input
    (do
      (def
        (main (: n Int64))
        (Qty.value
          (+ (Qty.of 1 (Qty.unit (Qty.of n (Unit.base #"meter")))) (Qty.of 4 (Unit.base #"meter")))))
      (export main)))
  (call main (: 3 Int64))
  (output (: 5 Int64))
  (call main (: 100 Int64))
...
(case
  "the unit recovered from a runtime-magnitude quantity is dimensionally checked"
  (doc
    "The recovered unit is checked like any other even when read off a runtime-magnitude quantity:
  ...
  (input
    (do
      (def
        (main (: n Int64))
        (Qty.value
          (+ (Qty.of 1 (Qty.unit (Qty.of n (Unit.base #"meter")))) (Qty.of 2 (Unit.base #"second")))))
      (export main)))
  (call main (: 3 Int64))
  (error CDZ0501))
...
(case
  ...
  (doc
    "`(+ (Qty.of 2.0 meter) (Qty.of 3.0 meter))` = a `(Qty Float64 meter)` with value 5.0. The
...
(case
  "a runtime-magnitude same-unit sum adds the erased magnitudes"
  (doc
    "`(+ (Qty.of n meter) (Qty.of 5 meter))` with `n` a boundary Int64 parameter: both operands are
  ...
  (input
    (do
      (def
        (main (: n Int64))
        (Qty.value (+ (Qty.of n (Unit.base #"meter")) (Qty.of 5 (Unit.base #"meter")))))
      (export main)))
  (call main (: 3 Int64))
  (output (: 8 Int64))
  (call main (: 100 Int64))
...
(case
  "a runtime mixed-unit sum emits the scale conversion before the add"
  (doc
    "The mixed-unit companion of the same-unit runtime sum: `(+ (Qty.of v kilometer) (Qty.of 500
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (Qty.value
          (+ (Qty.of v (Unit.prefix kilo (Unit.base #"meter"))) (Qty.of 500 (Unit.base #"meter")))))
      (export main)))
  (call main (: 1 Int64))
  (output (: 1500 Int64))
  (call main (: 2 Int64))
...
(case
  "a recursive fold SUMS a list of same-unit quantities threading a Qty accumulator"
  (doc
    "The COLLECTION face of the same-unit sum above (one binop there; a recursive walk here): a
  ...
  (input
    (do
      (def
        (sum-q
          (: xs (List (Qty Int64 (Unit.base #"meter"))))
          (: acc (Qty Int64 (Unit.base #"meter"))))
        (match xs (#list() acc) (#list(h (.. t)) (sum-q t (+ acc h)))))
      (def
        (main (: n Int64))
        (Qty.value
          (sum-q
            #list((Qty.of n (Unit.base #"meter"))
              (Qty.of 2 (Unit.base #"meter"))
              (Qty.of 30 (Unit.base #"meter")))
            (Qty.of 0 (Unit.base #"meter")))))
      (export main)))
  (call main (: 10 Int64))
  (output (: 42 Int64))
  (live-objects 0))
...
(case
  "a MAX-fold over quantities compares through the erased unit wrapper per step"
  (doc
    "The comparison-accumulator companion: the fold's step is `(if (> h best) h best)` — a Qty
  ...
  (input
    (do
      (def
        (max-q
          (: xs (List (Qty Int64 (Unit.base #"meter"))))
          (: best (Qty Int64 (Unit.base #"meter"))))
        (match xs (#list() best) (#list(h (.. t)) (max-q t (if (> h best) h best)))))
      (def
        (main (: n Int64))
        (Qty.value
          (max-q
            #list((Qty.of 5 (Unit.base #"meter"))
              (Qty.of n (Unit.base #"meter"))
              (Qty.of 7 (Unit.base #"meter")))
            (Qty.of 0 (Unit.base #"meter")))))
      (export main)))
  (call main (: 42 Int64))
  (output (: 42 Int64))
...
(case
  "a quantity over a BigInt magnitude runs unbounded arithmetic on the erased handles"
  (doc
    "A `(Qty BigInt meter)` — a quantity whose inner numeric is the UNBOUNDED BigInt — runs bigint
...
(case
  ...
  (doc
    "A `(Qty BigInt meter)` COMPARISON routes to the exact bigint compare (`bigint-cmp`) on the
...
(case
  "a quantity over a Rational magnitude compares by its exact value"
  (doc
    "The Rational companion: `(< (Qty (Rational 1/40) meter) (Qty (Rational 127/5000) meter))` — 25
  ...
  (input
    (<
      (Qty.of (Rational.of 1 40) (Unit.base #"meter"))
      (Qty.of (Rational.of 127 5000) (Unit.base #"meter"))))
  (output (: true Bool)))

(case
  "a runtime-magnitude same-unit difference subtracts the erased magnitudes"
  (doc
    "The subtraction companion: `(- (Qty.of n meter) (Qty.of 5 meter))` emits the erased `n - 5` as a
...
(case
  "adding quantities of incompatible dimension is a compile-time error"
  (doc
    "`(+ (Qty.of 1.0 meter) (Qty.of 1.0 second))` combines a length with a time — incompatible
...
(case
  "subtracting quantities of incompatible dimension is a compile-time error"
  (doc
    "The subtraction companion: `(- (Qty.of 5.0 meter) (Qty.of 2.0 second))` is the same
...
(case
  "adding a quantity to a NON-numeric value is a plain type mismatch, not a dimension slip"
  (doc
    "The quantity/non-quantity additive reject reports CDZ0501 'a quantity and a plain number' — with
...
(case
  ...
  (doc
    "`(* (Qty.of 2.0 meter) (Qty.of 3.0 meter))` derives the dimension meter·meter = meter² and
  ...
  (input (* (Qty.of 2.0 (Unit.base #"meter")) (Qty.of 3.0 (Unit.base #"meter"))))
  (output
    (: (Qty.of 6.0 (Unit.^ (Unit.base #"meter") 2)) (Qty Float64 (Unit.^ (Unit.base #"meter") 2)))))

(case
  "dividing quantities divides their dimensions"
  (doc
    "`(/ (Qty.of 6.0 meter) (Qty.of 2.0 second))` derives meter/second — a velocity — with value
  ...
  (output
    (:
      (Qty.of 3.0 (Unit./ (Unit.base #"meter") (Unit.base #"second")))
      (Qty Float64 (Unit./ (Unit.base #"meter") (Unit.base #"second"))))))

(case
  "an acceleration dimension renders as a quotient with a squared denominator"
  (doc
    "`(/ (Qty.of 10.0 meter) (Qty.pow (Qty.of 2.0 second) 2))` derives meter/second² — an acceleration
  ...
  (output
    (:
      (Qty.of 2.5 (Unit./ (Unit.base #"meter") (Unit.^ (Unit.base #"second") 2)))
      (Qty Float64 (Unit./ (Unit.base #"meter") (Unit.^ (Unit.base #"second") 2))))))

(case
  "a reciprocal dimension renders as Unit.one over the unit"
  (doc
    "`(/ (Qty.of 1.0 Unit.one) (Qty.of 4.0 second))` derives 1/second — a reciprocal (frequency-like)
  ...
  (output
    (:
      (Qty.of 0.25 (Unit./ Unit.one (Unit.base #"second")))
      (Qty Float64 (Unit./ Unit.one (Unit.base #"second"))))))

(case
  "a cube dimension renders as Unit.^ with exponent 3"
  (doc
    "`(* (* (Qty.of 2.0 meter) (Qty.of 3.0 meter)) (Qty.of 4.0 meter))` derives meter³ — a volume —
  ...
  (input
    (*
      (* (Qty.of 2.0 (Unit.base #"meter")) (Qty.of 3.0 (Unit.base #"meter")))
      (Qty.of 4.0 (Unit.base #"meter"))))
  (output
    (: (Qty.of 24.0 (Unit.^ (Unit.base #"meter") 3)) (Qty Float64 (Unit.^ (Unit.base #"meter") 3)))))

(case
  "a product of two DIFFERENT base dimensions renders as Unit.*"
  (doc
    "`(* (Qty.of 3.0 meter) (Qty.of 2.0 second))` composes two DIFFERENT base dimensions — meter and
  ...
  (output
    (:
      (Qty.of 6.0 (Unit.* (Unit.base #"meter") (Unit.base #"second")))
      (Qty Float64 (Unit.* (Unit.base #"meter") (Unit.base #"second"))))))

(case
  "a scaled derived quantity displays with BOTH scales folded into the magnitude"
  (doc
    "`100 km/h` = `(Qty.of 100.0 (Unit./ kilometer hour))` DISPLAYS as `27.77… meter/second`: the
  ...
  (output
    (:
      (Qty.of 27.77777777777778 (Unit./ (Unit.base #"meter") (Unit.base #"second")))
      (Qty Float64 (Unit./ (Unit.base #"meter") (Unit.base #"second"))))))

(case
  "a scaled cube dimension raises its prefix scale to the power (km³ → ×1e9 meter³)"
  (doc
    "`(Qty.of 2.0 (Unit.^ kilometer 3))` — a scaled base raised to a power — DISPLAYS as
  ...
  (output
    (:
      (Qty.of 2000000000.0 (Unit.^ (Unit.base #"meter") 3))
      (Qty Float64 (Unit.^ (Unit.base #"meter") 3)))))

(case
  "a scaled reciprocal dimension folds the denominator prefix scale (1/ms → ×1000 per second)"
  (doc
    "`(Qty.of 1.0 (Unit./ Unit.one millisecond))` — a reciprocal over a SCALED denominator —
  ...
  (output
    (:
      (Qty.of 1000.0 (Unit./ Unit.one (Unit.base #"second")))
      (Qty Float64 (Unit./ Unit.one (Unit.base #"second"))))))

(case
  "a bare TUPLE of quantities renders each element scaled to its reference (mixed inner types)"
  (doc
    "A compound VALUE — a `(Tuple (Qty …) (Qty …))` literal — crossing the boundary renders each Qty
  ...
  (input
    #tuple((Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter")))
      (Qty.of (Rational.of 5 1) (Unit.of #"mile"))))
  (output
    (:
      #tuple((Qty.of 5000.0 (Unit.base #"meter")) (Qty.of 201168/25 (Unit.base #"meter")))
      (Tuple (Qty Float64 (Unit.base #"meter")) (Qty Rational (Unit.base #"meter"))))))

(case
  "an OPTION payload quantity renders scaled to its reference in the value form"
  (doc
    "A `(Some (Qty …))` literal crossing the boundary renders its payload quantity scaled to its
  ...
  (output
    (: (Some (Qty.of 5000.0 (Unit.base #"meter"))) (Option (Qty Float64 (Unit.base #"meter"))))))

(case
  "a NESTED tuple of quantities renders every element scaled at depth"
  (doc
    "The recursion companion of the flat tuple-of-quantities pin: a `(Tuple (Tuple (Qty …)) (Tuple
  ...
  (input
    #tuple(#tuple((Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter"))))
      #tuple((Qty.of 2.0 (Unit.base #"meter")))))
  (output
    (:
      #tuple(#tuple((Qty.of 5000.0 (Unit.base #"meter"))) #tuple((Qty.of 2.0 (Unit.base #"meter"))))
      (Tuple (Tuple (Qty Float64 (Unit.base #"meter"))) (Tuple (Qty Float64 (Unit.base #"meter")))))))

(case
  "a MIXED-shape compound scales its quantity leaf beside a non-quantity element"
  (doc
    "The heterogeneous companion of the uniform tuple/nested pins: a compound whose leaves are a MIX of
  ...
  (output
    (:
      #tuple((Some (Qty.of 5000.0 (Unit.base #"meter"))) 7)
      (Tuple (Option (Qty Float64 (Unit.base #"meter"))) Int64))))

(case
  "a velocity multiplied by a time recovers the distance dimension"
  (doc
    "The multiply-direction inverse of the velocity quotient: `(* (Qty.of 6.0 (meter/second))
  ...
  (input
    (Qty.value
      (*
        (Qty.of 6.0 (Unit.* (Unit.base #"meter") (Unit.^ (Unit.base #"second") -1)))
        (Qty.of 2.0 (Unit.base #"second")))))
  (output (: 12.0 Float64)))

(case
  "dividing two same-dimension quantities cancels to a dimensionless number"
  (doc
    "`(/ (Qty.of 6.0 meter) (Qty.of 2.0 meter))` divides a length by a length — the meter exponents
...
(case
  "a data-rate quantity converts a binary-prefix composite unit to its byte/second reference"
  (doc
    "`59 GiB/s` (desugared `(Qty.of 59 (Unit./ (Unit.of GiB) (Unit.of s)))`) normalizes to the
  ...
  (output
    (:
      (Qty.of 63350767616 (Unit./ (Unit.base #"byte") (Unit.base #"second")))
      (Qty Int64 (Unit./ (Unit.base #"byte") (Unit.base #"second"))))))

(case
  "adding two data-rate quantities of the same composite dimension is exact"
  (doc
    "`2 GiB/s + 1 GiB/s` = `3 GiB/s`: adding two quantities of the SAME derived data-rate dimension
  ...
  (input
    (+
      (Qty.of 2 (Unit./ (Unit.of #"GiB") (Unit.of #"s")))
      (Qty.of 1 (Unit./ (Unit.of #"GiB") (Unit.of #"s")))))
  (output
    (:
      (Qty.of 3221225472 (Unit./ (Unit.base #"byte") (Unit.base #"second")))
      (Qty Int64 (Unit./ (Unit.base #"byte") (Unit.base #"second"))))))

(case
  "adding a data-rate to a length is a compile-time dimension error over a composite dimension"
  (doc
    "`(GiB/s) + meter` rejects CDZ0501: a data-rate (byte·second⁻¹) and a length (meter) are
...
(case
  "a runtime-magnitude product multiplies the erased magnitudes"
  (doc
    "`(* (Qty.of n meter) (Qty.of 2 second))` with `n` a runtime Int64: the dimension composes to
...
(case
  "a runtime-magnitude quotient divides the erased magnitudes"
  (doc
    "`(/ (Qty.of n meter) (Qty.of 2 second))` derives the velocity dimension meter/second and emits
...
(case
  "a runtime product's derived dimension cancels correctly through a divide"
  (doc
    "`(/ (* (Qty.of n meter) (Qty.of 3 second)) (Qty.of 3 second))` composes meter·second then
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (/
            (* (Qty.of n (Unit.base #"meter")) (Qty.of 3 (Unit.base #"second")))
            (Qty.of 3 (Unit.base #"second")))))
      (export main)))
  (call main (: 4 Int64))
  (output (: 4 Int64))
  (call main (: 30 Int64))
...
(case
  "a runtime product's derived dimension is enforced against an incompatible add"
  (doc
    "The composed dimension of a runtime product is CHECKED like any other: `(+ (* (Qty.of n meter)
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (* (Qty.of n (Unit.base #"meter")) (Qty.of 2 (Unit.base #"second")))
...
(case
  "a runtime velocity multiplied by a time cancels back to the base dimension"
  (doc
    "`(* (/ dist time) time)` over runtime magnitudes: `(/ (Qty.of d meter) (Qty.of t second))`
  ...
  (input
    (do
      (def
        (main (: d Int64) (: t Int64))
        (let
          ((dist (Qty.of d (Unit.base #"meter"))) (time (Qty.of t (Unit.base #"second"))))
          (let ((speed (/ dist time))) (Qty.value (* speed time)))))
      (export main)))
  (call main (: 100 Int64) (: 5 Int64))
  (output (: 100 Int64))
  (call main (: 9 Int64) (: 2 Int64))
  (output (: 8 Int64)))

(case
  "a runtime same-dimension multiply derives the SQUARED dimension and computes the area"
  (doc
    "The runtime companion of the const dimension-multiply pin (:508 folds): two boundary-parameter
...
(case
  "a runtime same-dimension quotient is dimensionless and its value joins bare integer arithmetic"
  (doc
    "`(/ (Qty.of a meter) (Qty.of a meter))` over a runtime magnitude cancels to the dimensionless
  ...
  (input
    (do
      (def
        ...
        (let
          ((x (Qty.of a (Unit.base #"meter"))) (y (Qty.of b (Unit.base #"meter"))))
          (+ (Qty.value (/ x y)) 41)))
      (export main)))
  (call main (: 9 Int64) (: 9 Int64))
  (output (: 42 Int64))
  (call main (: 9 Int64) (: 2 Int64))
  (output (: 45 Int64)))

(case
  "a derived dimension composes across a def boundary through annotated Qty parameters"
  (doc
    "`area` takes two explicitly-annotated `(Qty Int64 (Unit.base #\"meter\"))` parameters and
  ...
  (input
    (do
      (def
        (area (: w (Qty Int64 (Unit.base #"meter"))) (: h (Qty Int64 (Unit.base #"meter"))))
        (* w h))
      (def
        (main (: a Int64) (: b Int64) (: c Int64))
        (Qty.value
          (+
            (area (Qty.of a (Unit.base #"meter")) (Qty.of b (Unit.base #"meter")))
            (area (Qty.of c (Unit.base #"meter")) (Qty.of c (Unit.base #"meter"))))))
      (export main)))
  (call main (: 3 Int64) (: 4 Int64) (: 5 Int64))
  (output (: 37 Int64))
  (call main (: 0 Int64) (: 7 Int64) (: 2 Int64))
  (output (: 4 Int64)))
...
(case
  "adding a squared length to a plain length is a dimension mismatch"
  (doc
    "`(+ (* (Qty.of 2.0 meter) (Qty.of 3.0 meter)) (Qty.of 1.0 meter))` adds meter² (the product's
...
(case
  "adding two squared-length quantities is well-dimensioned"
  (doc
    "The positive companion: `(+ meter² meter²)` — two areas of the SAME derived dimension — is valid,
...
(case
  "adding a cubed length to a squared length is a dimension mismatch"
  (doc
    "`(+ meter³ meter²)` — meter·meter·meter (exponent 3) vs meter·meter (exponent 2) — is a mismatch
...
(case
  "Qty.pow and repeated multiplication produce the same dimension for a matching add"
  (doc
    "`(+ (Qty.pow (Qty.of 2.0 meter) 2) (* (Qty.of 1.0 meter) (Qty.of 1.0 meter)))` adds a `Qty.pow`-2
  ...
  (input
    (Qty.value
      (+
        (Qty.pow (Qty.of 2.0 (Unit.base #"meter")) 2)
...
(case
  "adding two equal high-power quantities joins (meter^100 + meter^100)"
  (doc
    "`(+ (Qty.pow q 100) (Qty.pow q 100))` over meter: two meter^100 quantities share the exact same
...
(case
  "adding adjacent high-power quantities is a mismatch (meter^100 + meter^101)"
  (doc
    "`(+ (Qty.pow q 100) (Qty.pow q 101))` — meter^100 vs meter^101 — are DISTINCT dimensions one
...
(case
  "a high-power product cancels exactly to dimensionless (meter^100 * meter^-100)"
  (doc
    "`(* (Qty.pow q 100) (Qty.pow q -100))` over meter: exponent 100 + (-100) = 0, so the product is
...
(case
  ...
  (doc
    "`(* (Qty.of 2.0 meter) (Qty.of 3.0 Unit.one))` multiplies a length by a dimensionless scalar:
...
(case
  "scaling a Float64 quantity by a bare integer is a numeric-type mismatch"
  (doc
    "`(* (Qty.of 5.0 meter) 1)` scales a `(Qty Float64 meter)` by a bare `Int64` `1` — the SAME
...
(case
  "scaling a quantity by a bare number of its own numeric type keeps its dimension"
  (doc
    "`(Qty.value (* (Qty.of 2.0 meter) 3.0))` = 6.0: multiplying a `(Qty Float64 meter)` by a bare
...
(case
  "scaling an integer quantity by a bare integer of its own type keeps its dimension"
  (doc
    "`(Qty.value (* (Qty.of 5 meter) 2))` = 10: the Int64 companion — a `(Qty Int64 meter)` scaled
...
(case
  "multiplying a scaled-unit quantity by the literal one keeps its dimension and displays scaled"
  (doc
    "`(* (Qty.of 5 (Unit.prefix kilo meter)) 1)` — the calc `5 kilometer * 1` case: multiplying by
...
(case
  ...
  (doc
    "`(/ (Qty.of 6.0 meter) (Qty.of 2.0 meter))` derives meter/meter = Unit.one — the base cancels
...
; ============================================================================================
(case
  "raising a quantity to a compile-time power composes the unit and the magnitude"
  (doc
    "`(Qty.pow (Qty.of 3.0 meter) 2)` squares a length: the unit is raised to the 2nd power
  ...
  (output
    (: (Qty.of 9.0 (Unit.^ (Unit.base #"meter") 2)) (Qty Float64 (Unit.^ (Unit.base #"meter") 2)))))

(case
  "raising a BigInt-magnitude quantity to a power runs the unbounded multiply"
  (doc
    "`(Qty.pow (Qty.of (BigInt.of n) meter) 2)` over a BigInt inner: `Qty.pow` lowers to a repeated
  ...
  (input
    (do
      (def (main (: n Int64)) (Qty.value (Qty.pow (Qty.of (BigInt.of n) (Unit.base #"meter")) 2)))
      (export main)))
  (call main (: 5 Int64))
  (output (: 25 BigInt))
...
(case
  ...
  (doc
    "`(= (Qty.pow (Qty.of 2.0 meter) 2) (* (Qty.of 2.0 meter) (Qty.of 2.0 meter)))` is true: raising
...
(case
  "raising a COMPUTED (derived) quantity to a power squares its magnitude and dimension"
  (doc
    "`(Qty.pow (/ (Qty.of 6.0 meter) (Qty.of 2.0 second)) 2)` — raising a COMPUTED velocity (a
  ...
  (input
    (Qty.value (Qty.pow (/ (Qty.of 6.0 (Unit.base #"meter")) (Qty.of 2.0 (Unit.base #"second"))) 2)))
  (output (: 9.0 Float64)))

(case
  "raising a runtime computed quantity to a power squares the runtime magnitude"
  (doc
    "The runtime companion: `(Qty.pow (/ (Qty.of n meter) (Qty.of 1 second)) 2)` with `n` a boundary
  ...
  (input
    (do
      (def
        (main (: n Int64))
        (Qty.value (Qty.pow (/ (Qty.of n (Unit.base #"meter")) (Qty.of 1 (Unit.base #"second"))) 2)))
      (export main)))
  (call main (: 4 Int64))
  (output (: 16 Int64)))

(case
  "a quantity raised to the zeroth power is a dimensionless one"
  (doc
    "`(Qty.pow (Qty.of 5.0 meter) 0)` is the empty product: the unit's exponents are all scaled to
...
(case
  "the power form cubes an integer-magnitude quantity exactly"
  (doc
    "`(Qty.pow (Qty.of 2 meter) 3)` over Int64: the unit is meter³ and the erased magnitude is
...
(case
  "a runtime-magnitude quantity raised to a power emits the repeated multiply"
  (doc
    "`(Qty.pow (Qty.of x meter) 2)` with `x` a runtime Float64: the power can't be folded, so it
  ...
  (input
    (do
      (def (main (: x Float64)) (Qty.value (Qty.pow (Qty.of x (Unit.base #"meter")) 2)))
      (export main)))
  (call main (: 3.0 Float64))
  (output (: 9.0 Float64)))

(case
  "a negative power is the reciprocal, deriving an inverse unit"
  (doc
    "`(Qty.pow (Qty.of 2.0 second) -1)` raises a time to the -1 power: the unit is second⁻¹ = a
  ...
  (output
    (:
      (Qty.of 0.5 (Unit./ Unit.one (Unit.base #"second")))
      (Qty Float64 (Unit./ Unit.one (Unit.base #"second"))))))

(case
  "the negative power agrees with dividing into the dimensionless one"
  (doc
    "`(= (Qty.pow (Qty.of 2.0 second) -1) (/ (Qty.of 1.0 Unit.one) (Qty.of 2.0 second)))` is true:
  ...
  (input
    (=
      (Qty.pow (Qty.of 2.0 (Unit.base #"second")) -1)
      (/ (Qty.of 1.0 Unit.one) (Qty.of 2.0 (Unit.base #"second")))))
  (output (: true Bool)))

(case
  "a negative power over an integer magnitude truncates the reciprocal"
  (doc
    "`(Qty.pow (Qty.of 2 second) -1)` over Int64: the unit is second⁻¹ and the reciprocal 1/2 is
...
(case
  "a runtime-magnitude quantity raised to a negative power emits the reciprocal"
  (doc
    "`(Qty.pow (Qty.of x second) -1)` with `x` a runtime Float64: the reciprocal can't be folded, so
  ...
  (input
    (do
      (def (main (: x Float64)) (Qty.value (Qty.pow (Qty.of x (Unit.base #"second")) -1)))
      (export main)))
  (call main (: 4.0 Float64))
  (output (: 0.25 Float64)))

(case
  "a negative power over a Rational magnitude is the EXACT reciprocal"
  (doc
    "`(Qty.pow (Qty.of (Rational.of 2 3) meter) -2)` over a Rational inner: the unit is meter⁻² and the
  ...
  (input
    (do
      (def (main) (Qty.value (Qty.pow (Qty.of (Rational.of 2 3) (Unit.base #"meter")) -2)))
      (export main)))
  (output (: 9/4 Rational)))

(case
  "a negative power over a BigInt magnitude truncates the reciprocal exactly"
  (doc
    "`(Qty.pow (Qty.of (BigInt.of 4) meter) -1)` over a BigInt inner: the unit is meter⁻¹ and the
...
(case
  ...
  (doc
    "`(< (Qty.of 2.0 meter) (Qty.of 3.0 meter))` compares two lengths and is true — comparison
...
(case
  "comparing two same-unit Float-inner quantities at RUNTIME rides the scalar float compare"
  (doc
    "`(< (Qty.of x meter) (Qty.of 5.0 meter))` with `x` a RUNTIME Float64: a same-unit Float-inner
  ...
  (input
    (do
      (def
        (main (: x Float64))
        (if (< (Qty.of x (Unit.base #"meter")) (Qty.of 5.0 (Unit.base #"meter"))) 1 0))
      (export main)))
  (call main (: 3.0 Float64))
  (output (: 1 Int64))
  (call main (: 7.0 Float64))
  (output (: 0 Int64)))

(case
  "comparing two same-unit scalar-Int quantities at RUNTIME rides the scalar integer compare"
  (doc
    "The Int-inner analogue of the Float case above. `(< (Qty.of x meter) (Qty.of 5 meter))` with `x`
  ...
  (input
    (do
      (def
        (main (: x Int64))
        (if (< (Qty.of x (Unit.base #"meter")) (Qty.of 5 (Unit.base #"meter"))) 1 0))
      (export main)))
  (call main (: 3 Int64))
  (output (: 1 Int64))
  (call main (: 7 Int64))
...
(case
  "a runtime cross-scale scalar-Int quantity comparison converts before comparing, not a raw magnitude compare"
  (doc
    "`(< (Qty.of a kilometer) (Qty.of b meter))` with RUNTIME a, b: a km operand converts to the
  ...
  (input
    (do
      (def
        (main (: a Int64) (: b Int64))
        (if
          (< (Qty.of a (Unit.prefix kilo (Unit.base #"meter"))) (Qty.of b (Unit.base #"meter")))
          1
          0))
      (export main)))
  (call main (: 1 Int64) (: 999 Int64))
  (output (: 0 Int64))
  (call main (: 1 Int64) (: 1001 Int64))
  (output (: 1 Int64)))

(case
  "a runtime Float quantity comparison against NaN is the IEEE partial order (false)"
  (doc
    "`(< (Qty.of nan meter) (Qty.of 5.0 meter))` — a runtime NaN magnitude compares FALSE under the
  ...
  (input
    (do
      (def
        ...
        (if (< (Qty.of x (Unit.base #"meter")) (Qty.of 5.0 (Unit.base #"meter"))) 1 0))
      (export main)))
  (call main (: nan Float64))
  (output (: 0 Int64)))

(case
  "comparing quantities of incompatible dimension is a compile-time error"
  (doc
    "`(< (Qty.of 2.0 meter) (Qty.of 3.0 second))` orders a length against a time — incompatible
...
(case
  "equality across incompatible dimensions is a compile-time error"
  (doc
    "`(= (Qty.of 1.0 meter) (Qty.of 1.0 second))` compares a length to a time for equality —
...
(case
  "a mixed-scale Int comparison truncates each operand to the reference (lossy, can compare equal)"
  (doc
    "`(= (Qty.of 30 centimeter) (Qty.of 1 foot))` over Int64: each converts to the reference `meter`
...
(case
  "the same mixed-scale comparison is EXACT over Rational (keeps the fractional scale)"
  (doc
    "`(< (Qty.of 30/1 centimeter) (Qty.of 1/1 foot))` over Rational: each converts to the reference
  ...
  (input
    (if
      (<
        (Qty.of (Rational.of 30 1) (Unit.of #"centimeter"))
        (Qty.of (Rational.of 1 1) (Unit.of #"foot")))
      1
      0))
  (output (: 1 Int64)))

(case
  "a mixed-scale equality finds GENUINE equality after a whole-ratio conversion (5000 m = 5 km)"
  (doc
    "`(= (Qty.of 5000.0 meter) (Qty.of 5.0 kilometer))` — a mixed-scale EQUALITY where the two values
  ...
  (input
    (if
      (= (Qty.of 5000.0 (Unit.base #"meter")) (Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter"))))
      1
      0))
  (output (: 1 Int64)))

(case
  "a mixed-scale equality is FALSE when the converted values differ (5000 m ≠ 6 km)"
  (doc
    "The negative companion of the whole-ratio equality: `(= (Qty.of 5000.0 meter) (Qty.of 6.0
  ...
  (input
    (if
      (= (Qty.of 5000.0 (Unit.base #"meter")) (Qty.of 6.0 (Unit.prefix kilo (Unit.base #"meter"))))
      1
      0))
  (output (: 0 Int64)))
...
(case
  "an if over two same-dimension quantities at different units is a type mismatch (no auto-convert)"
  (doc
    "`(if b (Qty.of 1 kilometer) (Qty.of 500 meter))` — the two branches are the SAME dimension
...
(case
  "an if over two quantities at the SAME unit joins normally"
  (doc
    "`(if b (Qty.of 1000 meter) (Qty.of 500 meter))` — both branches are the SAME `(Qty Int64
  ...
  (input
    (do
      (def
        (main (: b Bool))
        (Qty.value (if b (Qty.of 1000 (Unit.base #"meter")) (Qty.of 500 (Unit.base #"meter")))))
      (export main)))
  (call main (: true Bool))
  (output (: 1000 Int64)))

(case
  "a list of two same-dimension quantities at different units names the scale difference (CDZ0201)"
  (doc
    "The LIST-element peer-join sibling of the if-join scale-mismatch case: `(list (Qty.of 5.0
...
(case
  "remainder (%) on same-dimension integer quantities keeps the unit (7m % 3m = 1m)"
  (doc
    "`(% (Qty.of 7 meter) (Qty.of 3 meter))` — remainder on same-dimension integer quantity operands —
...
(case
  "remainder (%) of quantities of incompatible dimension is CDZ0501"
  (doc
    "`(% (Qty.of 7 meter) (Qty.of 3 second))` — a cross-DIMENSION remainder — is a dimensional error
...
(case
  "remainder (%) on a floating-point quantity is CDZ0301 (no float remainder)"
  (doc
    "`(% (Qty.of 7.0 meter) (Qty.of 3.0 meter))` — a remainder on a FLOAT-inner quantity — is rejected
...
(case
  ...
  (doc
    "`(+ (* (Qty.of 2.0 meter) (Qty.of 2.0 meter)) (Qty.of 1.0 (Unit.^ meter 2)))` adds an area
  ...
  (output
    (: (Qty.of 5.0 (Unit.^ (Unit.base #"meter") 2)) (Qty Float64 (Unit.^ (Unit.base #"meter") 2)))))
...
(case
  ...
  (doc
    "`(: (* (Qty.of 2.0 meter) (Qty.of 3.0 meter)) (Qty Float64 meter))` annotates a product whose
...
(case
  "annotating a quantity at a same-dimension DIFFERENT unit is accepted (dimension checked, not scale)"
  (doc
    "`(: (Qty.of 1 kilometer) (Qty Int64 meter))` annotates a kilometer quantity at meter — the SAME
  ...
  (input
    (Qty.value
      (: (Qty.of 1 (Unit.prefix kilo (Unit.base #"meter"))) (Qty Int64 (Unit.base #"meter")))))
  (output (: 1 Int64)))
...
(case
  "a same-dimension annotation preserves the value's own scale through a conversion"
  (doc
    "`(Unit.in meter (: (Qty.of 1 kilometer) (Qty Int64 meter)))` — the annotation CHECKS the
...
(case
  "a same-dimension annotation is the identity under its own unit's conversion"
  (doc
    "`(Unit.in kilometer (: (Qty.of 1 kilometer) (Qty Int64 meter)))` — converting the annotated
...
(case
  "an annotated quantity joins additions at its own scale, no silent mixed-scale bypass"
  (doc
    "`(+ (: (Qty.of 1 km) (Qty Int64 meter)) (Qty.of 2 km))` in meters — the annotated operand is
...
(case
  "a same-dimension quantity annotation whose inner numeric type differs is still an error"
  (doc
    "`(: (Qty.of 1 kilometer) (Qty Float64 meter))` shares the dimension (length) but the value's
  ...
  (input
    (Qty.value
      (: (Qty.of 1 (Unit.prefix kilo (Unit.base #"meter"))) (Qty Float64 (Unit.base #"meter")))))
  (error CDZ0203))
...
(case
  "a quantity annotation range-checks the inner width — an out-of-range magnitude is rejected"
  (doc
    "`(: (Qty.of 300 kilometer) (Qty UInt8 meter))` — the annotation checks the dimension (km and
...
(case
  "a quantity annotation at a same-unit out-of-range magnitude is rejected"
  (doc
    "`(: (Qty.of 300 meter) (Qty UInt8 meter))` — a SAME-UNIT annotation (the magnitude unit equals the
...
(case
  "a quantity annotation rejects a negative magnitude at an unsigned inner width"
  (doc
    "`(: (Qty.of -1 meter) (Qty UInt8 meter))` — a same-UNIT annotation whose inner width is unsigned:
...
(case
  "a quantity annotation at an in-range narrow width grounds and accepts"
  (doc
    "`(: (Qty.of 5 kilometer) (Qty UInt8 meter))` — the control: 5 fits UInt8 (0..=255), so the
...
(case
  ...
  (doc
    "`(= (Qty.value (Qty.of 5.0 meter)) 5.0)` is true: the value recovered from a quantity is the
...
(case
  "the underlying numeric type obeys the numeric core — no silent promotion under a unit"
  (doc
    "`(+ (Qty.of 2 (Unit.base #\"meter\")) (Qty.of 3.0 (Unit.base #\"meter\")))` adds an Int64
...
(case
  "a function deriving a velocity from a distance and a time"
  (doc
    "`speed` takes a distance `(Qty Float64 meter)` and a time `(Qty Float64 second)` and returns
  ...
  (input
    (do
      (def (speed d t) (/ d t))
      (def (main) (speed (Qty.of 100.0 (Unit.base #"meter")) (Qty.of 4.0 (Unit.base #"second"))))
      (export main)))
  (output
    (:
      (Qty.of 25.0 (Unit./ (Unit.base #"meter") (Unit.base #"second")))
      (Qty Float64 (Unit./ (Unit.base #"meter") (Unit.base #"second"))))))
...
(case
  "a quantity over an exact rational magnitude divides exactly and derives its dimension"
  (doc
    "`(/ (Qty.of (Rational.of 1 3) feet) (Qty.of (Rational.of 1 2) second))` divides an exact
  ...
  (input
    (/
      (Qty.of (Rational.of 1 3) (Unit.base #"feet"))
      (Qty.of (Rational.of 1 2) (Unit.base #"second"))))
  (output
    (:
      (Qty.of 2/3 (Unit./ (Unit.base #"feet") (Unit.base #"second")))
      (Qty Rational (Unit./ (Unit.base #"feet") (Unit.base #"second"))))))

(case
  "adding rational-magnitude quantities of incompatible dimension is still a compile-time error"
  (doc
    "`(+ (Qty.of (Rational.of 1 2) feet) (Qty.of (Rational.of 1 2) second))` adds an exact-rational
  ...
  (input
    (+
      (Qty.of (Rational.of 1 2) (Unit.base #"feet"))
      (Qty.of (Rational.of 1 2) (Unit.base #"second"))))
  (error CDZ0501))

(case
  "a rational-magnitude quantity erases to its exact rational value"
  (doc
    "`(Qty.value (Qty.of (Rational.of 1 3) feet))` = 1/3 : Rational. The unit erases (checked then
...
(case
  "converting a unit to another unit of the same dimension is an exact rational scale"
  (doc
    "`(Unit.in (Unit.of #\"meter\") (Qty.of (Rational.of 1 1) (Unit.of #\"inch\")))` converts 1 inch
...
(case
  "a chained Unit.in re-wrapped with Qty.of converts inch to cm exactly (127/50)"
  (doc
    "The two-step within-dimension conversion the chained-Unit.in CDZ0501 repair suggests: because
...
(case
  "adding two units of one dimension converts to the reference unit exactly"
  (doc
    "THE mixing case: `(+ (Qty 1 inch) (Qty 1 mm))` over exact rational magnitudes — both are
...
(case
  "mixing units of DIFFERENT dimensions is still a compile-time error"
  (doc
    "`(+ (Qty 1 inch) (Qty 1 second))` mixes `length` and `time` — DIFFERENT dimensions — so it is
  ...
  (input
    (+ (Qty.of (Rational.of 1 1) (Unit.of #"inch")) (Qty.of (Rational.of 1 1) (Unit.of #"second"))))
  (error CDZ0501))

(case
  "comparing two units of one dimension converts before comparing"
  (doc
    "`(< (Qty 25 mm) (Qty 1 inch))` compares a length in mm to a length in inches — same dimension,
  ...
  (input
    (<
      (Qty.of (Rational.of 25 1) (Unit.of #"millimeter"))
      (Qty.of (Rational.of 1 1) (Unit.of #"inch"))))
  (output (: true Bool)))
...
(case
  "an SI decimal prefix scales a unit by a power of ten"
  (doc
    "`(Unit.in (Unit.of #\"meter\") (Qty.of (Rational.of 3 1) (Unit.prefix kilo (Unit.of #\"meter\"))))`
...
(case
  "a negative-power SI prefix is an exact rational scale"
  (doc
    "`(Unit.in (Unit.of #\"second\") (Qty.of (Rational.of 5 1) (Unit.prefix milli (Unit.of #\"second\"))))`
...
(case
  "an IEC binary prefix scales a unit by a power of two"
  (doc
    "`(Unit.in (Unit.of #\"byte\") (Qty.of (Rational.of 1 1) (Unit.prefix mebi (Unit.of #\"byte\"))))`
...
(case
  "a decimal kilobyte and a binary kibibyte are distinct units of one dimension"
  (doc
    "`(+ (Qty 1 KiB) (Qty 1 kB))` over the `information` dimension: kibibyte = 1024 byte and
  ...
  (input
    (+
      (Qty.of (Rational.of 1 1) (Unit.prefix kibi (Unit.of #"byte")))
...
(case
  "a prefixed unit combines with its base by converting to the reference (Float)"
  (doc
    "`(+ (Qty.of 1.0 (Unit.prefix kilo meter)) (Qty.of 500.0 meter))` mixes kilometers and meters —
  ...
  (input
    (Qty.value
      (+ (Qty.of 1.0 (Unit.prefix kilo (Unit.base #"meter"))) (Qty.of 500.0 (Unit.base #"meter")))))
  (output (: 1500.0 Float64)))

(case
  "a decimal kilobyte and a binary kibibyte convert distinctly over Int64"
  (doc
    "`(+ (Qty 1 KiB) (Qty 1 kB))` over Int64: kibibyte = 1024 byte and kilobyte = 1000 byte are
...
(case
  "a negative-power SI prefix converts over Float"
  (doc
    "`(Unit.in (Unit.of #\"second\") (Qty.of 5.0 (Unit.prefix milli (Unit.of #\"second\"))))` converts
...
(case
  "an IEC binary prefix converts exactly over Int64"
  (doc
    "`(Unit.in (Unit.of #\"byte\") (Qty.of 1 (Unit.prefix mebi (Unit.of #\"byte\"))))` converts 1 MiB
...
(case
  "comparing quantities of one dimension at different scales converts before comparing"
  (doc
    "`(< (Qty 500.0 meter) (Qty 1.0 (Unit.prefix kilo meter)))` compares meters to kilometers — one
           ...
           rejecting them.")
  (input
    (< (Qty.of 500.0 (Unit.base #"meter")) (Qty.of 1.0 (Unit.prefix kilo (Unit.base #"meter")))))
  (output (: true Bool)))

(case
  "mixing a prefixed unit across DIFFERENT dimensions is still a compile-time error"
  (doc
    "`(+ (Qty 1.0 (Unit.prefix kilo meter)) (Qty 1.0 second))` mixes `length` and `time` — different
...
(case
  "adding two named family units of one dimension converts to the reference (Float)"
  (doc
    "`(+ (Qty.of 1.0 (Unit.of #\"inch\")) (Qty.of 1.0 (Unit.of #\"millimeter\")))` — inch and
  ...
  (input (Qty.value (+ (Qty.of 1.0 (Unit.of #"inch")) (Qty.of 1.0 (Unit.of #"millimeter")))))
  (output (: 0.0264 Float64)))

(case
  "a named family unit combines across DIFFERENT dimensions is a compile-time error"
  (doc
    "`(+ (Qty.of 1.0 (Unit.of #\"inch\")) (Qty.of 1.0 (Unit.of #\"second\")))` mixes `length` and
...
(case
  "a plural family-unit spelling names the same unit as its singular"
  (doc
  ...
  (input (Qty.value (+ (Qty.of 1.0 (Unit.of #"meters")) (Qty.of 4.0 (Unit.of #"feet")))))
...
(case
  "Unit.in converts a quantity to a chosen larger unit (Float)"
  (doc
    "`(Unit.in meter (Qty.of 3.0 kilometer))` converts 3 km to meters and UNWRAPS: `Unit.in`/`as`
...
(case
  "Unit.in converts a quantity to a chosen smaller unit exactly (Int)"
  (doc
    "`(Unit.in kilometer (Qty.of 2000 meter))` converts 2000 m to kilometers and UNWRAPS to the
...
(case
  "Unit.in to a unit of a different dimension is a compile-time error"
  (doc
    "`(Unit.in meter (Qty.of 3.0 second))` asks to convert a time to a length — different
...
(case
  "chaining two Unit.in conversions is a compile-time error — the inner one already unwrapped"
  (doc
    "`(Unit.in centimeter (Unit.in millimeter (Qty.of 1 inch)))` — the INNER `Unit.in` UNWRAPS to a
...
(case
  "Qty.value of a conversion result is a compile-time error — the conversion already unwrapped"
  (doc
    "`(Qty.value (Unit.in inch (Qty.of 5 foot)))` — the `Unit.in` UNWRAPS to a bare number (Q3: 60,
  ...
  (error
    CDZ0501
    (message "recovers a quantity")
    (message "which is not a quantity")
    (message "already UNWRAPS to a bare number")
    (not "no machine representation")))

(case
  "Unit.in of a NON-NUMERIC operand names the type, not a self-contradictory plain number"
  (doc
    "`(Unit.in meter true)` applies the unit conversion to a Bool — not a quantity. CDZ0501 names the
...
(case
  "Qty.value of a NON-NUMERIC operand names the type, not a self-contradictory plain number"
  (doc
    "`(Qty.value true)` extracts a magnitude from a Bool — not a quantity. CDZ0501 names the real
...
(case
  "re-wrapping the intermediate with Qty.of makes a two-step conversion well-formed"
  (doc
    "The repair for the chained-`Unit.in` error: wrap the first conversion's bare result back into a
  ...
  (input
    (let
      ((mm (Unit.in (Unit.of #"millimeter") (Qty.of (Rational.of 1 1) (Unit.of #"inch")))))
      (Unit.in (Unit.of #"centimeter") (Qty.of mm (Unit.of #"millimeter")))))
  (output (: 127/50 Rational)))

(case
  "an unwrapped conversion result is a bare number under ordinary numeric rules"
  (doc
    "`Unit.in` UNWRAPS: its result is a bare dimensionless number, so ordinary numeric arithmetic
...
(case
  "an unwrapped length adds to an unwrapped time with no dimension error"
  (doc
    "The unwrap is a REAL exit from dimensional checking: two `Unit.in` results of DIFFERENT
  ...
  (input
    (+
      (Unit.in (Unit.of #"meter") (Qty.of 3 (Unit.of #"kilometer")))
...
(case
  "a runtime mixed-unit sum emits the scale conversion (Int)"
  (doc
    "`(+ (Qty.of v kilometer) (Qty.of 500 meter))` with `v` a runtime Int64 parameter: km converts
...
(case
  "a runtime MIXED-SCALE BigInt-magnitude sum converts to the reference in unbounded arithmetic"
  (doc
    "`(+ (Qty.of (BigInt.of v) kilometer) (Qty.of (BigInt.of 500) meter))` — a MIXED-SCALE combine
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (Qty.of (BigInt.of v) (Unit.prefix kilo (Unit.base #"meter")))
...
(case
  ...
  (doc
    "A parameter annotated at a NON-reference unit — `(: a (Qty Int64 kilometer))` — must keep its
  ...
  (input
    (do
      (def
        (main
          (: a (Qty Int64 (Unit.prefix kilo (Unit.base #"meter"))))
          (: b (Qty Int64 (Unit.base #"meter"))))
        (Qty.value (+ a b)))
      (export main)))
  (call main (: 2 Int64) (: 500 Int64))
  (output (: 2500 Int64)))
...
(case
  "a runtime mixed-scale combine converts a COMPUTED BigInt quantity operand"
  (doc
    "`(+ (* (Qty.of (BigInt.of n) kilometer) (BigInt.of 2)) (Qty.of (BigInt.of 500) meter))` — the
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (* (Qty.of (BigInt.of n) (Unit.prefix kilo (Unit.base #"meter"))) (BigInt.of 2))
            (Qty.of (BigInt.of 500) (Unit.base #"meter")))))
      (export main)))
  (call main (: 3 Int64))
  (output (: 6500 BigInt))
...
(case
  ...
  (doc
    "The Float companion: `(+ (* (Qty.of n kilometer) 2.0) (Qty.of 500.0 meter))` with a runtime
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (* (Qty.of n (Unit.prefix kilo (Unit.base #"meter"))) 2.0)
            (Qty.of 500.0 (Unit.base #"meter")))))
      (export main)))
  (call main (: 3.0 Float64))
  (output (: 6500.0 Float64)))

(case
  "a runtime MIXED-SCALE BigInt comparison converts to the reference before comparing"
  (doc
    "`(< (Qty.of (BigInt.of v) kilometer) (Qty.of (BigInt.of 5000) meter))` — a mixed-scale BigInt
    ...
    (do
      (def
        ...
        (if
          (<
            (Qty.of (BigInt.of v) (Unit.prefix kilo (Unit.base #"meter")))
            (Qty.of (BigInt.of 5000) (Unit.base #"meter")))
          1
          0))
      (export main)))
  (call main (: 2 Int64))
...
(case
  "a runtime MIXED-SCALE Rational-magnitude sum converts to the reference EXACTLY"
  (doc
    "`(+ (Qty.of (Rational.of v 1) kilometer) (Qty.of (Rational.of 500 1) meter))` with a RUNTIME
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (Qty.of (Rational.of v 1) (Unit.prefix kilo (Unit.base #"meter")))
...
(case
  ...
  (doc
    "THE exact-mixing case at RUNTIME: `(+ (Qty.of (Rational.of v 1) inch) (Qty.of (Rational.of 1 1)
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (Qty.of (Rational.of v 1) (Unit.of #"inch"))
            (Qty.of (Rational.of 1 1) (Unit.of #"millimeter")))))
      (export main)))
  (call main (: 1 Int64))
...
(case
  ...
  (doc
    "`(Unit.in meter (Qty.of v kilometer))` with `v` a runtime Int64: converts v km to meters by
           *1000 at run time, so v=3 → 3000 m. The explicit-conversion companion of the runtime mixed
           sum.")
  (input
    (do
      (def (main (: v Int64)) (Unit.in (Unit.of #"meter") (Qty.of v (Unit.of #"kilometer"))))
      (export main)))
  (call main (: 3 Int64))
  (output (: 3000 Int64)))

(case
  "Unit.in over a BigInt-magnitude quantity converts in unbounded arithmetic and unwraps"
  (doc
    "`(Unit.in meter (Qty.of (BigInt.of v) kilometer))` — an explicit conversion of a `(Qty BigInt
...
(case
  ...
  (doc
    "`(Unit.in kilometer (Qty.of (BigInt.of v) meter))` — 2500 m in km, 2500/1000 does not divide,
...
(case
  ...
  (doc
    "`(Unit.in meter (Qty.of (Rational.of v 1) kilometer))` with a RUNTIME `v`: converts the
...
(case
  ...
  (doc
    "`(Unit.in meter (Qty.of (Rational.of v 1) inch))` with a runtime `v`: inch = 127/5000 m, so
...
(case
  "a named rate unit converts to the reference rate (Float)"
  (doc
    "`(Unit.in byte-per-second (Qty.of 1.0 mbps))` converts 1 megabit-per-second to bytes-per-
...
(case
  "a rate derived by division mixes with a named rate unit of the same dimension"
  (doc
    "`(bytes / seconds)` derives the dimension `byte/second` — the SAME dimension `mbps` names — so
...
(case
  "combining a named rate with a length is a dimensional error"
  (doc
    "`(+ (Qty.of 1.0 mbps) (Qty.of 1.0 meter))` combines a rate (`information/time`) with a length
...
(case
  "a program declares its own family unit and converts with it"
  (doc
    "`(Unit.define #\"furlong\" (Unit.of #\"foot\") 660 1)` declares a furlong as 660 feet; then
...
(case
  "declaring a unit with a conversion conflicting with a built-in is an error"
  (doc
    "`(Unit.define #\"foot\" (Unit.of #\"meter\") 2 1)` redeclares the built-in `foot` (381/1250 m)
  ...
  (input (do (Unit.define #"foot" (Unit.of #"meter") 2 1) (def (main) 0) (export main)))
  (error CDZ0502))

(case
  "an inline Unit.define conflicting with a built-in conversion is rejected wherever it occurs"
  (doc
    "The name->conversion-uniqueness rule (#A Named Unit's Conversion Is Unique) holds wherever a
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (Qty.of a (Unit.define #"foot" (Unit.base #"meter") 2 1))
            (Qty.of a (Unit.base #"meter")))))
      (export main)))
  (error CDZ0502))

(case
  "redeclaring a built-in unit with its own conversion is admissible"
  (doc
    "`(Unit.define #\"foot\" (Unit.of #\"meter\") 381 1250)` redeclares the built-in `foot` at its
...
(case
  "an agreeing redeclaration compares the normalized ratio, not the literal numerator and denominator"
  (doc
    "`(Unit.define #\"foot\" (Unit.of #\"meter\") 762 2500)` restates the built-in `foot` as 762/2500
...
(case
  "redeclaring a user-declared unit with the same conversion is admissible"
  (doc
    "`(Unit.define #\"span\" (Unit.of #\"meter\") 3 1)` twice declares `span` = 3 m identically — the
...
(case
  "a plain number def is unaffected by a following statement (the `as` sugar stays in its statement)"
  (doc
    "`def a() = 5.0` is a number, full stop. On the ML surface the `as` unit-conversion postfix
...
(case
  "a runtime magnitude unwraps through a unit conversion"
  (doc
    "`(Unit.in inch (Qty.of n foot))` with n a boundary PARAMETER: the constant cases fold at
  ...
  (input
    (do
      (def (main (: n Int64)) (Unit.in (Unit.of #"inch") (Qty.of n (Unit.of #"foot"))))
      (export main)))
  (call main (: 2 Int64))
  (output (: 24 Int64)))

(case
  "a non-dividing conversion ratio truncates on an Int magnitude"
  (doc
    "`(Unit.in kilometer (Qty.of 2500 meter))` — 2500/1000 does not divide, and the magnitude
...
(case
  "an identity conversion unwraps the magnitude unchanged"
  (doc
    "`(Unit.in meter (Qty.of 5 meter))` — source and target are the SAME unit, scale ratio 1:
...
(case
  "wrapping an unwrapped number re-enters the units world at the new unit"
  (doc
    "The unwrap-then-rewrap round trip: `(Unit.in meter (Qty.of 3 kilometer))` exits with the
  ...
  (input
    (Unit.in
      (Unit.of #"foot")
      (Qty.of (Unit.in (Unit.of #"meter") (Qty.of 3 (Unit.of #"kilometer"))) (Unit.of #"foot"))))
  (output (: 3000 Int64)))
...
(case
  "a bare number on the left scales the quantity and keeps its dimension"
  (doc
    "`(* 3 (Qty.of 2 kilometer))` — the commuted form: the BARE operand is examined first, so an
...
(case
  "dividing a quantity by a bare number keeps its dimension"
  (doc
    "`(/ (Qty.of 6 kilometer) 3)` = 2 km — division is the other multiplicative op the
...
(case
  "the magnitude of a bare-scaled quantity reads back"
  (doc
    "`(Qty.value (* (Qty.of 2 kilometer) 3))` = 6 — the exact expression shape the apply_type
...
(case
  "a squared quantity does not add to a linear quantity"
  (doc
    "`(* (Qty.of 2 meter) (Qty.of 3 meter))` composes dimensions (m²) — the qty×qty arm, NOT
  ...
  (input
    (+
      (* (Qty.of 2 (Unit.of #"meter")) (Qty.of 3 (Unit.of #"meter")))
      (Qty.of 1 (Unit.of #"meter"))))
  (error CDZ0501))
...
(case
  "a BigInt-inner quantity scales by a bare BigInt on the bigint path"
  (doc
    "`(* (Qty.of (BigInt.of 5) meter) (BigInt.of 3))` — a BigInt-inner quantity times a BARE
...
(case
  ...
  (doc
    "`(* (Qty.of (BigInt.of Int64.max) meter) (BigInt.of 2))` = 18446744073709551614 : BigInt —
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (Qty.value (* (Qty.of (BigInt.of 9223372036854775807) (Unit.of #"meter")) (BigInt.of 2))))
      (export main)))
  (call main (: 0 Int64))
...
(case
  ...
  (doc
    "The additive companion of the multiplicative grow-past-Int64.max case above: `(+ (Qty.of
...
(case
  "a squared BigInt-inner quantity added to a linear one is a dimension mismatch"
  (doc
    "The same-base power dimension distinction, through a BigInt inner: `(+ (Qty.pow (Qty.of (BigInt.of
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (+
            (Qty.pow (Qty.of (BigInt.of n) (Unit.base #"meter")) 2)
            (Qty.of (BigInt.of n) (Unit.base #"meter")))))
      (export main)))
  (call main (: 5 Int64))
  (error CDZ0501))
...
(case
  "a Rational-inner quantity addition stays exact through the quantity wrapper"
  (doc
    "`(+ (Qty.of 1/3 meter) (Qty.of 1/6 meter))` → `Qty.value` reads back 1/2 : Rational — the
  ...
  (input
    (Qty.value
      (+
        (Qty.of (Rational.of 1 3) (Unit.of #"meter"))
...
(case
  "a narrow-width Int quantity add that overflows the inner width traps at compile time"
  (doc
    "`(+ (Qty.of (Int8.of 100) meter) (Qty.of (Int8.of 100) meter))` — a same-unit add over an
  ...
  (input
    (Qty.value
      (+ (Qty.of (Int8.of 100) (Unit.base #"meter")) (Qty.of (Int8.of 100) (Unit.base #"meter")))))
  (error CDZ0304))

(case
  "a RUNTIME erased Qty add is still CHECKED at the inner width (overflow traps at run time)"
  (doc
    "The RUNTIME twin of the compile-time CDZ0304 quantity-overflow pins around it, closing the
  ...
  (input
    (do
      (def (main (: v Int64)) (let ((q (Qty.of v (Unit.base #"meter")))) (Qty.value (+ q q))))
      (export main)))
...
(case
  "a narrow-width Int quantity add that fits the inner width runs normally"
  (doc
    "`(+ (Qty.of (Int8.of 50) meter) (Qty.of (Int8.of 50) meter))` = 100, which FITS Int8 — the
...
(case
  "a mixed-scale narrow-width Int quantity combine that overflows the inner width traps"
  (doc
    "`(+ (Qty.of (UInt8.of 1) kilometer) (Qty.of (UInt8.of 50) meter))` — a MIXED-scale combine
...
(case
  "a narrow-width Int quantity stored as a map value round-trips through Map.lookup"
  (doc
    "A `(Qty Int8 meter)` stored as a MAP VALUE, read back via `Map.lookup` (→ `Option`), and let the
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (match
            (Map.lookup (Map.insert (Map.empty) 1 (Qty.of (Int8.of 100) (Unit.base #"meter"))) 1)
            ((Some q) q)
...
(case
  "Qty map VALUES unwrap, ADD, and COMPARE in one chain preserving the unit through the collection"
  (doc
    "The WORKING-chain face of quantities in collections (the case above round-trips ONE value):
  ...
  (input
    (do
      (def
        (getq (: m (Map Int64 (Qty Int64 (Unit.base #"meter")))) (: k Int64))
        (match (Map.lookup m k) ((Some q) q) ((None _u) (Qty.of 0 (Unit.base #"meter")))))
      (def
        (main (: k Int64))
        (do
          (def
            m
            (Map.insert
              (Map.insert Map.empty 1 (Qty.of 10 (Unit.base #"meter")))
              2
              (Qty.of 25 (Unit.base #"meter"))))
          (def s (+ (getq m 1) (getq m k)))
          (+ (* (Qty.value s) 10) (if (> s (Qty.of 30 (Unit.base #"meter"))) 1 0))))
      (export main)))
  (call main (: 2 Int64))
  (output (: 351 Int64))
  (call main (: 9 Int64))
...
(case
  "a tuple with a Qty leaf as a map key hits by magnitude-and-unit content"
  (doc
    "The COMPOUND-key face (the bare-Qty-key cluster below pins Int/BigInt/Rational inners
    ...
    (do
      (def
        ...
        (do
          ...
          (match
            (Map.lookup m #tuple((Qty.of 10 (Unit.base #"meter")) 3))
            ((Some x) x)
            ((None _u) -1))))
      (export main)))
  (call main (: 10 Int64))
...
(case
  "a quantity read from a map COMBINES with a fresh same-dimension quantity"
  (doc
    "The arithmetic face of the Qty-in-collection round-trip (the round-trip pins above read and
  ...
  (input
    (do
      (def
        (main (: n Int64))
        (let
          ((m (Map.insert Map.empty 1 (Qty.of n (Unit.base #"meter")))))
          (Qty.value
            (match
              (Map.lookup m 1)
...
(case
  "a Float32 quantity stored as a map value round-trips through Map.lookup"
  (doc
    "The Float32 analogue of the narrow-Int map-value case. A `(Qty Float32 meter)` stored as a MAP
  ...
  (input
    (do
      (def
        ...
        (Qty.value
          (match
            (Map.lookup (Map.insert (Map.empty) 1 (Qty.of (Float32.of 2.5) (Unit.base #"meter"))) 1)
            ((Some q) q)
...
(case
  "a nominal newtype over a Float32 quantity stored as a map value round-trips"
  (doc
    "The nominal-newtype layer over the Float32-quantity map case. `(type Len (Q (Qty Float32 meter)))`
  ...
  (input
    (do
      ...
      (def
        ...
        (match
          (Map.lookup
            (Map.insert (Map.empty) 1 (Len.Q (Qty.of (Float32.of 2.5) (Unit.base #"meter"))))
            1)
          ((Some _) 1)
          ((None) 0)))
      (export main)))
...
(case
  "a runtime quantity used as a Map key hits when a separately-built equal key is looked up"
  (doc
    "Insert under key `(Qty.of v kilometer)` with `v` a runtime Int64, then look up a key built
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k (Qty.of v (Unit.prefix kilo (Unit.base #"meter")))) (m (Map.insert (Map.empty) k 42)))
          (match
            (Map.lookup m (Qty.of v (Unit.prefix kilo (Unit.base #"meter"))))
            ((Some found) found)
            ((None) 0))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a runtime quantity Map key MISSES when the looked-up magnitude differs"
  (doc
    "The negative twin of the quantity-key hit: insert under `(Qty.of v kilometer)`, then look up
    ...
    (do
      (def
        ...
        (let
          ((k (Qty.of v (Unit.prefix kilo (Unit.base #"meter")))) (m (Map.insert (Map.empty) k 42)))
          (match
            (Map.lookup m (Qty.of (+ v 1) (Unit.prefix kilo (Unit.base #"meter"))))
            ((Some found) found)
            ((None) 0))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a DERIVED-dimension (velocity) quantity used as a Map key hits by content"
  (doc
    "The prior key cases use a BASE-dimension quantity key; this pins a DERIVED (quotient) dimension —
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k (/ (Qty.of v (Unit.base #"meter")) (Qty.of 2 (Unit.base #"second"))))
            (m (Map.insert (Map.empty) k 42)))
          (match
            (Map.lookup m (/ (Qty.of v (Unit.base #"meter")) (Qty.of 2 (Unit.base #"second"))))
            ((Some found) found)
            ((None) 0))))
      (export main)))
  (call main (: 8 Int64))
...
(case
  "a runtime BigInt-inner quantity used as a Map key hits, and a different magnitude misses"
  (doc
    "`(Qty (BigInt.of v) meter)` as a Map key with `v` a runtime Int64: a BigInt inner erases to a
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k (Qty.of (BigInt.of v) (Unit.base #"meter"))) (m (Map.insert (Map.empty) k 42)))
          (+
            (match
              (Map.lookup m (Qty.of (BigInt.of v) (Unit.base #"meter")))
              ((Some found) found)
              ((None) 0))
            (match
              (Map.lookup m (Qty.of (BigInt.of (+ v 1)) (Unit.base #"meter")))
              ((Some found) found)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a runtime Rational-inner quantity used as a Map key hits, and a different magnitude misses"
  (doc
    "The Rational twin of the BigInt-key case: `(Qty (Rational.of v 2) meter)` as a Map key. A Rational
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k (Qty.of (Rational.of v 2) (Unit.base #"meter"))) (m (Map.insert (Map.empty) k 42)))
          (+
            (match
              (Map.lookup m (Qty.of (Rational.of v 2) (Unit.base #"meter")))
              ((Some found) found)
              ((None) 0))
            (match
              (Map.lookup m (Qty.of (Rational.of (+ v 1) 2) (Unit.base #"meter")))
              ((Some found) found)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a Rational-inner quantity Map key matches an equal value written with a different spelling"
  (doc
    "Rational canonicalizes at construction, so a `(Qty (Rational.of v 2) meter)` key is looked up by
    ...
    (do
      (def
        ...
        (let
          ((k (Qty.of (Rational.of v 2) (Unit.base #"meter"))) (m (Map.insert (Map.empty) k 42)))
          (match
            (Map.lookup m (Qty.of (Rational.of (* v 2) 4) (Unit.base #"meter")))
            ((Some found) found)
            ((None) 0))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a trie of 30 QUANTITY keys resolves a cross-normalized magnitude lookup at depth"
  (doc
    "The Qty-key rows above run on single-key maps; this pins a POPULATED trie of Rational-inner
  ...
  (input
    (do
      (def
        (fill (: i Int64) (: m (Map (Qty Rational (Unit.base #"meter")) Int64)))
        (if
          (= i 0)
          m
      ...
      (def
        ...
        (do
          ...
          (+
            (* 10 (Map.len m))
            (match
              (Map.lookup m (Qty.of (Rational.of 10 4) (Unit.base #"meter")))
              ((Some v) v)
              ((None _u) -1)))))
      (export main)))
  (call main (: 30 Int64))
...
(case
  "a Qty-keyed trie churned with differently-normalized magnitudes equals the direct build"
  (doc
    "The normalization-identity churn for quantity keys: 24 keys (i = 1..n-1 at n = 25) INSERTED as `2i/4 meter` and
  ...
  (input
    (do
      (def
        (grow (: i Int64) (: n Int64) (: m (Map (Qty Rational (Unit.base #"meter")) Int64)))
        (if
          (= i n)
          m
          (grow (+ i 1) n (Map.insert m (Qty.of (Rational.of (* i 2) 4) (Unit.base #"meter")) i))))
      (def
        (shrink (: i Int64) (: n Int64) (: m (Map (Qty Rational (Unit.base #"meter")) Int64)))
        (if
          (= i n)
          m
          (shrink (+ i 1) n (Map.remove m (Qty.of (Rational.of i 2) (Unit.base #"meter"))))))
      (def
        (main (: n Int64))
        (do
          ...
          (+
            ...
            (match
              (Map.lookup churned (Qty.of (Rational.of 1998 2) (Unit.base #"meter")))
              ((Some v) (if (= v 50) 1 0))
              ((None _u) -1)))))
      (export main)))
  (call main (: 25 Int64))
...
(case
  "two equal runtime quantities in a Set dedup to one element"
  (doc
    "`(Set.of (list (Qty.of v kilometer) (Qty.of v kilometer)))` with `v` a runtime Int64: both
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (Set.len
          #set((Qty.of v (Unit.prefix kilo (Unit.base #"meter")))
...
(case
  "distinct-magnitude runtime quantities in a Set stay separate"
  (doc
    "The negative twin of the dedup case: `(Set.of (list (Qty.of v kilometer) (Qty.of (+ v 1)
...
(case
  "Set.contains finds a separately-built equal quantity but not a non-member"
  (doc
    "`Set.contains` over a Set of runtime quantities: a query quantity built INDEPENDENTLY from the
...
(case
  "a runtime quantity stored in a List reads back through List.at and unwraps"
  (doc
    "`(List.at (list (Qty.of v km) (Qty.of (+ v 1) km)) 1)` with `v` a runtime Int64: the list holds
  ...
  (input
    (do
      (def
        ...
        (match
          (List.at
            #list((Qty.of v (Unit.prefix kilo (Unit.base #"meter")))
              (Qty.of (+ v 1) (Unit.prefix kilo (Unit.base #"meter"))))
            1)
          ((Some q) (Qty.value q))
          ((None) 0)))
...
(case
  "a narrow-Int quantity stored in a List reads back through List.at and unwraps"
  (doc
    "The narrow-inner twin of the List round-trip: a `(Qty Int8 meter)` List element, read back via
    ...
    (do
      (def
        ...
        (Qty.value
          (match
            (List.at
              #list((Qty.of (Int8.of 100) (Unit.base #"meter"))
...
(case
  "a BigInt-inner quantity Set dedups equal elements and keeps distinct ones"
  (doc
    "`(Qty (BigInt.of v) meter)` as a Set element with `v` a runtime Int64. A BigInt inner erases to a
  ...
  (input
    (do
      (def
        ...
        (+
          (Set.len
            #set((Qty.of (BigInt.of v) (Unit.base #"meter"))
...
(case
  "a Rational-inner quantity Set dedups equal elements and keeps distinct ones"
  (doc
    "The Rational twin of the BigInt Set case: `(Qty (Rational.of v 2) meter)` elements. A Rational
  ...
  (input
    (do
      (def
        ...
        (+
          (Set.len
            #set((Qty.of (Rational.of v 2) (Unit.base #"meter"))
...
(case
  "a BigInt-inner quantity stored in a List reads back through List.at"
  (doc
    "`(List.at (list (Qty (BigInt v) m) (Qty (BigInt v+1) m)) 1)` with `v` a runtime Int64: the list
  ...
  (input
    (do
      (def
        ...
        (match
          (List.at
            #list((Qty.of (BigInt.of v) (Unit.base #"meter"))
              (Qty.of (BigInt.of (+ v 1)) (Unit.base #"meter")))
            1)
          ((Some q) (Qty.value q))
          ((None) (BigInt.of 0))))
...
(case
  ...
  (doc
    "The Rational twin of the BigInt List case: `(List.at (list (Qty (Rational v 2) m) …) 0)`. A
  ...
  (input
    (do
      (def
        ...
        (match
          (List.at
            #list((Qty.of (Rational.of v 2) (Unit.base #"meter"))
              (Qty.of (Rational.of (+ v 1) 2) (Unit.base #"meter")))
            0)
          ((Some q) (Qty.value q))
...
(case
  "a list-of-quantities used as a Map key canonicalizes and hits"
  (doc
    "`(list (Qty v m) (Qty (+ v 1) m))` as a Map key: a list-CONTAINING key is canonicalized at the
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k #list((Qty.of v (Unit.base #"meter")) (Qty.of (+ v 1) (Unit.base #"meter"))))
            (m (Map.insert (Map.empty) k 42)))
          (+
            (match
              (Map.lookup
                m
                #list((Qty.of v (Unit.base #"meter")) (Qty.of (+ v 1) (Unit.base #"meter"))))
              ((Some found) found)
              ((None) 0))
            (match
              (Map.lookup
                m
                #list((Qty.of v (Unit.base #"meter")) (Qty.of (+ v 2) (Unit.base #"meter"))))
              ((Some found) found)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a Set of lists-of-quantities dedups equal members and keeps distinct ones"
  (doc
    "The Set twin, on the canonicalization path: `Set.of` over two `(list (Qty …))` members. Two equal
  ...
  (input
    (do
      (def
        ...
        (+
          (Set.len
            #set(#list((Qty.of v (Unit.base #"meter"))) #list((Qty.of v (Unit.base #"meter")))))
          (*
            10
            (Set.len
              #set(#list((Qty.of v (Unit.base #"meter")))
...
(case
  "a NESTED list-of-lists-of-quantities used as a Map key canonicalizes and hits"
  (doc
    "The quantity-element shape peel recurses through nesting: a `(list (list (Qty …)))` Map key — a
  ...
  (input
    (do
      (def
        (main (: v Int64))
        (let
          ((k #list(#list((Qty.of v (Unit.base #"meter"))))) (m (Map.insert (Map.empty) k 42)))
          (+
            (match
              (Map.lookup m #list(#list((Qty.of v (Unit.base #"meter")))))
              ((Some x) x)
              ((None) 0))
            (match
              (Map.lookup m #list(#list((Qty.of (+ v 1) (Unit.base #"meter")))))
              ((Some x) x)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a Set of tuples-containing-a-quantity dedups equal members and keeps distinct ones"
  (doc
    "The tuple analogue of the list-of-Qty Set case: `Set.of` over `(tuple (Qty …) v)` members. The
  ...
  (input
    (do
      (def
        ...
        (+
          (Set.len
            #set(#tuple((Qty.of v (Unit.base #"meter")) v)
...
(case
  "a Map VALUE that is a list-of-quantities round-trips through Map.lookup and List.at"
  (doc
    "A quantity nested in a compound Map VALUE (not key): the map holds `1 → (list (Qty v m) (Qty (v+1)
...
(case
  ...
  (doc
    "The value-form RENDER of a whole Map whose VALUE is a quantity (the render companion of the
  ...
  (output
    (:
      #map((= 1 (Qty.of 5000.0 (Unit.base #"meter"))))
      (Map Int64 (Qty Float64 (Unit.base #"meter"))))))

(case
  "a whole LIST of quantities renders every element scaled to reference in the value form"
  (doc
    "The list sibling of the whole-Map-value render: a `(List (Qty …))` returned WHOLE renders every
  ...
  (output
    (:
      #list((Qty.of 5000.0 (Unit.base #"meter")) (Qty.of 2000.0 (Unit.base #"meter")))
...
(case
  "a nominal newtype over a quantity is a Map key that hits by content"
  (doc
    "`(type Len (Q (Qty Int64 meter)))` — a nominal newtype over a quantity — used as a Map key. The
  ...
  (input
    (do
      ...
      (def
        (main (: v Int64))
        (let
          ((m (Map.insert (Map.empty) (Len.Q (Qty.of v (Unit.base #"meter"))) 42)))
          (+
            (match (Map.lookup m (Len.Q (Qty.of v (Unit.base #"meter")))) ((Some x) x) ((None) 0))
            (match
              (Map.lookup m (Len.Q (Qty.of (+ v 1) (Unit.base #"meter"))))
              ((Some x) x)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a Set of nominal-over-quantity newtypes dedups equal members"
  (doc
    "The Set twin: `Set.of` over `(Len.Q (Qty …))` members. Two equal nominal-over-Qty values dedup to
  ...
  (input
    (do
      ...
      (def
        ...
        (+
          (Set.len
            #set((Len.Q (Qty.of v (Unit.base #"meter"))) (Len.Q (Qty.of v (Unit.base #"meter")))))
          (*
            10
            (Set.len
              #set((Len.Q (Qty.of v (Unit.base #"meter")))
...
(case
  "a list of nominal-over-quantity newtypes is a compound Map key"
  (doc
    "The compound-key composition: a `(list (Len.Q (Qty …)))` Map key — a nominal-over-Qty nested in a
  ...
  (input
    (do
      ...
      (def
        (main (: v Int64))
        (let
          ((k #list((Len.Q (Qty.of v (Unit.base #"meter"))))) (m (Map.insert (Map.empty) k 42)))
          (+
            (match
              (Map.lookup m #list((Len.Q (Qty.of v (Unit.base #"meter")))))
              ((Some x) x)
              ((None) 0))
            (match
              (Map.lookup m #list((Len.Q (Qty.of (+ v 1) (Unit.base #"meter")))))
              ((Some x) x)
              ((None) 0)))))
      (export main)))
  (call main (: 5 Int64))
...
(case
  "a same-unit quantity join flows and its magnitude reads back"
  (doc
    "`(if (> b 0) (Qty.of 1 kilometer) (Qty.of 5 kilometer))` — BOTH branches are `(Qty Int64
...
(case
  "an explicit conversion repairs a mixed-unit join"
  (doc
    "The REPAIR the scale-clash diagnostic names: convert one branch explicitly so both are
      ...
      (def
        ...
        (Unit.in
          ...
          (if
            ...
            (Qty.of
              (Unit.in (Unit.of #"meter") (Qty.of 1 (Unit.of #"kilometer")))
              (Unit.of #"meter")))))
      (export main)))
  (call main (: 1 Int64))
  (output (: 1000 Int64))
...
(case
  "an annotated argument joins a callee-side quantity at its own scale"
  (doc
    "`(f (: (Qty.of 2 kilometer) (Qty Int64 meter)))` where f adds one kilometer — the
  ...
  (input
    (do
      (def (f (: q (Qty Int64 (Unit.of #"meter")))) (+ q (Qty.of 1 (Unit.of #"kilometer"))))
      (def
        (main (: d Int64))
        (Unit.in
          (Unit.of #"meter")
          (f (: (Qty.of 2 (Unit.of #"kilometer")) (Qty Int64 (Unit.of #"meter"))))))
      (export main)))
  (call main (: 0 Int64))
  (output (: 3000 Int64)))

(case
  "a double same-dimension annotation preserves the original scale"
  (doc
    "`(: (: (Qty.of 1 kilometer) (Qty Int64 meter)) (Qty Int64 centimeter))` — TWO stacked
...
(case
  "a non-unit factor in a Unit.* composition is rejected"
  (doc
    "`(Unit.* (Unit.base #\"meter\") 5)` multiplies a unit by the integer `5` — `5` is not a unit, so
...
(case
  "a non-unit factor in a Unit./ composition is rejected"
  (doc
    "`(Unit./ (Unit.base #\"meter\") 5)` — the quotient sibling: a non-unit divisor `5` is rejected
...
(case
  "a non-integer exponent in a Unit.^ power is rejected"
  (doc
    "`(Unit.^ (Unit.base #\"meter\") 2.5)` raises a unit to `2.5` — a unit power's exponent must be a
...
(case
  "a non-unit base in a Unit.^ power is rejected"
  (doc
    "`(Unit.^ 5 2)` raises the integer `5` to a power — the BASE of a unit power must be a unit, so a
...
(case
  "a Qty-typed variant payload keeps its type non-generic and matches back"
  (doc
    "A `(type Holder (H (Qty Rational (Unit.base #\"meter\"))))` — a variant whose payload is a Qty with
...
(case
  "a product type of three Qty fields with concrete units constructs and projects"
  (doc
    "The units-carrying product a CAD `Vec3` needs: `(type V3q (V3 (Qty Rational m) (Qty Rational m)
...
(case
  "a record of quantities RETURNED as a value renders every field scaled to its reference"
  (doc
    "The value-form RENDER companion of the construct-and-project case above: a GENERIC single-
  ...
  (input
    (do
      ...
      (def
        (main)
        (V3q.V3
          (Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter")))
          (Qty.of 2.0 (Unit.prefix kilo (Unit.base #"meter")))
          (Qty.of 3.0 (Unit.prefix kilo (Unit.base #"meter")))))
      (export main)))
  (output
    (:
      #tuple((Qty.of 5000.0 (Unit.base #"meter"))
        (Qty.of 2000.0 (Unit.base #"meter"))
        (Qty.of 3000.0 (Unit.base #"meter")))
      V3q)))

(case
  "a USER-DEFINED multi-variant sum payload quantity renders scaled to its reference"
  (doc
    "A quantity in a USER-DEFINED multi-variant sum's payload displays scaled to its reference when the
...
(case
  "a Qty payload's INNER type stays a real type parameter — the unit-arg skip does not over-skip"
  (doc
    "The companion of the two cases above, guarding the OTHER edge of the `(Qty T u)` type-parameter
...
(case
  "a Qty payload with a GENERIC inner type rejects a bare use with no type argument"
  (doc
    "The reject twin of the genericity pin above: since `(type Box (B (Qty a (Unit.base #\"meter\"))))`
...
(case
  "angle units radian and degree are first-class and exact within their own dimension"
  (doc
    "ANGLE units — `radian` and `degree`, first-class built-ins for CAD revolve/rotate angles (the
...
(case
  "adding a degree quantity to a radian quantity is a dimension mismatch"
  (doc
    "The honest cross-dimension reject: `degree` and `radian` are DISTINCT base dimensions (their
...
(case
  "a quantity in a recursive sum payload folds through construction and match"
  (doc
    "`(type QList (QNil) (QCons (Qty Int64 meter) QList))` — a recursive sum whose payload is a
  ...
  (input
    (do
      ...
      (def (total (: l QList)) (match l ((QNil) 0) ((QCons q rest) (+ (Qty.value q) (total rest)))))
      (def
        (main (: a Int64))
        (total
          (QCons (Qty.of a (Unit.base #"meter")) (QCons (Qty.of 5 (Unit.base #"meter")) (QNil)))))
      (export main)))
  (call main (: 3 Int64))
  (output (: 8 Int64))
  (live-objects known-leak))
...
(case
  "a Qty-typed export carries its unit frame across the module boundary into caller algebra"
  (doc
    "Units × modules: `double-len : (Qty Int64 meter) -> ...` is exported, and the IMPORTER's
  ...
  (input
    (do
      ...
      (def
        (main (: k Int64))
        (Qty.value (+ (double-len (Qty.of k (Unit.base #"meter"))) (Qty.of 1 (Unit.base #"meter")))))
      (export main)))
  (module "geo"
    (do (def (double-len (: d (Qty Int64 (Unit.base #"meter")))) (+ d d)) (export double-len)))
  (call main (: 5 Int64))
  (output (: 11 Int64))
  (call main (: 0 Int64))
  (output (: 1 Int64)))
...
(case
  "a seconds quantity is rejected by a meter-typed import at the call site"
  (doc
    "The dimension-reject twin of the Qty-export pin: the importer hands
  ...
  (input
    (do
      (import "geo" (double-len))
      (def (main (: k Int64)) (Qty.value (double-len (Qty.of k (Unit.base #"second")))))
      (export main)))
  (module "geo"
    (do (def (double-len (: d (Qty Int64 (Unit.base #"meter")))) (+ d d)) (export double-len)))
  (error CDZ0501))
...
(case
  "a map of meter quantities rejects a second-dimension value insert"
  (doc
    "The dimension-safety face of the Qty collection cycle: `(Map Int64 (Qty Int64 meter))`
...
(case
  "a quantity captured in a closure env adds against per-call quantities"
  (doc
    "Units × captures: the factory's `(Qty Int64 meter)` param rides the returned closure's
  ...
  (input
    (do
      (def
        (mk (: base (Qty Int64 (Unit.base #"meter"))))
        (fn ((: n Int64)) (Qty.value (+ base (Qty.of n (Unit.base #"meter"))))))
      (def
        (main (: k Int64))
        (do (def f (mk (Qty.of 100 (Unit.base #"meter")))) (+ (* 10 (f k)) (f 1))))
      (export main)))
  (call main (: 5 Int64))
  (output (: 1151 Int64))
  (call main (: 0 Int64))
...
(case
  "unit arithmetic on a map-extracted quantity re-enters the map typed"
  (doc
    "The extract-compute-reinsert cycle for UNIT-typed values (the 18-units collection pins
    ...
    (do
      (def
        ...
        (do
          ...
          (+ (* 10 (Qty.value (Option.expect (Map.lookup m2 2) "p"))) (Map.len m2))))
      (export main)))
  (call main (: 5 Int64))
  (output (: 352 Int64))
  (call main (: 0 Int64))
...
(case
  "a nested unit power multiplies exponents — (m^2)^2 is the same dimension as m^4"
  (doc
    "The free-abelian-group laws beyond product/quotient cancellation: NESTED power must MULTIPLY exponents ((Unit.^ (Unit.^ m 2) 2) = m^4 — adding or concatenating gives m^2/m^6 and the same-dimension divide rejects); verified by a dimensionless divide (5).")
  (input
    (do
...
(case
  "a unit raised to the ZEROTH power is the dimensionless identity Unit.one"
  (doc
    "u^0 must BE Unit.one — the zero-exponent entry drops from the canonical map (a map keeping meter->0 fails the same-dimension + with a Unit.one quantity).")
  (input
    (do
...
(case
  "a negative-exponent unit cancels its base through a multiply to dimensionless"
  (doc
    "A negative exponent as a def-bound first-class unit VALUE (hz = s^-1) cancelling through a runtime multiply to dimensionless — the existing neg-exp case is inline and Float-inner; this is def-bound with an Int64 inner.")
  (input
    (do
      (def hz (Unit.^ (Unit.base #"second") -1))
      (def (main (: a Int64)) (Qty.value (* (Qty.of a hz) (Qty.of 3 (Unit.base #"second")))))
      (export main)))
  (call main (: 7 Int64))
  (output (: 21 Int64)))
...
(case
  "a helper RETURNS a derived-dimension (m squared) quantity and same-dimension results add"
  (doc
    "The derived-dimension flow face (the value-form pins render m2/m3 in OUTPUT annotations only): a helper's INFERRED return type carries (Unit.^ meter 2) and two results add as same-dimension m2 — the composed-dimension frame flows through a call boundary. (Spelled with a def-bound unit + inferred return: a derived unit in an INPUT-position type annotation trips an ML-printer re-parse gap, reported to v-syntax.)")
  (input
    (do
      (def m2 (Unit.^ (Unit.base #"meter") 2))
      (def (area (: v Int64)) (Qty.of (* v v) m2))
      (def (main (: k Int64)) (Qty.value (+ (area k) (area 3))))
      (export main)))
  (call main (: 4 Int64))
  (output (: 25 Int64)))
...
(case
  "a product-REACHED quantity equals the directly-composed unit in either written order"
  (doc
    "Construction-path equality on the unit lattice: `(* 2m 3s)` REACHES the unit m·s through Qty
      ...
      (def
        ...
        (+
          (*
            ...
            (if
              (=
                (* (Qty.of n (Unit.base #"m")) (Qty.of 3 (Unit.base #"s")))
                (Qty.of (* n 3) (Unit.* (Unit.base #"m") (Unit.base #"s"))))
              1
              0))
          (if
            (=
              (* (Qty.of n (Unit.base #"m")) (Qty.of 3 (Unit.base #"s")))
              (Qty.of (* n 3) (Unit.* (Unit.base #"s") (Unit.base #"m"))))
            1
            0)))
      (export main)))
  (call main (: 2 Int64))
...
(case
  "a division-REACHED quantity equals the inverse-composed unit and cancels to Unit.one"
  (doc
    "The division face: `(/ 6m 3s)` reaches m/s = m·s⁻¹; the direct composition spells it
      ...
      (def
        ...
        (+
          (*
            ...
            (if
              (=
                (/ (Qty.of (* n 3) (Unit.base #"m")) (Qty.of 3 (Unit.base #"s")))
                (Qty.of n (Unit.* (Unit.base #"m") (Unit.^ (Unit.base #"s") -1))))
              1
              0))
          (if
            (= (/ (Qty.of n (Unit.base #"m")) (Qty.of 1 (Unit.base #"m"))) (Qty.of n Unit.one))
            1
            0)))
      (export main)))
  (call main (: 2 Int64))
...
(case
  "a mixed-scale combine over a computed BigInt quantity operand converts across scales"
  (doc
    "`(Qty.value (+ (* (Qty.of (BigInt.of n) km) 2) (Qty.of 500 m)))`: a COMPUTED (2x-scaled) km
...
(case
  "qkm1 a same-unit Float quantity Map key round-trips with value discrimination"
  (input
    (do
      (def
        (main (: n Int64))
        (let
          ...
          (+
            (*
              100
              (match (Map.lookup m (Qty.of 5000.0 (Unit.base #"meter"))) ((Some v) v) ((None u) -1)))
...
(case
  "qkm2 a CROSS-scale Map probe is a compile-time type error (the mixed-scale = conversion does not extend to collection ops)"
  (input
    (do
      (def
        (main (: n Int64))
        (match
          (Map.lookup
            (Map.insert (Map.empty) (Qty.of 5000.0 (Unit.base #"meter")) 42)
            (Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter"))))
          ((Some v) v)
          ((None u) -1)))
      (export main)))
...
(case
  "qkm3 an explicitly converted cross-scale probe (Unit.in + re-wrap) hits the same-unit key"
  (input
    (do
      (def
        (main (: n Int64))
        (let
          ...
          (match
            (Map.lookup
              m
              (Qty.of
                (Unit.in (Unit.base #"meter") (Qty.of 5.0 (Unit.prefix kilo (Unit.base #"meter"))))
                (Unit.base #"meter")))
            ((Some v) v)
            ((None u) -1))))
      (export main)))
...
(case
  "an if-join over different-unit same-dimension quantities names the scale, not a shadowed declaration"
  (input
    (do
      (def
        (main (: b Bool))
...
(case
  "a list-element join over different-unit same-dimension quantities names the scale"
  (input
    (do
      (def
        (main)
...
(case
  "a cross-dimension if-join clash stays a plain mismatch (no same-dimension scale tail)"
  (input
    (do
      (def
        (main (: b Bool))
        (Qty.value (if b (Qty.of 1 (Unit.base #"meter")) (Qty.of 2 (Unit.base #"second")))))
      (export main)))
  (error CDZ0203 (message "meter") (not "SAME dimension at DIFFERENT units")))

(case
  "a cross-dimension list-join clash stays a plain mismatch (no same-dimension scale tail)"
  (input
    (do
      (def
        (main)
