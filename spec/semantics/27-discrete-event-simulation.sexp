...
(case "a stored continuation is popped from a time-ordered pqueue (multi-payload match) and resumed"
  (doc    "The multi-task step over the single-escaping-continuation case: the scheduler's pending
           ...
           — a MULTI-payload constructor. Popping the earliest event match-binds all three fields, then
