              ...
              (def (main (: seed Int64))
                (let ((a (next seed)))
                  (let ((b (next a)))
                    (let ((c (next b)))
                      (let ((d (next c)))
                        (= (Set.union #set(a b) #set(c d))
                           (Set.union #set(c d) #set(a b))))))))
              (export main)))
  (call   main (: 777 Int64)) (output (: true Bool)))
              ...
              (let ((a (lvl (next seed))))
                (let ((b (lvl (next (next seed)))))
                  (let ((c (lvl (next (next (next seed))))))
                    (if (= (Set.to-list #set(a b c))
                           (Set.to-list #set(c a b)))
...
(case "a Value.encode/Value.decode round-trip preserves a SET's elements"
  ...
  (input  (do
            (def (main (: n Int64))
              (match (: (Value.decode (Value.encode #set(n (+ n 5) (+ n 10)))) (Option (Set Int64)))
