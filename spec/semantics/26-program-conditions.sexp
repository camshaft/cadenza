...
(case "a no-overflow obligation is DISCHARGED: for x <= 100, (x + 1) <= MAXINT via monotonicity + a CHECKED numeral fact"
  ...
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq add le maxint concl assume le-ax mono-add-r trans-le))
            (def (main)
...
(case "the b2 match predicate LICENSES the elision: the discharged no-overflow proof matches the obligation and its hyps are covered by the node precondition"
  ...
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq add le maxint concl hyps assume le-ax mono-add-r trans-le licenses))
            (def (main)
              (let ((x   (Term.Var 0))
                    (one (Term.Num 1))
                    (c   (Term.Num 100)))
...
(case "b4c(proven): @requires(<= x 100)/@ensures(<= ret MAXINT) on (f x)=x+1 discharges — P denoted as hyp, Q[ret:=body] as goal"
  ...
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq add le maxint concl hyps assume le-ax mono-add-r trans-le))
            (def (main)
              (let ((x    (Term.Var 0))
                    (one  (Term.Num 1))
                    (c100 (Term.Num 100)))
...
(case "b4c(conjunctive): two stacked @requires give a 2-hyp proof; licenses requires BOTH hyps covered by the precondition"
  ...
  (module "bounds"
    (do
      ...
      (export (. HeadOp *))
      (export Thm)
      (export op-eq term-eq add le ge maxint concl hyps assume assume-both le-ax mono-add-r trans-le licenses)))
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq add le ge maxint concl hyps assume assume-both le-ax mono-add-r trans-le licenses))
            (def (main)
              (let ((x    (Term.Var 0))
                    (one  (Term.Num 1))
                    (c100 (Term.Num 100))
...
(case "b4c(conjunctive) NEG-1: precondition covers only {le x 100} (missing ge x 0) — the 2-hyp proof is NOT licensed"
  ...
  (module "bounds"
    (do
      ...
      (export (. HeadOp *))
      (export Thm)
      (export op-eq term-eq add le ge maxint concl hyps assume-both le-ax mono-add-r trans-le licenses)))
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq add le ge maxint concl hyps assume-both le-ax mono-add-r trans-le licenses))
            (def (main)
...
(case "b(sub): a no-underflow obligation is DISCHARGED — for x >= 0, (x - 1) >= MININT via monotonicity + a CHECKED numeral fact"
  ...
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq sub ge minint concl hyps assume ge-ax mono-sub-r trans-ge))
            (def (main)
              (let ((x    (Term.Var 0))
                    (one  (Term.Num 1))
                    (zero (Term.Num 0)))
...
(case "b(mul): a no-overflow obligation is DISCHARGED for x <= 100, (x * 2) <= MAXINT via positive-multiplier monotonicity"
  ...
  (input  (do
            (import "bounds" (HeadOp Term Thm term-eq mul le maxint concl hyps assume le-ax mono-mul-r trans-le))
            (def (main)
              (let ((x    (Term.Var 0))
                    (two  (Term.Num 2))
                    (c100 (Term.Num 100)))
