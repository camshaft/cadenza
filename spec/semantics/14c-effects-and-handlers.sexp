...
(case "tpwJ a cross-scope shared-let in a resumptive arm — a let-bound NESTED-IF (outer cond reads a state field, inner reads the param) feeding 3+ consumers across 3+ dispatches with a state-reading second op, the thread-path no-local-slot face closed by 8276ad1a6"
  (input  (do
...
(case "tmb4 a FOREIGN LEVY FEEDS THE DISCARDED REPLAY'S ANSWER — the arm resumes with an outer levy as the answer then discards the replay and answers a tombstone, the levy still fires and advances the outer thread even though its value flowed only into abandoned work, and the outer body's later levy reads five higher proving the discarded dataflow's effect landed"
  ...
  (def (main (: n Int64))
    (handle E (% n 3)
      ...
      (+ (E.tick) (* 10 (E.tick)))))
  (export main)))
  (call   main (: 10 Int64)) (output (: 483 Int64))
  (call   main (: 0 Int64)) (output (: 472 Int64)))
...
(case "iso-b a let-bound escaped-closure helper-call answer inside the handle body folds — the recovery forces the re-parented load-time ref onto the structural walk so it resolves against the rebuilt let (was a false CDZ0101)"
  (input (do
    (effect E (op tick (-> Int64)))
