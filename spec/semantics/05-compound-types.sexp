...
; no dup) would corrupt. `go` inserts into `m` on the recursive path AND reads `Map.len m` (the pre-insert
...
(case "a map inserted-into on one path and size-read on another keeps copy-on-write across recursion"
  (doc    "`go(m,n) = if n=0 then Map.len m else go(Map.insert m n n, n-1) + Map.len m` threads a runtime
           map `m` that is CONSUMED by `Map.insert` on the recursive path AND BORROWED by `Map.len m` on the
           return path. Immutability + Perceus copy-on-write means the `Map.len m` at each level sees the
  ...
  (input  (do
            (def (go (: m (Map Int64 Int64)) (: n Int64))
              (if (= n 0) (Map.len m)
                  (+ (go (Map.insert m n n) (- n 1)) (Map.len m))))
