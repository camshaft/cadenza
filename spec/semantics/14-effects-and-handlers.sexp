...
(case "a two-site arm with a second op that REPLACES the state mid-chain declines cleanly"
  (doc    "The decline BOUNDARY of the served two-site family (breaker sy1). ts1 above FOLDS because its
           ...
           state with a DIFFERENT value mid-chain — `(flip (u) s (resume 0 (Symbol.of \"quiet\")))` — a
           mode-switch interleaved BETWEEN the two `emit` performs. breaker's position-sharpened matrix:
           a second op INTERLEAVED between a two-site arm's performs is the not-yet-reducible conjunct
           (trailing second ops are served); the two-hole refold cannot yet thread a DIFFERENT op through
           the middle of a multi-site continuation chain. This is an HONEST decline (a fold-capability gap,
           never a wrong value), NOT the ts1/ag5 false-CDZ0101 orphan (those were bugs, fixed). The
           eventual fold: emit(5) sees `loud` → 5·100 = 500, flip sets state `quiet` and resumes 0, emit(3)
           sees `quiet` → 3 → 500 + 0 + 3 = 503. Pins the boundary as a clean todo, not a leak.")
  (input  (do
