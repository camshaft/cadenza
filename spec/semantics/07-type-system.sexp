...
(case "a nullary sum type declared inside a do block resolves and matches"
  (doc    "`(do (type C R G B) (match R (R 1) (G 2) (B 3)))` declares the enum `C` LOCALLY inside `main`'s
           `do`; `R`/`G`/`B` resolve although the type is not top-level. Constant scrutinee `R` folds to 1.")
  (input  (do
            (def (main) (do (type C R G B) (match R (R 1) (G 2) (B 3))))
            (export main)))
  (call   main) (output (: 1 Int64)))

(case "a payload sum type declared inside a do block resolves and matches its payload"
  (doc    "`(do (type Box (Bx Int64)) (match (Bx 5) ((Bx x) x)))` declares the payload-variant type locally;
