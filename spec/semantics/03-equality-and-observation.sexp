...
(case
  ...
  ; RE-TIGHTENED (v-memory-safety + v-core-opt joint #9537-re-land): the RATIONAL-key sibling of 06-numeric:13328 — same self-recursive-fold threaded-prev shape. The owned-fold surplus-skip is BACK with the COARSE liveness-across-vec-split predicate (surplus.rs): inc's head element (#tuple(k _v)) is dead-after with its threaded child k consumed via (inc t k …) -> consuming_sites>=1 -> ADMIT -> spine head reclaimed. DBG census 0, value 40 (in-order, v-core-opt verified NOT a miscompile). The choreography over-drop UAF (#9551's revert reason) is DECLINED by the consuming-site conjunct (pure-borrow head -> consuming_sites==0). Was known-leak (post-#9551); re-tightened alongside 06 + v-core-opt's 05/22.
  (live-objects 0))
