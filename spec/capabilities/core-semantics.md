...
An implementation MAY decline to evaluate a computation whose value cannot affect the program's observable behavior — one whose result reaches neither the program's terminal value nor any host call — and so MAY elide a trap that computation would raise. A tuple or record element that is never projected, a `let` binding that is never referenced, and an argument bound to a parameter the function body never uses are unobserved in this sense: constructing the surrounding value does not require evaluating them, so an implementation that omits them, and the traps they would have raised, is conformant. A heap-materialized collection — a list, a set, or a map — is NOT deferrable in this sense: whenever the collection's object is materialized — because its length or size is taken, its membership is queried, or it flows to the program's result or a host call — constructing it MUST evaluate its elements or entries to build the collection, so a trapping element or entry is observed at that construction and its trap MUST occur even when only the collection's length or size, and not the element's value, is subsequently taken; a list, set, or map is thus strict in its elements at the point its object is materialized, and the deferral above is otherwise specific to an unprojected tuple element or record field, whose surrounding value a projection can construct without it. A structural equality whose operands are collections is nonetheless decided element-by-element and short-circuits at the first differing element exactly as a tuple comparison does, so a list operand of `=` is observed only up to the deciding element: an element past a difference that has already decided the result is unobserved — the list object is never materialized for such a comparison — so its trap does not occur, while an equal prefix is forced through to the first not-yet-decided element, whose trap does occur. This is the same laziness the language already grants a conditional's unselected branch, a boolean connective's shielded operand, and a match arm that does not match — generalized to any subexpression whose value no observation depends on. Whether a trap occurs is therefore a property of whether its computation's value is observed, not of the syntactic form the computation appears in.
