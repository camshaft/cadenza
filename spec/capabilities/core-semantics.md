...
The canonical stored form of a compound-construction node is fixed by the AST-encoding contract (`../contracts/ast-encoding.md` §"The Encoding Is General And Stable"), which represents every node as a prelude symbol applied to an ordered sequence of children, so a compound-construction node's head in the stored binary AST MUST be the prelude symbol denoting the tuple or record constructor rather than a string-literal leaf, because a string literal is a leaf value node and not a symbol. The string-headed surface spelling `("tuple" …)` / `("record" …)` and the alias spelling `(tuple …)` / `(record …)` MUST both, where they denote construction, encode to that one constructor symbol, so a decode of the stored tree observes a name-leaf head — `tuple` or `record` — and never a string literal in head position; a program that shadows the alias name instead resolves that occurrence to the shadowing binding under *Binding Is Lexical*, an ordinary reference node and not the constructor symbol, whereas the string-headed spelling — being beyond shadowing — always resolves to the constructor symbol. In a compound-construction node so encoded, a tuple's children MUST be its element expressions in positional order and a record's children MUST be its `(= <key> <value>)` field pairs, so that a reader of the stored tree recovers the compound's shape from the constructor symbol and its children rather than from any surface spelling.
