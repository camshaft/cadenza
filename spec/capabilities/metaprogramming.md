...
Reflecting a type to its definition AST MUST be total over concrete types — a nominal or sum type reflects its declaration, and a structural type (a record, tuple, list, map, set, primitive, or function) reflects its canonical type-surface form (a function type's surface is its arrow form) — and MUST reject a type that is not concrete (one carrying an unresolved type variable) with a machine-readable diagnostic, since a type variable has no definition to reflect, rather than fabricate a definition or silently decline.
