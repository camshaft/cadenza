//! `cargo xtask install-lsp` — build the `cdz` LSP server and wire the VS Code extension into the
...
pub(crate) fn run(paths: &Paths, uninstall: bool) {
    let ext_src = paths.repo.join("integrations/vscode");
    if !ext_src.is_dir() {
...
fn build_cdz(paths: &Paths) -> PathBuf {
    let sh = Shell::new().expect("open a shell for the cdz build");
    let _pushed = sh.push_dir(&paths.seed);
...
/// dependency for the one place xtask needs it.
