...
fn main() {
    ...
    match cli.command {
        ...
        Cmd::Gate {
            ...
            // The corpus BEHAVIOR gate (grade each case vs its recorded outcome, --check/--save/--case)
            // moved OUT of xtask into the nix per-file COARSE gates (v-xtask-decompose seq-202) — use
            // `nix run .#gate` (the folded coarse gates) or the `corpus-gate-coarse` /
            // `corpus-rust-gate-coarse` checks + `nix run .#save-baseline` to regenerate. `xtask gate`
            // now serves ONLY the opt-level-equivalence sweep (`--opt-sweep`), which has no coarse twin.
            if !opt_sweep {
                eprintln!(
                    "xtask gate: the corpus behavior gate moved to nix (seq-202) — use `nix run .#gate` \
                     (or the corpus-gate-coarse / corpus-rust-gate-coarse checks; `nix run .#save-baseline` \
                     to regenerate baselines). `xtask gate` now only runs `--opt-sweep`."
                );
                std::process::exit(2);
            }
...
// dead_code: `Value`'s warnings field + `Declined`'s `message` are still CAPTURED by the preserved
// `run_program_wasm`/`run_program_rust` runners (opt-sweep drives them), but were only READ by the
// in-process corpus grader deleted in the seq-202 gate-machinery delete. Kept intact to avoid churning
// the preserved run-case path; vestigial-but-harmless — safe to deep-clean in a follow-up.
#[allow(dead_code)]
enum Ran {
...
// dead_code: `Default`/`Expect` (and the `from_record` constructor) are now only reachable via the
// deleted in-process grader's heap-balance path; the preserved opt-sweep passes only `Off`. Kept so the
// preserved `run_program_*` runners (which take a `LiveObjectsCheck` + call `expected_for_heap`) stay
// byte-intact — vestigial, safe to prune with the Ran cleanup above.
#[allow(dead_code)]
#[derive(Clone, Copy)]
...
fn decode_value_doc(hex: &str) -> Result<String, String> {
    ...
    // cdz-run's one-line render (breaker finding: a long record result false-mismatched the fuzz oracle).
...
fn check(paths: &Paths, profile: &str) {
    ...
    // The behavior gate — the corpus grade moved OUT of this xtask binary into nix (v-xtask-decompose
    // seq-202): the per-file COARSE gates (`corpus-gate-coarse` wasm + `corpus-rust-gate-coarse` rust,
    // CA-cached, fail-on-regression vs the committed baselines) folded into localGate, so `nix run .#gate`
    // is now the authoritative behavior gate — a green run means the library is healthy AND the compiler
    // didn't backslide, same guarantee the old in-process `gate --check` gave. The full corpus battery
    // runs nightly; the per-MR gate is the operator-bounded LIGHT subset (merge-velocity ruling 2026-09-03).
    log.step_show("gate", "nix run .#gate", repo);
...
pub(crate) fn baseline_titles_agree_lint(paths: &Paths) -> Result<(), String> {
    let targets = [
        ".gate-baseline",
        ".gate-baseline-rust",
        ".gate-baseline-rust-async",
    ];
    let mut sets: Vec<(&str, std::collections::BTreeSet<String>)> = Vec::new();
    for name in targets {
        let path = paths.repo.join("spec/semantics").join(name);
