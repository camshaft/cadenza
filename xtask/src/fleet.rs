...
fn find_stale_queued_mrs(fleet: &Fleet, now: u64) -> Vec<(String, String, String, u64)> {
    ...
    // it is NOT stuck. `in_flight_unknown` = some in-flight ref's files couldn't be resolved; then we
    // can't prove ANY queued MR file-disjoint, so force every one blocked (never stale-flag under
    // unknown in-flight territory — PR #1330, the in-flight side of the #1244 None-handling fix).
    let (in_flight_files, in_flight_unknown) = in_flight_file_set(&dispatches);
    let mut out = Vec::new();
    let Ok(rd) = std::fs::read_dir(fleet.inbox("pr-sync")) else {
        return out;
    };
    for entry in rd.filter_map(Result::ok) {
        ...
        // ref's files were UNKNOWN (can't prove disjointness → conservatively blocked), OR (b) THIS MR's
        // files can't be computed (None), OR (c) they collide. An unknown file set on EITHER side must
        // not read as "file-disjoint" and surface a dispatchable-but-stuck MR (PR #1244/#1330).
        let file_blocked = in_flight_unknown
            || match changed_files_of(&mr.r#ref) {
...
fn watchdog(fleet: &Fleet, opts: WatchdogOpts) {
    ...
    for a in &reg.agents {
        ...
        if hb_age.is_some() && age <= stale_after {
            // Fresh this sweep. Clear the consecutive-nudge streak ONLY if the freshness is genuinely
            // SELF-PRODUCED (its own cron fired), not a leftover heartbeat from a recent nudge — see
...
/// NOT merely a leftover heartbeat from a recent watchdog nudge.
///
/// THE BUG THIS FIXES (concierge 2026-08-05, post-#2250): a dead-cron agent that the watchdog nudges
/// runs ONE inline tick + stamps a heartbeat, then looks "fresh" for the whole ~stale-window until it
/// goes idle again. If we clear the streak on ANY fresh sweep, that nudge-produced freshness resets the
/// streak to 0 between every nudge → it oscillates 0→1→0 and NEVER reaches the escalation threshold, so
...
fn merge_request_foreign_commits(fleet: &Fleet, from: &str, r#ref: &str) -> Option<Vec<String>> {
    ...
    // The commits the MR would INTRODUCE relative to trunk. Anchor on `merge-base(trunk, ref)..ref` — the
    // ref's OWN commits since it forked from trunk — NOT the bare `trunk..ref`. When the ref is built on an
    // OLD trunk tip that is tree-equal but COMMIT-DISTINCT from current trunk (the re-parent model), a bare
    // `trunk..ref` includes trunk-SIDE commits the ref never authored, over-reporting them as the sender's
    // foreign commits and risking a false send-refusal on a clean MR (the same stale-base wrong-range class
    // publish_candidate hit — both use the shared `merge_base_range` helper). The merge-base is the exact
    // fork point, so the range is precisely the sender's introduced commits regardless of trunk divergence.
    // Fall back to
    // `trunk..ref` only if the merge-base can't be resolved. If trunk or ref is unresolvable, bail (None)
    // rather than refuse — an unresolvable ref is pr-sync's problem to report, not ours to block.
    let merge_base = Command::new("git")
        .current_dir(&fleet.repo)
        .args(["merge-base", TRUNK, r#ref])
        .output()
        .ok()
        .filter(|o| o.status.success())
...
/// The `<base>..<ref>` rev-range naming exactly the commits `ref` INTRODUCES since it forked from its
/// integration target: `<merge-base>..ref` when the merge-base resolved (immune to a stale/commit-distinct
/// base — the re-parent model), else the legacy `trunk..ref` fallback (used only when the merge-base
/// couldn't be computed; the caller's rev-list then reports a bad ref). Shared by `publish_candidate`
/// (merge-base against origin/main, the publish target) and the send-time contamination guard
/// (`merge_request_foreign_commits`, merge-base against trunk) — both need the ref's own delta, NOT a bare
/// `trunk..ref` that over-includes target-side commits on a diverged base. Pure so it is unit-tested.
fn merge_base_range(merge_base: Option<&str>, r#ref: &str) -> String {
    match merge_base {
...
/// The changed file paths of a commit-ish (`git show --name-only`), trimmed + non-empty: `Some(files)`
/// on a resolved ref (possibly an EMPTY vec — a genuine no-file commit), `None` on an unresolvable ref
/// / git error. Callers MUST NOT read `None` as "empty/clean" — an unknown file set is handled
/// conservatively (treat as blocked / force collision), never as file-disjoint (PR #1244/#1330). The
/// shared source for lane classification AND file-level collision checks (same file set for both).
...
/// The union of files changed by every in-flight candidate, PLUS whether ANY of their refs was
/// UNRESOLVABLE (`changed_files_of` → None). Returns `(files, any_unresolved)`. The `any_unresolved`
/// flag is the fix for the in-flight side of the None-vs-empty trap (PR #1330): if an in-flight ref's
/// files can't be computed, its real files are UNKNOWN — dropping it (the old `filter_map`) would make
/// a queued MR that ACTUALLY collides with it read as file-disjoint (falsely dispatchable/stale). The
/// caller must instead treat `any_unresolved` conservatively: force every queued MR blocked this pass.
fn in_flight_file_set(dispatches: &[CiDispatch]) -> (std::collections::HashSet<String>, bool) {
    let mut files = std::collections::HashSet::new();
    let mut any_unresolved = false;
    for d in dispatches {
        match changed_files_of(&d.r#ref) {
            Some(fs) => files.extend(fs),
            None => any_unresolved = true, // unknown in-flight territory → caller forces collision
        }
    }
    (files, any_unresolved)
}
...
fn schedule_plan(fleet: &Fleet, cap: usize) {
    ...
    // NOT lane-label (a baseline-toucher must block a 2nd baseline-toucher of ANY lane label; `code`
    // candidates on DISJOINT files run parallel). `in_flight_unknown` = some in-flight ref's files
    // couldn't be resolved → we can't prove ANY candidate disjoint, so dispatch NOTHING new this pass
    // rather than risk dispatching into a collision we couldn't see (PR #1330, in-flight side of the
    // None-handling fix). Handled below by skipping the dispatch entirely.
    let (in_flight_files, in_flight_unknown) = in_flight_file_set(&dispatched);
    ...
    let queued: Vec<SchedCandidate> = queued_merge_requests(fleet)
        ...
        .collect();
    // Under unknown in-flight territory, dispatch nothing (pass an empty candidate list) — safest.
    let picks = if in_flight_unknown {
        Vec::new()
    } else {
        schedule_dispatch(&queued, &in_flight_files, cap, dispatched.len())
    };
    println!(
...
fn batch_commit_inner(repo: &Path, execute: bool) -> Result<String, String> {
    ...
    // `git merge-base --is-ancestor` distinguishes three outcomes by exit code: 0 = ancestor (FF ok),
    // 1 = genuinely NOT an ancestor (a real non-FF that would discard commits), and 128 (or a spawn
    // failure → None) = a FATAL git error, e.g. an invalid/missing/corrupt SHA or a broken repo. All
    // three refuse to move trunk (safe either way), but collapsing 1 and 128 into one message would
    // print "NOT a descendant / staging ref stale-or-corrupt" for a fatal error too — hiding the real
    // git fault exactly when a ref IS corrupt (the diagnostic that matters most). So branch on the code
    // and surface stderr on the fatal path.
    let ancestry = git_out(&["merge-base", "--is-ancestor", &staged_base, &staged_tip]);
    match ancestry.status.code() {
        Some(0) => {} // ancestor — FF is safe, fall through
        Some(1) => {
            return Err(format!(
                "batch-commit: REFUSED — the staged tip ({staged_tip}) is NOT a descendant of the \
                 staged base ({staged_base}); moving trunk to it would be a non-fast-forward that \
                 discards commits. The staging ref is stale — re-stage off the current trunk. \
                 (trunk NOT moved.)"
            ));
        }
        _ => {
            ...
            return Err(format!(
                "batch-commit: REFUSED — `git merge-base --is-ancestor {staged_base} {staged_tip}` \
                 failed ({}): {stderr}. The staging ref may be corrupt or point at a missing object \
                 — could not verify fast-forward-ness. (trunk NOT moved.)",
                ancestry.status.code().map_or_else(
...
mod tests {
    ...
    #[test]
    fn batch_commit_surfaces_the_git_error_when_the_ancestry_check_fails_fatally() {
        // PR#768 review: `--is-ancestor` exits 0 (ancestor), 1 (genuinely not-ancestor), or 128 (FATAL:
        // invalid/corrupt SHA, not-a-commit, broken repo). All three refuse (safe), but the diagnostic
        // must NOT print "NOT a descendant / staging ref stale" for the fatal case — that hides the real
        // git fault. Here we make the check fail FATALLY by pointing the staging ref at a TREE object
        // (not a commit): `rev-parse --verify -q` still returns it (non-empty, drift guard passes since
        // trunk stays at the base), but `merge-base --is-ancestor <commit> <tree>` is a fatal error.
        let dir = init_batch_fixture_repo("fatal");
        let git = |args: &[&str]| {
            Command::new("git")
                .current_dir(&dir)
                .args(args)
        ...
        let staged_base = git_rev(&dir, TRUNK);
        // A tree object of the current trunk — a valid object that is NOT a commit.
        let tree = git_rev(&dir, "HEAD^{tree}");
        assert!(!tree.is_empty());
        assert_ne!(tree, staged_base);

        let _ = git(&["update-ref", BATCH_STAGING_REF, &tree]);
        let _ = git(&["update-ref", BATCH_STAGING_BASE_REF, &staged_base]);
        assert_eq!(
            git_rev(&dir, TRUNK),
            staged_base,
            "trunk still at base → drift guard passes, so ancestry is the sole (fatally-failing) check"
        );

        for execute in [false, true] {
            let res = batch_commit_inner(&dir, execute);
            assert!(
                res.is_err(),
                "a fatal ancestry check must refuse (execute={execute}): {res:?}"
            );
            let err = res.unwrap_err();
            // The distinct fatal-path message — surfaces the git invocation, NOT the non-FF wording.
            assert!(
                err.contains("git merge-base --is-ancestor") && err.contains("failed"),
                "fatal path must surface the failed git command, got: {err}"
            );
            assert!(
                !err.contains("NOT a descendant"),
                "fatal path must NOT reuse the exit-1 non-FF wording, got: {err}"
            );
        }
        // trunk unmoved, staging refs survive — same safety as the non-FF path.
        assert_eq!(git_rev(&dir, TRUNK), staged_base, "trunk must be UNMOVED");
        assert!(!git_rev(&dir, BATCH_STAGING_REF).is_empty());
    ...
    #[test]
    fn merge_base_range_anchors_on_merge_base_else_falls_back_to_trunk() {
        ...
        assert_eq!(
            merge_base_range(Some("abc123"), "deadbeef"),
            "abc123..deadbeef"
        );
        // Merge-base unresolved (None) → legacy trunk..ref fallback (rev-list faces report a bad ref).
        assert_eq!(merge_base_range(None, "deadbeef"), "trunk..deadbeef");
...
+ 3333333333333333333333333333333333333333";
    ...
    fn in_flight_file_set_flags_an_unresolvable_ref() {
        ...
        // Empty in-flight set → nothing unresolved (vacuously), empty files.
        let (files, any) = in_flight_file_set(&[]);
        assert!(files.is_empty() && !any);
    }
