...
fn render_chapter(a: &Arenas, chapter: StructId, text: &str, spans: &SpanTable) -> String {
    let mut title = "";
    let mut slug = "";
    ...
    for b in &blocks {
        match *b {
            ...
            Block::Runnable(n) => out.push_str(&render_runnable(a, n, text, spans)),
            Block::Exercise(n) => out.push_str(&render_exercise(a, n, text, spans)),
...
fn render_runnable(a: &Arenas, node: StructId, text: &str, spans: &SpanTable) -> String {
        ...
        .find(|&&f| a.head_name(f) == Some("files"))
    {
        ...
        for &file in children(a, *files) {
            ...
            out.push_str(&format!(
                "            source: `{}`,\n",
                tmpl(&code_payload(a, text, spans, file, "source").unwrap_or_default())
            ));
            out.push_str(&format!(
                "            surface: {},\n",
        ...
        let src = tmpl(&code_payload(a, text, spans, node, "source").unwrap_or_default());
...
fn render_exercise(a: &Arenas, node: StructId, text: &str, spans: &SpanTable) -> String {
    ...
    if let Some(s) = code_payload(a, text, spans, node, "starter") {
        out.push_str(&format!("        starter={{`{}`}}\n", tmpl(&s)));
    }
    if let Some(s) = code_payload(a, text, spans, node, "solution") {
...
mod tests {
    ...
    fn string_source_payload_is_the_string() {
        ...
        assert_eq!(
            code_payload(&a, text, &spans, r, "source").as_deref(),
            Some("(def (main) 5)")
        );
    }

    #[test]
    fn embed_source_span_slices_to_exact_display_text() {
        // Embedded forms, continuation lines at column 0 (as the converter writes them) → the slice
        // reproduces the exact multi-line displayed code, INCLUDING the author's 2-space inner indent.
        let text = "(chapter (slug \"x\")\n  (runnable (source (def (main) (f 5))\n(@ (requires (>= x 0))\n  (def (f (: x Int64)) (+ x 1))))))\n";
        let (a, spans) = parse(text);
        let r = first(&a, locate_chapter(&a).unwrap(), "runnable");
        assert_eq!(
            code_payload(&a, text, &spans, r, "source").unwrap(),
            "(def (main) (f 5))\n(@ (requires (>= x 0))\n  (def (f (: x Int64)) (+ x 1)))"
        );
    }

    #[test]
    fn embed_source_preserves_a_line_comment() {
        // A `;` line comment lives in the sliced bytes (span-slice reads source, never re-prints) — so it
        // survives even though the reader drops `;` trivia from the AST.
        let text = "(chapter (slug \"x\") (runnable (source (def (main) 5) ; the entry point\n)))";
        let (a, spans) = parse(text);
        let r = first(&a, locate_chapter(&a).unwrap(), "runnable");
        // slice spans first-form..last-form; the trailing `; comment` sits AFTER the last form, so it is not
        // in [first.start, last.end) — the invariant we care about is that a comment BETWEEN forms is kept:
        let text2 = "(chapter (slug \"x\") (runnable (source (def a 1) ; between\n(def b 2))))";
        let (a2, spans2) = parse(text2);
        let r2 = first(&a2, locate_chapter(&a2).unwrap(), "runnable");
        assert_eq!(
            code_payload(&a2, text2, &spans2, r2, "source").unwrap(),
            "(def a 1) ; between\n(def b 2)"
        );
    ...
    #[test]
    fn string_form_and_embed_form_render_byte_identical() {
        // THE migration invariant: a runnable whose source is a STRING and the SAME source embedded as FORMS
        // must render byte-identical <Runnable> TSX. (No backtick/${ in the code, so tmpl is identity.)
        let code = "(def (main) (f 5))\n(@ (requires (>= x 0))\n  (def (f (: x Int64)) (+ x 1)))";
        let str_text =
            format!("(chapter (slug \"x\") (runnable (source \"{code}\") (expect \"error\")))");
        let embed_text =
            format!("(chapter (slug \"x\") (runnable (source {code}) (expect \"error\")))");
        let (sa, ss) = parse(&str_text);
        let (ea, es) = parse(&embed_text);
        let sr = first(&sa, locate_chapter(&sa).unwrap(), "runnable");
        let er = first(&ea, locate_chapter(&ea).unwrap(), "runnable");
        assert_eq!(
            render_runnable(&sa, sr, &str_text, &ss),
            render_runnable(&ea, er, &embed_text, &es)
        );
    }
}
