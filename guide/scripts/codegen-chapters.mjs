...
// Binary resolution (the guide-examples nix gate has NO cargo vendor, so it can't `cargo build` here —
// v-nix builds `xtaskCodegenGuideBin` as its OWN crane derivation with a minimal closure and injects it).
// Order: (1) explicit override env $CDZ_XTASK_CODEGEN_GUIDE (pinning / CI); (2) `xtask-codegen-guide` on
// $PATH — v-nix's `nativeBuildInputs` injection, the nix path, no env contract needed; (3) `cargo build -p`
