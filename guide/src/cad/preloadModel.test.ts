...
/// model buffer with the `import … from "exact"` + `export main` so it compiles against the preloaded CAD
/// library. The load-bearing invariant is CONTIGUITY: the reader's verbatim text must appear as a single
/// substring of the injected output, or the linter's `wrapPrefixOf` span-mapping (a linear prefix
/// subtraction) misplaces every squiggle. These tests pin that + the exact per-surface import forms.
...
const ML_MODEL = `@!default-fraction Rational
def main() = lower(Solid.Difference(Solid.Cube(v3r(4/1, 4/1, 4/1)), Solid.Sphere(5/2)))`;

const SEXPR_MODEL = `(pragma default-fraction Rational)
(def (main) (lower ((. Solid Difference) ((. Solid Cube) (v3r (/ 4 1) (/ 4 1) (/ 4 1))) ((. Solid Sphere) (/ 5 2)))))`;

test("ML: injects the import as a PREFIX line + a trailing export", () => {
...
test("s-expr: wraps the inner forms in (do (import …) … (export main))", () => {
