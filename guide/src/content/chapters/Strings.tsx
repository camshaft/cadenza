...
(def (main) (greet "Cadenza"))`}
  ...
  (String.scalar-len
    (Option.expect (String.slice "hello world" 0 5) "in range")))`}
      ...
      <P>
        ...
        an out-of-range position is <C>None</C> rather than a trap. The character at position <C>1</C> of{" "}
        <C>"hello"</C> is <C>#\e</C>, whose scalar value is <C>101</C>:
      </P>
      <Runnable
        source={`(def (main)
  (Char.to-int
    (Option.expect (String.scalar-at "hello" 1) "in range")))`}
      />
      <P>
        <C>Char.to-int</C> reads a character's Unicode scalar value (its code point) as an <C>Int64</C>.
        It's <em>total</em>: every character has a code point, so it never fails. Going the other way,{" "}
        <C>Char.from-int</C> is <em>fallible</em>, since not every integer is a valid scalar, so it
        returns an <C>Option Char</C>. Code point <C>97</C> is <C>#\a</C>:
      </P>
      <Runnable
        source={`(def (main)
  (Char.to-int
    (Option.expect (Char.from-int 97) "valid scalar")))`}
      />
      <P>
        Because <C>from-int</C> is fallible, the invalid cases are data, not crashes. Code point{" "}
        <C>55296</C> is <C>U+D800</C>, a surrogate that is never a standalone scalar, so <C>from-int</C>{" "}
        gives <C>None</C>, and this match takes the <C>None</C> arm to return <C>0</C>:
      </P>
      <Runnable
        source={`(def (main)
  (match (Char.from-int 55296)
    ...
    ((None) 0)))`}
      />
      <Why tenet="A character converts to and from an integer, honestly">
        Every character has an integer code point, so <C>Char.to-int</C> is total. But the reverse isn't,
        because the surrogate range and everything past <C>U+10FFFF</C> aren't scalar values, so{" "}
        <C>Char.from-int</C> returns an <C>Option</C> instead of inventing an ill-formed character. The type
        tells you which direction can fail.
      </Why>
