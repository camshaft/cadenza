...
export default function Rationals() {
  return (
    <article>
      ...
      <Lede>
        A <C>Float64</C> is fast but approximate, so when you need an exact fraction like a third that
        really is a third, reach for a <C>Rational</C>.
      </Lede>

      <P>
        You saw in <strong>Floating-point numbers</strong> that <C>(+ 0.1 0.2)</C> isn't quite{" "}
        <C>0.3</C>, because floats trade exactness for speed. A <C>Rational</C> makes the other trade by
        holding a number as an exact ratio of two integers, so arithmetic never rounds. Build one with{" "}
        <C>Rational.of</C>, giving a numerator and a denominator:
      </P>
      <Runnable source={`(Rational.of 1 2)`} />
      <P>
        The value comes back as <C>1/2</C> tagged with its type, which reads <C>1/2 : Rational</C> in the
        conventional surface and <C>(: 1/2 Rational)</C> in s-expressions. Since a whole number is just a
        denominator of one, <C>Rational.of-int</C> makes that explicit:
      </P>
      ...
      <P>
        ...
        with an <C>R</C> suffix is a rational <em>literal</em> that the compiler reads exactly and
        converts to a fraction, so <C>0.5R</C> is <C>1/2</C> and <C>1.25R</C> is <C>5/4</C>:
      </P>
      <Runnable source={`0.5R`} />
      <P>
        It's the very same value as the constructor, since <C>0.5R</C> equals <C>(Rational.of 1 2)</C>,
        just terser to write. And it's where the contrast with <strong>Floating-point numbers</strong>
        stops being a claim and becomes something you can watch. Add a tenth and two tenths as{" "}
        <em>floats</em> and the answer isn't <C>0.3</C> but the nearest float to it, which isn't quite{" "}
        <C>0.3</C>:
      </P>
      <Runnable source={`(+ 0.1 0.2)`} />
      <P>
        <C>0.30000000000000004</C>, so the drift is real rather than hypothetical, and the natural test{" "}
        <em>fails</em> because the float sum is not equal to <C>0.3</C>.
      </P>
      ...
      <P>
        <C>true</C>, the same digits you'd type for a float with one letter's difference, and now{" "}
        <C>0.1 + 0.2</C> is the number you <em>meant</em>. The float wasn't buggy; it was doing exactly
        what binary floating-point must. The <C>Rational</C> simply makes the other trade of exactness
        over speed, so the arithmetic never rounds in the first place.
      </P>

      <H2>Whole numbers that outgrow Int64: <C>BigInt</C></H2>
      <P>
        The same instinct of trading speed for exactness when it matters has a whole-number counterpart.
        An <C>Int64</C> refuses to hold a value past its range, so <C>9223372036854775807 × 1000</C>{" "}
        overflows and the compiler declines rather than wrap.
      </P>
      <Runnable source={`(* 9223372036854775807 1000)`} expect="error" />
      <P>
        When you genuinely need bigger, <C>BigInt</C> is the arbitrary-precision integer that grows to fit
      ...
      <P>
        The result comes back as a <C>BigInt</C> far beyond the 64-bit range with no overflow and no wrap.
        It's the same trade as <C>Rational</C>: reach for it when a value must be exact whatever its size,
        and pay for the arbitrary precision only where you asked for it.
      </P>

      <H2>Always in lowest terms</H2>
      <P>
        A rational normalizes itself on construction, stored in lowest terms with the sign on the
      ...
      />
      <P>
        <C>1/1</C>, not <C>0.9999999999999999</C>. Division is exact too, and unlike integer division it
        stays total for any nonzero divisor, so <C>(3/4) / (2/1)</C> is <C>3/8</C> with no remainder and no
        rounding. You can try exact fractions yourself in the{" "}
        <Link to="/calculator" className="text-cadenza-300 underline-offset-2 hover:underline">
          calculator
        </Link>{" "}
        by typing <C>1 / 3 + 1 / 3 + 1 / 3</C> and watching it come back <C>1</C>.
      </P>
      ...
      <Why tenet="Exactness is a choice you can make">
        ...
        right tool when you want speed and can tolerate rounding, as in measurements, graphics, and
        physics. A <C>Rational</C> is the right tool when a rounding error would be a <em>bug</em>, as in
        money, exact ratios, and anything that must add up. They're different types with different operators, so you say
