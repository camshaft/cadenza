...
export default function BinaryMatching() {
  return (
    <article>
      ...
      <P>The same grammar, on the left of a <C>match</C> arm, <em>reads</em> a <C>Bytes</C> back. Matching a two-byte value against <C>(bin (u16 n))</C> binds <C>n</C> to the integer those bytes encode, since construction and matching are exact inverses:</P>
      <Runnable
      ...
      />
      <P>The fix is a trailing unsized <C>(bytes rest)</C>, which absorbs the variable-length remainder, so now the <C>u16</C> reads the first two bytes and <C>rest</C> takes the third, the arm matches, and the result is <Cadenza>(Some 258)</Cadenza>:</P>
      <Runnable
