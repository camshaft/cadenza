...
export default function BinaryMatching() {
  return (
    <article>
      ...
      <P>A <C>(bin …)</C> expression lays out fixed-width fields. <Cadenza ast="Y2R6YXN0AAECCgN1MTYAAgECAwAAAAEBAgABAg==" kind="expr">(u16 258)</Cadenza> is a 16-bit unsigned integer, two bytes, most-significant first (big-endian, the network default), while <C>(u8 …)</C> is one byte. Return it and you see the exact octets it lays down:</P>
      <Runnable
      ...
      />
      <P>Written as a hex literal, the same idea reads a magic-number header legibly, so a <C>u32</C> equal to <C>0x89504E47</C> is the PNG signature, and a trailing <Cadenza ast="Y2R6YXN0AAECCgVieXRlcwoEcmVzdAMAAAABAQIAAQI=" kind="expr">(bytes rest)</Cadenza> absorbs the payload after it. Here the question is only <em>is this a PNG</em>, so the arms return a <C>Bool</C> directly, the matched header being <C>true</C>:</P>
      <Runnable
  ...
  (_ 0))`}
        expected="3"
        hint={<>The size is the name bound by the earlier segment, <C>n</C>, read from the first byte (<C>3</C>). So <Cadenza ast="Y2R6YXN0AAEDCgVieXRlcwoEYm9keQoBbgQAAAABAAIBAwABAgM=" kind="expr">(bytes body n)</Cadenza> binds the next three bytes, and <C>rest</C> takes the trailing <C>99</C>.</>}
      />
    </article>
  );
}
