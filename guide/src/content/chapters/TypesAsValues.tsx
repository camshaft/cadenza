  ...
  return (
    <article>
      ...
      <Exercise
        ...
        prompt={<>Compare two types at compile time. Fill the type to compare against so that <Cadenza ast="Y2R6YXN0AAEEGgoEVHlwZQoCb2YJBgAAAAEAAgEDAAECAAMBAgMEBQ==" kind="expr">(Type.of true)</Cadenza> matches, making the comparison <C>true</C>, not <C>false</C>.</>}
        starter={`(Type.eq (Type.of true) ?)`}
        solution={`(Type.eq (Type.of true) Bool)`}
        expected="true"
        hint={<><Cadenza ast="Y2R6YXN0AAEEGgoEVHlwZQoCb2YJBgAAAAEAAgEDAAECAAMBAgMEBQ==" kind="expr">(Type.of true)</Cadenza> is the type <C>Bool</C>, so comparing it against <C>Bool</C> is <C>true</C>. (Compare against <C>Int64</C> and you'd get <C>false</C>.)</>}
      />
      <Exercise
        id="types-as-values:2"
        prompt={<>Two type-values are equal only when the types match exactly. Fill the value so its reflected type equals <Cadenza ast="Y2R6YXN0AAEEGgoEVHlwZQoCb2YAAQUGAAAAAQACAQMAAQIAAwECAwQF" kind="expr">(Type.of 5)</Cadenza>, an <C>Int64</C>, making the check <C>true</C>.</>}
        starter={`(Type.eq (Type.of 5) (Type.of ?))`}
        solution={`(Type.eq (Type.of 5) (Type.of 99))`}
        expected="true"
        hint={<><Cadenza ast="Y2R6YXN0AAEEGgoEVHlwZQoCb2YAAQUGAAAAAQACAQMAAQIAAwECAwQF" kind="expr">(Type.of 5)</Cadenza> is <C>Int64</C>, so you need another value whose type is <C>Int64</C>, any integer literal, e.g. <C>99</C>. A <C>true</C> or a <C>1.0</C> would be a different type and give <C>false</C>.</>}
      />
    </article>
  );
}
