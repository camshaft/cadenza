  ...
  return (
    <article>
      ...
      <P>
        ...
        <C>|&gt;</C> flips that around — <C>x |&gt; f</C> means "take <C>x</C> and feed it to <C>f</C>",
...
(def (main) (|> (|> 5 inc) dbl))`}
      ...
      <P>
        ...
        pipeline reads like a sentence — data flowing through a sequence of steps.
      </P>

      <Why tenet="One program, many syntaxes">
        <C>|&gt;</C> isn't a magic construct baked into the compiler — it's an ordinary operator the
      ...
      <P>
        Sometimes you don't want to pipe a value right now — you want a <em>new function</em> that is
        two functions glued together. Since functions are values, you can write <C>compose</C> yourself:
...
(def (main)
  ((compose inc dbl) 10))`}
      />
      <P>
        <C>(compose inc dbl)</C> is a brand-new function — double, then increment — and applying it to
      ...
      <P>
        ...
        function waiting for the second. So you can apply arguments one at a time — this is called{" "}
  ...
  (let ((add3 (adder 3)))
    (+ (add3 1) (add3 10))))`}
      ...
      <Why tenet="Uniformity over special cases">
        Currying isn't a bolted-on feature — it falls out of a single rule: every function takes exactly
        one argument. Multi-argument functions, partial application, and the pipeline operator are all
...
(def (main)
  (map-sum (fn (x) (+ x 1)) 0 (list 5 7 30)))`}
      ...
      <Exercise
        id="functions:1"
        prompt={<>Use the pipeline to feed <C>4</C> through <C>dbl</C> — the result should be <C>8</C>.</>}
