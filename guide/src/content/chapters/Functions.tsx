...
export default function Functions() {
  return (
    <article>
      ...
      <Lede>
        Functions are values, so you can build bigger transformations out of smaller ones by piping,
        by composing, and by applying one argument at a time.
      </Lede>

      <P>
        You've already met higher-order functions in <C>Values &amp; functions</C>: because a function
        is an ordinary value, you can pass one to another function and return one from a function. This
        chapter is about the everyday shapes that grow out of that single idea.
      </P>

      <H2>The pipeline operator</H2>
      <P>
        When you transform a value through a series of steps, writing it as nested calls reads
        inside-out: the <em>last</em> thing to happen is written <em>first</em>. The pipeline operator{" "}
        <C>|&gt;</C> flips that around, so <C>x |&gt; f</C> means "take <C>x</C> and feed it to <C>f</C>",
        i.e. exactly <C>(f x)</C>, but written left-to-right in the order things happen.
      </P>
      <Runnable
...
(def (main) (|> (|> 5 inc) dbl))`}
      />
      <P>
        Read the <C>main</C> in the conventional syntax (flip the toggle in the header) and it becomes{" "}
        <C>5 |&gt; inc |&gt; dbl</C>: start with 5, increment to 6, double to 12. Same program, but the
        pipeline reads like a sentence, with data flowing through a sequence of steps.
      </P>
      ...
      <Why tenet="One program, many syntaxes">
        ...
        pipeline is sugar for readability, never a second way for a program to <em>mean</em> something.
      </Why>

      <H2>Composing two functions into one</H2>
      <P>
        Sometimes you don't want to pipe a value right now, but instead want a <em>new function</em> that
        is two functions glued together. Since functions are values, you can write <C>compose</C> yourself:
        it takes two functions and returns a function that runs one, then the other.
      </P>
      <Runnable
...
(def (main)
  ((compose inc dbl) 10))`}
      />
      <P>
        <C>(compose inc dbl)</C> is a brand-new function that doubles then increments, so applying it to
        10 gives 21. Nothing built-in made this possible; <C>compose</C> is a few characters of ordinary
        Cadenza, because functions are just values you can package up.
      </P>

      <H2>One argument at a time</H2>
      <P>
        A two-argument function is really a function that takes the first argument and returns a
        function waiting for the second. So you can apply arguments one at a time, which is called{" "}
        <em>currying</em>. Give <C>add</C> just its first argument and you get back a function:
      </P>
      <Runnable
...
(def (main) ((add 3) 4))`}
      />
      <P>
        <C>(add 3)</C> is a function that adds 3 to whatever comes next; applying it to 4 yields 7. And
        capturing that partial application in a binding gives you a small, reusable specialization:
      </P>
      <Runnable
...
(def (main)
  (let ((add3 (adder 3)))
    (+ (add3 1) (add3 10))))`}
      />
      <P>
        <C>add3</C> remembers its <C>n</C> and can be called as many times as you like, here twice, for{" "}
        <C>4 + 13 = 17</C>.
      </P>
      ...
      <P>
        The everyday use of a higher-order function is to pass a small function <em>inline</em>, as in the
        ...
        because inference works both out from how <C>f</C> is used and in from the lambda's body:
      </P>
      <Runnable
...
(def (main)
  (fold-list (fn (x a) (+ a x)) 0 #list(5 7 30)))`}
      ...
      <P>
        ...
        inference recovers all of it from how they're used. That's the everyday shape: write the callback
        inline and let the types follow.
      </P>
...
(def (main) (|> 4 dbl))`}
      ...
      <Exercise
        ...
        prompt={
          <>
            <C>compose</C> is written for you, and <C>(compose f g)</C> runs <C>g</C> first, then <C>f</C>.
