...
export default function MapsSets() {
  ...
  (Option.expect (Map.lookup #map((= 1 10) (= 2 20)) 2) "missing"))`}
      ...
      <P>
        A map holds one value per key, so inserting the same key again replaces the old value, and the size
        doesn't grow:
      </P>
      <Runnable
        source={`(def (main)
  ...
  (match (. (Map.take #map((= 1 10) (= 2 20)) 1) 0)
    ...
    ((None _) -1)))`}
      />
      <P>
        The other half of the tuple, <C>.1</C>, is the smaller map with one entry left. Return it and you can
      ...
      <Exercise
        ...
        prompt={
          <>
            ...
            removing a key builds one without it, which is what <C>Map.remove</C> does. Take one key away, then ask{" "}
