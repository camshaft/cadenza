(chapter
  ...
  (p "You saw in " (strong "Floating-point numbers") " that " (c "(+ 0.1 0.2)") " isn't quite " (c "0.3") ", because floats trade exactness for speed. A " (c "Rational") " makes the other trade by holding a number as an exact ratio of two integers, so arithmetic never rounds. Build one with " (c "Rational.of") ", giving a numerator and a denominator:")
  (runnable
  ...
  (runnable
    (source "0.5R"))
  (p "It's the very same value as the constructor, since " (c "0.5R") " equals " (c "(Rational.of 1 2)") ", just terser to write. And it's where the contrast with " (strong "Floating-point numbers") " stops being a claim and becomes something you can watch. Add a tenth and two tenths as " (em "floats") " and the answer isn't " (c "0.3") " but the nearest float to it, which isn't quite " (c "0.3") ":")
  (runnable
    (source "(+ 0.1 0.2)"))
  ...
  (note "A zero denominator has no value to denote, so " (c "(Rational.of 1 0)") " is a compile-time error (" (c "CDZ0304") "), the same \"no correct answer, so refuse\" rule as dividing an integer by zero.")
  ...
  (exercise
    (id "rationals:1")
    (prompt "A rational is compared by value, so equal fractions are " (c "=") " however they're written. Write the fraction " (c "3/6") " in lowest terms so it equals " (c "(Rational.of 1 2)") " and the comparison gives " (c "true") ".")
  ...
  (exercise
    ...
    (expected "true")
    (hint "A quarter is " (c "(Rational.of 1 4)") ". Dividing by it multiplies by its reciprocal " (c "4/1") ", so " (c "1/2") " becomes " (c "4/2 = 2/1") ", exactly two with no rounding."))
  (exercise
    (id "rationals:3")
    ...
    (expected "true")
    (hint (c "6/8") " reduces to " (c "3/4") ", whose numerator is " (c "3") ". " (c "Rational.numerator") " reads the top; " (c "Rational.denominator") " would give " (c "4") ". The accessor returns a " (c "BigInt") ", so it's compared against " (c "(BigInt.of 3)") ".")))
