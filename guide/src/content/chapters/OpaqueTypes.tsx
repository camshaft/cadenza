...
export default function OpaqueTypes() {
  return (
    <article>
      ...
      <P>Exporting a type and exporting its <em>constructors</em> are two independent decisions. A bare <Cadenza ast="Y2R6YXN0AAECCgZleHBvcnQKB1BlcmNlbnQDAAAAAQECAAEC" kind="expr">(export Percent)</Cadenza> publishes only the type's <em>handle</em>, enough to name it but not to construct it. Adding <Cadenza ast="Y2R6YXN0AAECCgZleHBvcnQKCVBlcmNlbnQuKgMAAAABAQIAAQI=" kind="expr">(export Percent.*)</Cadenza> (or naming a specific variant) publishes the constructors too, making the type <em>concrete</em>. So opacity is the <em>default</em> of exporting a type; concreteness is opt-in. Withholding the constructor is what makes a type opaque.</P>
