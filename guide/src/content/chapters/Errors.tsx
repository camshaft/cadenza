...
export default function Errors() {
  return (
    <article>
      ...
      <P>Not every operation has an answer, and a language has to do something when you look past the end of a list or look up a missing key or ask for a result that doesn't fit. Cadenza represents a value that might be missing with the <C>Option</C> type, which is either <Cadenza ast="Y2R6YXN0AAECCgRTb21lCgF4AwAAAAEBAgABAg==" kind="expr">(Some x)</Cadenza> with a value or <Cadenza ast="Y2R6YXN0AAECCgROb25lCgR1bml0AwAAAAEBAgABAg==" kind="expr">(None unit)</Cadenza> with nothing, so the type system makes a caller acknowledge both cases rather than return a bogus default. A genuinely undefined operation like dividing by zero is a different story that halts rather than inventing a value, as you saw in <strong>The numeric model</strong>, so this chapter is about the kind of absence you can handle.</P>
...
(def (main) (match (safe-div 10 2) ((Some q) q) ((None _) -1)))`}
      ...
      <P>Match on one <C>Option</C> and you write two arms. Chain several (add two checked results, each of which might overflow) and the nested matches pile up, burying the happy path. The <C>?</C> operator (written <C>(try …)</C>) collapses that: on a <C>Some</C> it unwraps the value and carries on; on a <C>None</C> it <em>short-circuits</em>, making the whole function's result that <C>None</C>. Here two checked-adds both succeed, so <C>x</C> and <C>y</C> unwrap and the function returns <Cadenza ast="Y2R6YXN0AAECCgRTb21lAAFUAwAAAAEBAgABAg==" kind="expr">(Some 84)</Cadenza>:</P>
      <Runnable
        source={`(def
  (main)
  (let
  ...
  (let
    ...
    (let ((y (try (Int64.checked-add 40 2)))) (Some (+ x y)))))`}
      ...
      <P>A pattern can look <em>inside</em> a variant, not just name it. Here the first arm only fires for exactly <Cadenza ast="Y2R6YXN0AAECCgRTb21lAAADAAAAAQECAAEC" kind="expr">(Some 0)</Cadenza>; a different <C>Some</C> falls through to the binding arm.</P>
      <Runnable
...
(def (main) (describe (Some 0)))`}
      ...
      <P>An <C>Option</C> says <em>whether</em> there's an answer; sometimes you also want to say <em>why</em> there isn't. <C>Result</C> is the sum for that: <Cadenza ast="Y2R6YXN0AAECCgJPawoFdmFsdWUDAAAAAQECAAEC" kind="expr">(Ok value)</Cadenza> when it worked, <Cadenza ast="Y2R6YXN0AAECCgNFcnIKAWUDAAAAAQECAAEC" kind="expr">(Err e)</Cadenza> carrying a reason when it didn't, and you take it apart with <C>match</C> exactly like an <C>Option</C>. Here <C>safe-div</C> reports the offending divisor on failure:</P>
      <Runnable
...
(def (main) (match (safe-div 20 4) ((Ok q) q) ((Err e) -1)))`}
      ...
      <Exercise
        id="errors:1"
        prompt={<><C>check</C> returns <Cadenza ast="Y2R6YXN0AAECCgNFcnIAAQgDAAAAAQECAAEC" kind="expr">(Err 8)</Cadenza> for this input. Finish the <C>Err</C> arm to hand back the reason it carries, so the result is <C>8</C>.</>}
...
(def (main) (match (take-from 3 10) ((Some left) left) ((None _) -1)))`}
        expected="-1"
        hint={<>The <C>Some</C> branch already carries a value; the empty branch carries nothing, which you write as <Cadenza ast="Y2R6YXN0AAECCgROb25lCgR1bml0AwAAAAEBAgABAg==" kind="expr">(None unit)</Cadenza>, the same <C>None</C> the <C>match</C> below is waiting for.</>}
      />
