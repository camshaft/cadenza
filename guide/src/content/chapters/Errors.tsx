...
(def (main)
  (match (safe-div 10 2)
    ...
    ((None _) -1)))`}
      ...
      <H2>Safe indexing</H2>
      <P>
        The same pattern shows up in the standard library. <C>List.at</C> returns an <C>Option</C>, so you
  ...
  (let ((x (try (Int64.checked-add 20 22))))
    (let ((y (try (Int64.checked-add 40 2))))
      ...
      <P>
        ...
        answer immediately, so the second <C>try</C> and the body never run:
      </P>
      <Runnable
        source={`(def (main)
...
(def (main)
  (match (check 8)
    ...
    ((Err e) e)))`}
        ...
        hint={
          <>
            The <C>Err</C> arm binds its payload to <C>e</C>, so return that binding to surface the reason:{" "}
            just <C>e</C>.
          </>
        }
      />
  ...
  (match (take-from 3 10)
    ...
    ((None _) -1)))`}
      ...
      <P>
        ...
        operation and lets whoever runs it decide what it means, so the caller answers instead of just
