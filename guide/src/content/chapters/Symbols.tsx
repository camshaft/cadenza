...
export default function Symbols() {
  return (
    <article>
      <H1>Symbols</H1>
      <Lede>
        Sometimes a value is just a <em>name</em>, like a status, a mode, or one choice from a fixed set. A
        symbol is exactly that: an interned name you compare by identity.
      </Lede>

      <P>
        A symbol is written <C>#"…"</C>, where the <C>#</C> tells it apart from a text string. Where a string
  ...
  (if (= #"red" #"red") 1 0))`}
      ...
      <Note>
        ...
        <C>#"red"</C> may be written <C>#red</C>, and the two are the same symbol. The quotes are only needed
      ...
      <P>
        ...
        <C>#"red"</C>, <C>#"yellow"</C>, or <C>#"green"</C>, and a function can decide on it, here choosing
...
(def (main) (wait #"red"))`}
      ...
      <P>
        ...
        around as a plain value and matched by name where the decision is made, with no numbers to remember
        and no strings to keep in sync.
      </P>
      ...
      <Why tenet="A name compared by identity, not by its text">
        ...
        label it is</em>. Making it its own type says so, so you can't accidentally slice a status tag or
      ...
      <P>
        When a name arrives as text, whether parsed from input or assembled at run time, <C>Symbol.of</C>{" "}
  ...
  (if (= (Symbol.of (String.concat "ye" "s")) #"yes") 1 0))`}
      ...
      <Note>
        ...
        place you cross between the two types is spelled out, and a symbol and a string never silently
      ...
      <Exercise
        ...
        prompt={
          <>
            ...
            fallback arms are done, so fill the middle comparison to make <C>(score #"silver")</C> give <C>2</C>.
          </>
        }
        starter={`(def (score m)
  (if (= m #"gold") 3
