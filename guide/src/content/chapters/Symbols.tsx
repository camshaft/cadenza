...
export default function Symbols() {
  return (
    <article>
      ...
      <P>That's what symbols are for: a value drawn from a small, known set of names. A traffic light is <Cadenza ast="Y2R6YXN0AAEBDwNyZWQBAAAA" kind="expr">#"red"</Cadenza>, <Cadenza ast="Y2R6YXN0AAEBDwZ5ZWxsb3cBAAAA" kind="expr">#"yellow"</Cadenza>, or <Cadenza ast="Y2R6YXN0AAEBDwVncmVlbgEAAAA=" kind="expr">#"green"</Cadenza>, and a function can decide on it, here choosing how many seconds to wait:</P>
      <Runnable
        source={`(def (wait light) (if (= light #"red") 30 (if (= light #"yellow") 5 0)))

(def (main) (wait #"red"))`}
      />
      <P><Cadenza ast="Y2R6YXN0AAEBDwNyZWQBAAAA" kind="expr">#"red"</Cadenza> waits 30, <Cadenza ast="Y2R6YXN0AAEBDwZ5ZWxsb3cBAAAA" kind="expr">#"yellow"</Cadenza> 5, and anything else (green) 0. The light is passed around as a plain value and matched by name where the decision is made, with no numbers to remember and no strings to keep in sync.</P>
      ...
      <P>When a name arrives as text, whether parsed from input or assembled at run time, <C>Symbol.of</C> interns it into a symbol. The result is the very same value as writing the literal: a symbol built from the pieces <C>"ye"</C> and <C>"s"</C> equals <Cadenza ast="Y2R6YXN0AAEBDwN5ZXMBAAAA" kind="expr">#"yes"</Cadenza>:</P>
      <Runnable
      ...
      <Exercise
        id="symbols:1"
        prompt={<><C>score</C> dispatches on a medal from the fixed set <Cadenza ast="Y2R6YXN0AAEBDwRnb2xkAQAAAA==" kind="expr">#"gold"</Cadenza> / <Cadenza ast="Y2R6YXN0AAEBDwZzaWx2ZXIBAAAA" kind="expr">#"silver"</Cadenza> / <Cadenza ast="Y2R6YXN0AAEBDwZicm9uemUBAAAA" kind="expr">#"bronze"</Cadenza>: gold scores <C>3</C>, silver <C>2</C>, anything else <C>1</C>. The gold and fallback arms are done, so fill the middle comparison to make <Cadenza ast="Y2R6YXN0AAECCgVzY29yZQ8Gc2lsdmVyAwAAAAEBAgABAg==" kind="expr">(score #"silver")</Cadenza> give <C>2</C>.</>}
...
(def (main) (score #"silver"))`}
        expected="2"
        hint={<>The middle arm handles silver, so compare <C>m</C> against <Cadenza ast="Y2R6YXN0AAEBDwZzaWx2ZXIBAAAA" kind="expr">#"silver"</Cadenza>. Each symbol is checked by equality; <Cadenza ast="Y2R6YXN0AAEBDwZicm9uemUBAAAA" kind="expr">#"bronze"</Cadenza> matches neither and falls through to <C>1</C>.</>}
      />
      <Exercise
        id="symbols:2"
        prompt={<>A symbol is an ordinary value, so a function can <em>return</em> one, not just test it. <C>next</C> advances a traffic light around its cycle: red turns to green, green to yellow, yellow back to red. The green and yellow cases are written, so fill the hole with the symbol red becomes, making <Cadenza ast="Y2R6YXN0AAECCgRuZXh0DwNyZWQDAAAAAQECAAEC" kind="expr">(next #"red")</Cadenza> return <Cadenza ast="Y2R6YXN0AAEBDwVncmVlbgEAAAA=" kind="expr">#"green"</Cadenza> and the check give <C>true</C>.</>}
...
(def (main) (= (next #"red") #"green"))`}
        expected="true"
        hint={<>The hole is the value the function <em>hands back</em> for red, a symbol literal, <Cadenza ast="Y2R6YXN0AAEBDwVncmVlbgEAAAA=" kind="expr">#"green"</Cadenza>. The result is a symbol like any other, which the check then compares against <Cadenza ast="Y2R6YXN0AAEBDwVncmVlbgEAAAA=" kind="expr">#"green"</Cadenza>.</>}
      />
    </article>
  );
}
