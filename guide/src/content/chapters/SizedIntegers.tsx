...
export default function SizedIntegers() {
  return (
    <article>
      ...
      <Lede>
        <C>Int64</C> is the everyday integer, but when a value has a fixed width, whether a byte, a 16-bit
        field, or a protocol number, Cadenza gives it a type that says so, and never converts one width to
      ...
      <P>
        ...
        literal, since a bare <C>200</C> is an <C>Int64</C>. You <em>convert into</em> a sized type by
        name, with <C>.of</C>:
      </P>
      <Runnable source={`(UInt8.of 200)`} />
      <P>
        <C>200</C> fits in a byte, so this is the <C>UInt8</C> value 200. Different widths hold different
        ranges, so <C>UInt16.of</C> happily takes a number a byte couldn't:
      </P>
      <Runnable source={`(UInt16.of 300)`} />

      <H2>Conversion is checked</H2>
      <P>
        What if the number doesn't fit? <C>.of</C> is the <em>checked</em> conversion, so it verifies the
      ...
      <Note>
        This one is <strong>meant to trap</strong>. Run it and read the status bar, where the range check
        firing is the point, so you find out immediately, not via a corrupted value downstream.
      </Note>
      <Runnable source={`(UInt8.of 300)`} expect="error" />

      <H2>When you want the low bits: <C>wrap</C></H2>
      <P>
        Sometimes truncation is exactly what you mean, whether for a checksum, a rollover counter, or
        packing into a byte. That's a different, explicit operation, since <C>wrap</C> keeps the low bits
        and discards the rest. <C>300</C> wrapped to 8 bits is <C>300 − 256 = 44</C>:
      </P>
      <Runnable source={`(UInt8.wrap 300)`} />
      <P>
        <C>of</C> and <C>wrap</C> are the two honest choices for "this doesn't fit", either halting or
        taking the low bits, and you say which one you mean. Neither happens by accident.
      </P>

      <H2>Widths are distinct types</H2>
      <P>
        A <C>UInt8</C> and a <C>UInt16</C> are <em>different types</em>. Try to add them and the compiler
        refuses, because it won't quietly widen one to match the other:
      </P>
      <Runnable source={`(+ (UInt8.of 1) (UInt16.of 300))`} expect="error" />
      <P>
        The fix is to say which width you want and convert there, so the widening is a thing you write,
    ...
    (export main))`} />
      ...
      <P>
        ...
        <C>UInt8</C>s add as a <C>UInt8</C>, and a sum that would exceed <C>255</C> is caught, exactly
        like <C>Int64</C> overflow. <C>200 + 100</C> can't fit a byte, so it's refused:
      </P>
      <Note>
      ...
      <Note>
        Signed widths work the same way, so <C>Int8.of</C> takes −128 through 127, traps outside it, and{" "}
        <C>Int8.wrap</C> gives you the low 8 bits. Same two choices, at every width.
      </Note>

      <P>
        Integers, checked and sized and never blurred, are only half the number line. Next comes the other
        half, the approximate, real-valued world of <em>floating-point numbers</em>, with its own operators
        and its own honest tradeoff.
      </P>
      ...
      <Exercise
        ...
        prompt={
          <>
            ...
            the operation that takes the low bits, so the result should be <C>2</C> (that's{" "}
        ...
        hint={
          <>
            <C>wrap</C> keeps the low bits, whereas <C>of</C> would <em>refuse</em> <C>258</C> as out of
            range. You asked for truncation, so it's <C>wrap</C>, and <C>258</C> wraps to <C>2</C>.
          </>
        }
      />
      ...
      <Exercise
        ...
        hint={
          <>
            Widths don't mix, so convert the <C>1</C> at the same width as the other operand, namely{" "}
