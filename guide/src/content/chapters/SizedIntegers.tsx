...
export default function SizedIntegers() {
  return (
    <article>
      ...
      <P>A newcomer's first reflex is to annotate with a bare <C>Int</C>, the way most languages name their integer type. In Cadenza <C>Int</C>, <C>UInt</C>, and <C>Float</C> are not types at all: they're <em>width constructors</em> that <em>build</em> a sized type from a bit width. <C>(Int 64)</C> is <C>Int64</C>, <C>(UInt 8)</C> is <C>UInt8</C>, <C>(Float 32)</C> is <C>Float32</C>. So writing a bare <C>Int</C> where a type belongs uses a value where a type is required, and the compiler says so:</P>
      ...
      <Runnable
        ...
        expect="error"
      />
      <P>The fix is to name a concrete width, so <C>Int64</C> for the everyday integer, or <C>Int32</C>, <C>UInt8</C>, and the rest for a fixed size. The compound form <C>(Int 64)</C> is itself a perfectly good type, exactly equal to <C>Int64</C>, so this compiles and runs, and <C>(add1 41)</C> is <C>42</C>:</P>
      <Runnable
        source={`(do
