...
export default function Welcome() {
  return (
    <article>
      ...
      <Why tenet="Written for agents and humans">
        Cadenza's whole shape follows from one choice of audience: it is meant to be{" "}
        <em>written and read by AI agents</em> as much as by people. Once your author is a machine that
        manipulates structure and acts on your output, a cascade of decisions follows: code stored as
        data, diagnostics that carry verified fixes, one program shown in whichever syntax the reader
        prefers. As you go, watch how often a feature traces back to this.
      </Why>
      ...
      <P>
        ...
        (the direct code-as-data form). Switching never changes the program, since it is the same structure,
        printed differently.
      </P>
      ...
      <Why tenet="One representation, many syntaxes">
        A Cadenza program isn't <em>really</em> text, but a uniform data structure (code as data),
        and each syntax is just a lossless <em>projection</em> of it. That's why there's no single
        "the syntax" everyone must agree on: humans get a readable surface, agents get a
        literal-structure surface, and neither is privileged. It also means whitespace, line endings,
        and which syntax you chose can never change a program's identity, because the identity is the tree,
        not the text.
      </Why>
      ...
      <Why tenet="Decline, don't miscompile">
        Cadenza orders its possible outcomes by safety: a wrong value is worse than a crash, a crash is
        worse than an honest refusal, and a correct answer is best of all. So when the compiler can't be
        sure it would compile your program <em>correctly</em>, it declines rather than guessing. A
        refusal is never a dead end, either, since the goal is that every rejection carries a machine-checkable
        route back to a working program.
      </Why>

      <Why tenet="The source is the truth">
        A program's meaning lives in its source, never in the artifact it compiles to. The WebAssembly
        component is a rebuildable, content-addressed <em>function</em> of the source, so you fix a bug
      ...
      <P>
        ...
        The next chapter, <em>Why Cadenza is the way it is</em>, lays them out, and you don't need them to write
