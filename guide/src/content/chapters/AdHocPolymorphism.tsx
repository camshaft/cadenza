...
(def
  ...
  (+
    ...
    (show-with Bool #record((= describe describe-bool)) true)))`}
      ...
      />
      <P>The <C>(+ 1.5 1.5)</C> is float addition and the <C>(+ 1 2)</C> integer addition, the same symbol picking the right implementation per type and folding to <C>3</C>. You didn't pass a dictionary because the prelude already provides the instances; it's the record-of-operations idea with the record built in.</P>
...
(def (main) (apply-op #record((= op triple)) 5))`}
      ...
      <Exercise
        ...
        prompt={<>The same <C>+</C> means integer or float addition depending on its operands. Fill the float so the comparison <C>(&lt; (+ 2.0 ?) 5.0)</C> is <em>true</em>, which selects the <C>(+ 10 20)</C> integer branch and gives <C>30</C>.</>}
        starter={`(def (main) (if (< (+ 2.0 ?) 5.0) (+ 10 20) 0))`}
        solution={`(def (main) (if (< (+ 2.0 1.0) 5.0) (+ 10 20) 0))`}
        expected="30"
        hint={<>You need <C>(+ 2.0 ?)</C> to stay under <C>5.0</C>, so any float below <C>3.0</C> works, and <C>1.0</C> gives <C>3.0 &lt; 5.0</C>, true. The addition on <C>2.0</C> is float addition, while <C>(+ 10 20)</C> is integer addition: one operator, its per-type instances.</>}
      />
    </article>
  );
}
