...
export default function AdHocPolymorphism() {
  return (
    <article>
      ...
      <P>Say you want a <C>show</C>-like operation that turns a value into a number, differently per type. The "trait" is a record with a <C>describe</C> field: <C>{"{ describe: T -> Int64 }"}</C>. An <em>instance</em> for a type is a record with that field filled in. A polymorphic function takes the record and calls <Cadenza ast="Y2R6YXN0AAEDGgoEZGljdAoIZGVzY3JpYmUEAAAAAQACAQMAAQID" kind="expr">dict.describe</Cadenza>. Dispatch is picking which record to hand it:</P>
      <Runnable
...
(def
  ...
  (+
    ...
    (show-with #record((= describe describe-bool)) true)))`}
      ...
      <P>The unannotated version is generic but silent about it. If you'd rather <em>document</em> the constraint that "this works for some type <C>t</C>, given a dictionary of <C>describe : t → Int64</C>", take the type as an explicit <Cadenza ast="Y2R6YXN0AAEDCgE6CgF0CgRUeXBlBAAAAAEAAgEDAAECAw==" kind="expr">(: t Type)</Cadenza> parameter (the same first-class <em>types as values</em> idea) and annotate the rest in terms of it. The caller passes the concrete type at the front; both instances still dispatch through the one <C>show-with</C>:</P>
      <Runnable
        source={`(def (describe-int n) (+ n 100))
