  ...
  return (
    <article>
      ...
      <Lede>
        ...
        system for this, but Cadenza doesn't need one, because a "trait" is just a <em>record of functions
        you pass in</em> and dispatch is choosing which record to pass. No new machinery: records and
        functions you already have.
      </Lede>

      <P>
        "Polymorphism" means one piece of code working over many types. The <em>ad-hoc</em> kind is where
        the behaviour genuinely differs per type, so <C>show</C> renders an integer one way and a boolean
      ...
      <P>
        Say you want a <C>show</C>-like operation that turns a value into a number, differently per type.
        The "trait" is a record with a <C>describe</C> field: <C>{`{ describe: T -> Int64 }`}</C>. An{" "}
...
(def (main)
  (+ (show-with (int-show) 5)
     (show-with (bool-show) true)))`}
      />
      <P>
        <C>show-with</C> is the polymorphic function, and it doesn't know or care <em>which</em> describe
        it's calling. <C>int-show</C> and <C>bool-show</C> are the two instances (the "trait
        implementations"), each a plain record. <C>main</C> dispatches by passing the right one, so{" "}
        <C>5</C> becomes <C>105</C> and <C>true</C> becomes <C>1</C>, summing to <C>106</C>. That's the
        whole mechanism, with no trait keyword and no instance resolution, just a record argument. And it's
        deliberate: the spec says a trait instance is an <em>explicitly-passed dictionary</em>, never
        resolved from ambient scope, so you can always see exactly which implementation a call uses by
        reading the argument.
      </P>

      <H2>Make the dictionary free: a const parameter</H2>
      <P>
        Passing a record around sounds like it should cost something at run time, but it doesn't have to,
        because the <C>const</C> parameter you met in the previous chapter applies here: mark the dictionary
        parameter <C>const</C> and the compiler inlines it into a specialized copy and erases it, so the{" "}
        <C>(. dict describe)</C> lookup folds to the concrete function with no record passed and no field
  ...
  (+ (show-with #record((= describe describe-int)) 5)
     (show-with #record((= describe describe-bool)) true)))`}
      ...
      <P>
        ...
        <C>describe : t → Int64</C>", take the type as an explicit <C>{`(: t Type)`}</C> parameter (the same
        first-class{" "}
...
(def (main)
  (+ (show-with Int64 #record((= describe describe-int)) 5)
     (show-with Bool #record((= describe describe-bool)) true)))`}
      ...
      <P>
        ...
        leaves to inference, so pick whichever fits how much you want the reader to see.
      </P>
      ...
      <P>
        ...
        per-type implementation, choosing integer addition for integers, floating-point for floats, and
        exact for rationals by the operand type. Here the very same <C>+</C> and <C>&lt;</C> run over floats
  ...
  (if (< (+ 1.5 1.5) 4.0)
    ...
    99))`}
      />
      <P>
        The <C>(+ 1.5 1.5)</C> is float addition and the <C>(+ 1 2)</C> integer addition, the same symbol
        picking the right implementation per type and folding to <C>3</C>. You didn't pass a dictionary
        because the prelude already provides the instances; it's the record-of-operations idea with the
        record built in.
      </P>
...
(def (main)
  (+ (len (list 1 2 3)) (len (list "a" "b"))))`}
      ...
      <P>
        ...
        separate machine function for each and no runtime type tag. (<C>cdz instantiations len some-file.cdz</C>{" "}
        lists the concrete types it was compiled at; the editor shows the same as a CodeLens above the
        definition.) This is the <em>parametric</em> case, one algorithm reused, whereas the dictionary and
      ...
      <Note>
        ...
        keyword and no compiler search for "the <C>Show</C> instance for this type" from ambient scope, so
        you pass the dictionary yourself or use a prelude operator that already carries its instances. You
        also
      ...
      <Why tenet="Traits are records you pass in, not new machinery">
        A typeclass or trait system is a second little language: instance declarations, resolution rules,
        coherence checks. Cadenza gets the same expressiveness from what it already has, since a record of
...
(def (main) (apply-op #record((= op triple)) 5))`}
        ...
        hint={
          <>
            The record's <C>op</C> field is the function to call, so put <C>triple</C> there and{" "}
      ...
      <Exercise
        ...
        prompt={
          <>
            ...
            the comparison <C>(&lt; (+ 2.0 ?) 5.0)</C> is <em>true</em>, which selects the{" "}
            <C>(+ 10 20)</C> integer branch and gives <C>30</C>.
          </>
        }
        starter={`(def (main)
  (if (< (+ 2.0 ?) 5.0) (+ 10 20) 0))`}
  ...
  (if (< (+ 2.0 1.0) 5.0) (+ 10 20) 0))`}
        ...
        hint={
          <>
            You need <C>(+ 2.0 ?)</C> to stay under <C>5.0</C>, so any float below <C>3.0</C> works, and{" "}
