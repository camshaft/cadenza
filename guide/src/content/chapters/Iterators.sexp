(chapter
  ...
  (p "The shape is a " (em "lazy pull") ": an iterator answers one question, " (c "next") ", \"give me the next element, and the iterator for the rest.\" When there's nothing left it says so. We model the answer as an " (c "Option") " of a " (c "(element, rest)") " pair: " (c "(Some #tuple(v rest))") " yields " (c "v") " and hands back the iterator " (c "rest") " for what follows, or " (c "(None unit)") " when the sequence is exhausted.")
