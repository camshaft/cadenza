  ...
  return (
    <article>
      ...
      <P>
        ...
        numbers. A number written with a decimal point is a <C>Float64</C>, a 64-bit IEEE-754 value that
        most languages call a "double".
      </P>
      <Runnable source={`3.14`} />

      <H2>The same operators, dispatched on the type</H2>
      <P>
        You add, subtract, multiply, and divide floats with the <em>same</em> <C>+</C>, <C>-</C>,{" "}
        <C>*</C>, <C>/</C> operators as integers. There's no separate float operator to remember, because
        when both operands are floats <C>+</C> <em>is</em> floating-point addition, and when both are
      ...
      <P>
        Notice <C>(/ 7.0 2.0)</C> is <C>3.5</C>, which is real division because the operands are floats. Give{" "}
      ...
      <P>
        That <C>0.30000000000000004</C> isn't a bug, since it's the true value of the nearest float to the
      ...
      <P>
        ...
        <C>Float64.of-int</C>. It's an ordinary function, visible and deliberate and exactly where you
        meant the conversion to happen.
      </P>
      <Runnable source={`(Float64.of-int 7)`} />
      <P>
        Now the number is a float, so it composes with float arithmetic using the same <C>*</C>, now
      ...
      <P>
        Putting it together, here's the area of a circle, all in floating-point. Edit the radius and Run.
      </P>
      <Runnable
...
(def (main) (area 2.0))`}
      ...
      <P>
        ...
        runnable value, and dividing two of them gives a <C>Float32</C> back, <C>7 ÷ 2 = 3.5</C>:
      </P>
      ...
      <P>
        ...
        <C>Float32</C> to a <C>Float64</C> and the compiler stops you, because a <C>Float32</C> and a{" "}
        <C>Float64</C> have different precision, so combining them is a conversion you must write, not one
        the language guesses:
      </P>
      <Note>
        This one is <strong>meant to be refused</strong>. Run it and the diagnostic is <C>CDZ0301</C>,{" "}
      ...
      <P>
        ...
        <em>bug</em>, as with money or exact ratios, you want the opposite trade. That's{" "}
        <em>exact fractions</em>, next.
      </P>
      ...
      <Exercise
        ...
        prompt={
          <>
            ...
            <C>Float32</C>, so convert the <C>5</C> at the matching width for <C>/</C> to work and{" "}
            <C>5 ÷ 2</C> to give <C>2.5</C>. Which of <C>Float32</C> / <C>Float64</C> goes in the blank?
          </>
        }
