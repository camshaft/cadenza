...
(@ (requires (>= x 0))
  ...
  (p "A contract's predicate is just a boolean expression in the def's scope, so it can say far more than " (c "&gt;= 0") ". It can relate two parameters (" (c "(requires (< lo hi))") "), compare the result to an input (" (c "(ensures (> ret x))") "), reach into a tuple or record, or even " (em "match") " a sum result. Here " (c "safe-dec") " returns an " (c "Option") ", and the postcondition matches it, promising that whenever there " (em "is") " a value it's non-negative:")
  (runnable
...
(@ (requires (>= x 0))
  ...
  (p "Called with " (c "5") " the result is " (c "(Some 4)") ", and the postcondition's " (c "Some") " arm checks " (c "4 &gt;= 0") ". Because the predicate is ordinary code, a contract is as expressive as any other boolean you could write, it just runs at the boundary instead of in the body.")
