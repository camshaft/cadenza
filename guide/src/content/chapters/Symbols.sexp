(chapter
  ...
  (p "That's what symbols are for: a value drawn from a small, known set of names. A traffic light is " (c "#\"red\"") ", " (c "#\"yellow\"") ", or " (c "#\"green\"") ", and a function can decide on it, here choosing how many seconds to wait:")
  (runnable
  ...
  (if (= light #"red") 30
    (if (= light #"yellow") 5
      0)))
(def (main) (wait #"red"))))
  (p (c "#\"red\"") " waits 30, " (c "#\"yellow\"") " 5, and anything else (green) 0. The light is passed around as a plain value and matched by name where the decision is made, with no numbers to remember and no strings to keep in sync.")
  ...
  (p "When a name arrives as text, whether parsed from input or assembled at run time, " (c "Symbol.of") " interns it into a symbol. The result is the very same value as writing the literal: a symbol built from the pieces " (c "\"ye\"") " and " (c "\"s\"") " equals " (c "#\"yes\"") ":")
  (runnable
  ...
  (exercise
    (id "symbols:1")
    (prompt (c "score") " dispatches on a medal from the fixed set " (c "#\"gold\"") " / " (c "#\"silver\"") " / " (c "#\"bronze\"") ": gold scores " (c "3") ", silver " (c "2") ", anything else " (c "1") ". The gold and fallback arms are done, so fill the middle comparison to make " (c "(score #\"silver\")") " give " (c "2") ".")
    (starter (def (score m)
  (if (= m #"gold") 3
    (if (= m ?) 2 1)))
...
(def (main) (score #"silver")))
    (expected "2")
    (hint "The middle arm handles silver, so compare " (c "m") " against " (c "#\"silver\"") ". Each symbol is checked by equality; " (c "#\"bronze\"") " matches neither and falls through to " (c "1") "."))
  (exercise
    (id "symbols:2")
    (prompt "A symbol is an ordinary value, so a function can " (em "return") " one, not just test it. " (c "next") " advances a traffic light around its cycle: red turns to green, green to yellow, yellow back to red. The green and yellow cases are written, so fill the hole with the symbol red becomes, making " (c "(next #\"red\")") " return " (c "#\"green\"") " and the check give " (c "true") ".")
...
(def (main) (= (next #"red") #"green")))
    (expected "true")
    (hint "The hole is the value the function " (em "hands back") " for red, a symbol literal, " (c "#\"green\"") ". The result is a symbol like any other, which the check then compares against " (c "#\"green\"") ".")))
