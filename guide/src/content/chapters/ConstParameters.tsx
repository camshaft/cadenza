...
export default function ConstParameters() {
  return (
    <article>
      ...
      <Lede>
        Some arguments you know before the program ever runs, like a size, a type, or a table of
        behaviour. What if the compiler could bake those in and charge you nothing at run time? That's a{" "}
        <C>const</C> parameter: an argument the compiler must know at compile time, so it is{" "}
        <em>inlined</em> into a specialized copy of the function and then erased, which means a different
        const value produces a different specialization and nothing about it survives to run time.
      </Lede>
      ...
      <P>
        ...
        argument, and the only rule is that it must be a closed, compile-time-known value such as a
        constant, a type, or a record of functions, never something computed from runtime data.
      </P>
      ...
      <P>
        ...
        factor into a specialized <C>scale</C>, giving one copy with <C>k = 3</C> and another with{" "}
        <C>k = 2</C>:
      </P>
      <Runnable
...
(def (main)
  (+ (scale 3 5) (scale 2 3)))`}
      ...
      <P>
        The const value doesn't have to be a scalar, since it can be a whole <em>record of functions</em>,
        a "dictionary" that tells the function how to behave. Here <C>fold-n</C> applies an operation{" "}
        <C>n</C> times, and the operation lives in a const dictionary <C>d</C>. Calling it with two
        different dictionaries, one that adds 10 and one that doubles, specializes each call with its
        operation inlined:
      </P>
      <Runnable
  ...
  (+ (fold-n #record((= op (fn (x) (+ x 10)))) 3 0)
     (fold-n #record((= op (fn (x) (* x 2)))) 3 1)))`}
      ...
      <P>
        ...
        from <C>1</C> to reach <C>8</C>, and together they make <C>38</C>. Because <C>d</C> is const, the{" "}
        <C>(. d op)</C> lookup folds to the concrete function in each specialized copy, so no record is
        passed at run time and no indirect call is emitted. You've hand-written the mechanism a typeclass
        or trait system would automate, handing the implementation to the function as a compile-time
        argument.
      </P>
      ...
      <P>
        ...
        type-value at compile time (types are ordinary values, as shown in <em>Types as values</em>), and
        the body can branch on it with <C>Type.eq</C>, folding to a constant per specialization. Here{" "}
        <C>is-int</C>{" "}
...
(def (main) (is-int Int64 5))`}
      ...
      <Note>
        A <C>const</C> parameter can carry either a type or a dictionary, and both are the same idea seen
        from two angles. Passing a <C>const</C> <em>type</em> (a <C>(const (: t Type))</C> parameter) lets
        the caller name the type the code specializes for, which is <em>types as values</em> made into an
        argument, while passing a <C>const</C> <em>dictionary</em> lets the caller name the behaviour the
        code runs. Both ride the same machinery, since a compile-time-known argument is inlined and
        specialized and then erased before the program runs.
      </Note>

      <Why tenet="A compile-time argument costs nothing at run time">
        A const parameter lets a function be <em>parameterized</em> by a size, a strategy, a type, or a
        ...
        code concrete. And the compiler enforces the honesty, because a const argument that isn't actually
        known at compile time is a clean error rather than a silent fallback to a runtime value.
      </Why>
...
(def (main) (scale 4 5))`}
      ...
      <Exercise
        ...
        prompt={
          <>
            The const dictionary decides the operation. Fill the body of <C>op</C> so that folding it three
