...
export default function IrrefutablePatterns() {
  return (
    <article>
      ...
      <Lede>
        Destructuring that always matches, so it can bind directly in a let or in a function's arguments,
        no match needed.
      </Lede>

      <P>
        In <strong>Pattern matching</strong> a <C>match</C> arm might or might not fire, so the compiler
        makes you cover every case. But some patterns can never fail: a tuple is always a tuple, and a
        type with a single constructor has only one shape. A pattern that always matches is{" "}
        <em>irrefutable</em>, and because it can't fail there's nothing to decide, so you can use it to bind
        a value directly, with no <C>match</C> at all.
      </P>

      <H2>Destructuring in a let</H2>
      <P>
        A <C>let</C> binder doesn't have to be a plain name. Give it a tuple pattern and it takes the pair
      ...
      <P>
        The same move works right in a parameter position. <C>add-pair</C> takes one tuple argument and
...
(def (main) (mag #record((= x 3) (= y 4))))`}
      ...
      <P>
        The moment a pattern <em>could</em> fail, it stops being usable as a binder. A sum with more than
        one variant is <em>refutable</em>: matching <C>Some</C> leaves <C>None</C> with nowhere to go. So
        the compiler refuses it in a binding position and tells you to use a <C>match</C> instead:
      </P>
      <Note>
        This one is <strong>meant to be refused</strong>. Run it and read the status bar: <C>CDZ0210</C>,
        "a multi-variant constructor pattern is refutable ... only in a <C>match</C> arm". That's the
        dividing line, the same exhaustiveness guarantee from <strong>Pattern matching</strong>, now
        deciding where a pattern is allowed to bind.
      </Note>
      <Runnable
...
(def (main) (unwrap (Some 5)))`}
      ...
      <P>
        ...
        and tail or unwrapping a state tuple as it threads through a loop. That's <em>Iteration without
        loops</em>, next.
      </P>
