...
(def (sum-to n acc)
  (if (= n 0)
    acc
    (sum-to (- n 1) (+ acc n))))"))
  (p "Read it as a loop turned inside out: " (c "acc") " is the running total, " (c "n") " counts down, the " (c "(= n 0)") " check is the exit condition, and each call adds " (c "n") " to " (c "acc") " and continues. When " (c "n") " reaches " (c "0") " the base case hands back the total, " (c "15") ". Nothing mutates; each call just receives the next pair of values.")
  (p "The same shape works over a list. Match the list by its structure, either the empty list " (c "#list()") " or a non-empty " (c "#list(x .. rest)") " that binds the first element to " (c "x") " and the remainder to " (c "rest") ", and thread the accumulator through:")
  (runnable
...
(def (sum-list xs acc)
  ...
  (p "The empty list is the base case (return the accumulator); the non-empty case adds the head to the accumulator and recurses on the tail. Toggle to the ML surface and the pattern reads as " (c "[x, .. rest]") ". Building a value instead of a number is the identical move. Here it reverses a list by taking each element off the front and putting it on the " (em "front") " of the accumulator, so the first element read ends up deepest and the last read ends up first:")
  (runnable
