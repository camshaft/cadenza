...
# NOTE: this launcher used to export TMUX so the bridge's shelled `cargo xtask fleet wake` would pass
# `wake_window`'s `in_tmux()` guard (the daemon runs outside tmux). That workaround is now REDUNDANT: the
# `fleet wake` subcommand takes `--session` (default `main`) and addresses the tmux server by name, which
# bypasses the `in_tmux()` gate at the source (v-fleet-tooling's durable fix). So the wake fires from a
# no-TMUX context on its own — no env plumbing needed here. Removed to keep the launcher simple.
