...
2. **Drain your inbox**, oldest-first — list it with `cargo xtask fleet inbox pr-sync` (resolves the
   ...
   **⚡ INTEGRATION = ONE COMMAND: `cargo xtask fleet schedule-pass --batch --execute`** (the CI-gated
   executor; replaces the old local-gate/gate-batch/bisect + manual publish loop — see
   `fleet/CI-GATED-LANES-DESIGN.md` for the model + rationale). One `schedule-pass --batch --execute` does
   BOTH halves of a scheduler pass, honoring the reply-invariant + single-writer + forward-only-trunk rules:
   - **BATCH-PR DISPATCH (operator throughput directive 2026-08-08/09):** the TOP-UP phase combines a
     file-DISJOINT subset of the queue (up to `--batch-cap`, default 6) into ONE candidate PR — re-parents
     each member onto origin/main in a scratch worktree, runs the local-nix `local-gate` PRE-FILTER on the
     combined tree (rejecting culprits by bisect BEFORE spending a GHA cycle), pushes ONE PR, and the reap
     acks EVERY member on the single merge. This amortizes one ~16-job GHA cycle over N lands instead of one
     tiny PR per MR (branch protection blocks direct FF-push, so batching is how throughput is won). It is
     SAFE by construction: a batch that fails the pre-filter or hits a setup/re-parent problem AUTOMATICALLY
     FALLS BACK to per-MR dispatch for the affected MRs (never strands one), and a batch is ONE in-flight
     slot gated on `in-flight < --cap` (member count is the independent `--batch-cap` bound). If you ever
     need the old one-candidate-per-MR behaviour (debugging a batch), drop `--batch` — plain
   ...
   **⟳ DRAIN-UNTIL-QUIESCENT within the tick (bounded).** ONE `schedule-pass --batch --execute` is a SINGLE
   ...
   NOT stop at one pass while there is more to do: **re-run `schedule-pass --batch --execute` again, in the
   same tick, whenever the previous pass MADE PROGRESS (reaped ≥1 or dispatched ≥1) AND actionable
