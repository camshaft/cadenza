# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value "s" rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 1
- **Signature:** `differential-artifact-wasm-wasm-value-s-rust-artifact-error-error-e-cannot-find`

## Reproducer

`differential-artifact-wasm-wasm-value-s-rust-artifact-error-error-e-cannot-find.smith.sexp` (also inline):

```scheme
(do (def (main) "s") (export main))
```

Reproduce in-process:

```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-s-rust-artifact-error-error-e-cannot-find.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value "s" rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
