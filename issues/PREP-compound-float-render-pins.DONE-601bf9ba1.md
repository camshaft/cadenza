...
## ALSO PENDING (found 2026-07-21, blocked on fresh binary rebuild):
NEW silent-omission gap from v-core-opt b96349590 (comparison reassociation fold):
- case "a complementary comparison pair nested past a third operand still folds via reassociation"
  (02-binding-and-control.sexp:4420) — in .sexp, absent from ALL 3 baselines. Expected (output (tuple 0 1)) x2 calls.
  MUST gate with a FRESH binary (the fold `reassociate_comparison_pair` is NEW in b96349590; my 04:28 stale
  binary predates it → would give a wrong verdict). Baseline at its gated verdict once binary rebuilt.
NEXT-TICK PLAN (once cdz binary is fresh, i.e. post build-thrash): gate + baseline BOTH
(1) the 4 compound/large-significand float pins AND (2) this reassociation case, in one MR.
