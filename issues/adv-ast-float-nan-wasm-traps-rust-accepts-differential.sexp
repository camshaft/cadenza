...
; UPDATE 2026-07-16: concierge RULED (A) uniform-decline (operator pinged for A-vs-B on Slack, A=default). v-runtime analyzed + re-routed the 2 loci PRECISELY (its wasm side is ALREADY correct — the trap IS reject-dont-miscompile): (1) v-metaprogramming reify guard = decline (Ast.Float <const non-finite>) at compile (matches +inf sibling; fixes CONSTANT case, both compile-decline); (2) v-rust-backend AST-encode = trap a non-canonical float to match wasm runtime trap (fixes RUNTIME case (Ast.Float (- x nan)), uniform-trap). Root of NaN-vs-+inf asymmetry: +inf declines at the OP guard (lower.rs:14072) but Float64.nan is a bare ConstFloatNan prim, dodges it, reaches escape. Both owners routed by v-runtime. If operator picks (B) accept → needs v-runtime (canonicalize NaN into encode). Watching both loci land; verify: case 1+control both compile-decline, case 2 both trap.

; UPDATE 2026-07-16 (split confirmed, no gap): CONSTANT half = v-metaprogramming MR 0dbd51e5 QUEUED (lower_sum_new rejects ConstFloatNan payload → both backends decline; 3 cases pinned NaN/+inf/finite-control). RUNTIME half (case 2 (Ast.Float (- x nan))) = v-rust-backend (already routed by v-runtime: "rust AST-encode must reject a non-canonical float" — make rust trap to match wasm, which ALREADY traps correctly). v-metaprogramming did NOT flip case 2 (correctly — left for v-rust-backend). Verify on land: case1+control DECLINE both backends (const); case2 both TRAP (runtime). No orphaned piece — both halves owned.

; UPDATE 2026-07-16 (breaker confirm 1848 + I verified): CONST half LANDED (28ff86659) — (Ast.Float NaN) declines uniformly both backends; -inf/+inf decline, -0.0 correctly ACCEPTED (canonical). RUNTIME half STILL live: (Ast.Float (- x nan)) wasm-traps/rust-value. Owner = v-rust-backend (has BOTH v-runtime route "rust AST-encode must reject non-canonical" AND v-metaprogramming residual note — firmly owned, no orphan). 🪤 case-1 + case-2 assertions are now STALE placeholders (assert (output Ast.Float NaN) but target is uniform (declines)) — the OWNER flips them to (declines) on promotion when the runtime half lands; NOT mine to flip prematurely (loose queue repro, not graded corpus). breaker also confirmed narrow-newtype a1510c912 fully thorough (Int8/nonzero-lit/Int16-max/Bool all pass) — validates my all-widths close.

; UPDATE 2026-07-16: BOTH halves now MR-sent, awaiting pr-sync (CI-gated batch, trunk flat 21m on external check "inprogress" — NOT a stall, pr-sync hb fresh + mid-turn polling checks). const-half = v-metaprogramming (landed 28ff86659). runtime-half = v-rust-backend MR cedac5b31: (Ast.Float (- x nan)) now TRAPS on rust matching wasm → case2 uniform-trap. ON LAND: flip reproducer case1+control→(declines) both, case2→(trap) both, promote to graded corpus. v-rust-backend offered its trap-message spelling if the gate needs it.
