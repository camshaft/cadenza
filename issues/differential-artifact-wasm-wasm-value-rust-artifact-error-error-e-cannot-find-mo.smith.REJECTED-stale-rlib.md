# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value -129 rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 21
- **Signature:** `differential-artifact-wasm-wasm-value-rust-artifact-error-error-e-cannot-find-mo`

## Reproducer

`differential-artifact-wasm-wasm-value-rust-artifact-error-error-e-cannot-find-mo.smith.sexp` (also inline):

```scheme
(do (def (main) -129) (export main))
```

Reproduce in-process:

```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-rust-artifact-error-error-e-cannot-find-mo.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value -129 rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
