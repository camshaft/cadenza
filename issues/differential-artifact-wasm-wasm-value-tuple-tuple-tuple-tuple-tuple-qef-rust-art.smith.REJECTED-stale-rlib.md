# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value (tuple 1089 (tuple 1089 (tuple 1089 (tuple 1089 (tuple "" "qef"))))) rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 1
- **Signature:** `differential-artifact-wasm-wasm-value-tuple-tuple-tuple-tuple-tuple-qef-rust-art`

## Reproducer

`differential-artifact-wasm-wasm-value-tuple-tuple-tuple-tuple-tuple-qef-rust-art.smith.sexp` (also inline):

```scheme
(do (def (main) (tuple 1089 )) (export main))
```

Reproduce in-process:

```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-tuple-tuple-tuple-tuple-tuple-qef-rust-art.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value (tuple 1089 (tuple 1089 (tuple 1089 (tuple 1089 (tuple "" "qef"))))) rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
