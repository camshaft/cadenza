...
; ---
; 5th CELL (breaker, 2026-07-18): MATCH-join of capturing lambdas -> INVALID WASM wasm[0]::function[2]
; (both selectors; worse than the if-join wrong-value face). Same root (init classification lifts join
; arms' closures; arms beta-inline poisoned). ONE fix covers if+match joins — extend the short-circuit to
; an IF or MATCH whose arms contain capturing lambdas. Grid: single✓ / compound✓(7fdb1dcb8) / if-join✗ /
; match-join✗ / call-return✓. Grade BOTH if-join + match-join in the @test + corpus migration.
