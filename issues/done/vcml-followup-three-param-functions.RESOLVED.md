...
## RESOLVED 2026-07-21 (v-compiler-ml): both remaining items LANDED
Both items in "STILL OPEN" are now DONE, landed in the param3/param4 slices:
  1. TYPED 3rd param `(: c T)` — `read-typed-third-param` (sread.cdz:446) added; a typed 3rd param parses +
     infer-param3 fit-checks it. LANDED.
  2. 4+-PARAM — `read-fourth-param` (untyped) + `read-typed-fourth-param` (sread.cdz:382/401) added; param4/arg4
     tables + resolve/infer/lower wiring all landed (MR e24228335 "WIRE param4/arg4 end-to-end"). Corpus 4-arg
     helpers (sl b s n, go b i a, skip-elems b i k) now RUN. LANDED.
Chose param4/arg4 tables (not the List-based N-ary rep) — the table sprawl is bounded at 4 (corpus max useful
arity is 4-5; 5-param declines cleanly). The List-based rep remains a possible future cleanup but is NOT
blocking. Nothing open here. (The higher-value forward-ref/recursion work is tracked separately in
vcml-followup-recursive-user-functions-decline + the design doc.)
