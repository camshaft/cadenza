# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value (tuple (tuple)) rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 1
- **Signature:** `differential-artifact-wasm-wasm-value-tuple-tuple-rust-artifact-error-error-e-ca`

## Reproducer

`differential-artifact-wasm-wasm-value-tuple-tuple-rust-artifact-error-error-e-ca.smith.sexp` (also inline):
...
```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-tuple-tuple-rust-artifact-error-error-e-ca.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value (tuple (tuple)) rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
