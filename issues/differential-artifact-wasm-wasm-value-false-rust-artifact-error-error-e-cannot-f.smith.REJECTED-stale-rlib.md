# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value false rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 3
- **Signature:** `differential-artifact-wasm-wasm-value-false-rust-artifact-error-error-e-cannot-f`

## Reproducer

`differential-artifact-wasm-wasm-value-false-rust-artifact-error-error-e-cannot-f.smith.sexp` (also inline):

```scheme
(do (def (main) false) (export main))
```

Reproduce in-process:

```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-false-rust-artifact-error-error-e-cannot-f.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value false rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
