# SMITH FINDING — backends DISAGREE on value: [artifact] wasm=wasm value true rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
...
- **Hits:** 2
- **Signature:** `differential-artifact-wasm-wasm-value-true-rust-artifact-error-error-e-cannot-fi`

## Reproducer

`differential-artifact-wasm-wasm-value-true-rust-artifact-error-error-e-cannot-fi.smith.sexp` (also inline):

```scheme
(do (def (main) true) (export main))
```

Reproduce in-process:

```
cargo run -p cdz-smith -- verify differential-artifact-wasm-wasm-value-true-rust-artifact-error-error-e-cannot-fi.smith.sexp
```
...
- **Disagreement:** [artifact] wasm=wasm value true rust=artifact-error error[E0433]: cannot find module or crate `cdz_num` in this scope
