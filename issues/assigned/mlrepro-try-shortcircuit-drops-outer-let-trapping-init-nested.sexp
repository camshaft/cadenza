...
; UPDATE 2026-07-16 (t7) — RESOLUTION FLIPPED (concierge note 1432, operator ruling §283 #622465): correct outcome = ELIDE→None + CDZ0305 WARNING, *NOT* CDZ0304 reject. concierge RETRACTED assign #1354 (contradicted the standing ruling). An unreachable/unobserved trap is a WARNING not a trap; ALL 3 shapes (A same-let / B if-false-DCE / C nested-let) consistent-ELIDE. The same-let :146 guard I cited as gold is ALSO reverting to elide (over-strict, NOT the template). v-try-operator shipping the revert (elide both fast-paths, KEEP host-call guard) + corpus CDZ0304→None+CDZ0305 with fresh .gate-baseline.
; NEW CLOSE-TARGET (verify-by-content): nested-let COMPILES → outputs None + CDZ0305 present (NOT CDZ0304). ⚠ NOTE current trunk a07a188bc ALREADY emits CDZ0305 (2 warnings) + compiles — close-target nearly met; awaiting v-try-operator revert MR + corpus baseline flip, then verify + close. (My earlier grep|head landing-check false-positived; fn enclosing_let_inits_discardable is NOT in source — MR still pending.)
