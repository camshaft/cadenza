...
<!-- NO-MISCOMPILE 2026-07-16 (v-runtime): the INVALID-WASM face of the runtime sub-face is ELIMINATED — it now DECLINES cleanly (reject-don't-miscompile), no more un-instantiable component from a check-clean program. ROOT CAUSE found: `lower_sum_new`'s NEWTYPE-erasure arm ran BEFORE the partial-arity guard, so a single-variant sum (a NEWTYPE, e.g. `(type T (Mk Int64 Int64))`) applied SHORT of arity erased `(T.Mk 10)` to the bare payload `10` (dropping the arity check) → the tuple stored a raw i64 where a closure handle belonged → project+apply `call_indirect`'d it → `func N: expected i32, found i64`. FIX: moved the partial-arity DECLINE guard ahead of newtype-erasure + the nullary arm, so a partial ctor of ANY sum declines. Graded `todo` case added (05-compound-types: "a partial constructor in a runtime tuple element declines cleanly (was invalid wasm)"), intended value 15 once the eta-lift lands. STILL OPEN (now a clean todo, NOT a miscompile): the genuine runtime eta-closure lift that would make it PASS. Gate 2995/4/0, --check green, hash-neutral. -->
