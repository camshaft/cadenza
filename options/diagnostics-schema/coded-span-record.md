...
| `CDZ0213` | *(warning)* a match arm that can never be reached because an earlier arm already covers every value it would match — a duplicate variant/literal arm, or any arm after a catch-all binder/wildcard. The dual of `CDZ0210` (non-exhaustiveness): where that flags a value no arm covers, this flags an arm no value reaches. Not a rejection — the program is well-formed (first-match wins, so the shadowed arm is dead), but a redundant arm is almost always a defect | `spec/capabilities/core-semantics.md#matching-is-exhaustive-or-rejected` |
...
| `CDZ0502` | a named unit declared more than once with conflicting conversions — a differing reference dimension or a differing scale — so its name would not resolve to a single well-defined conversion (a redeclaration that agrees is admissible) | `spec/capabilities/units-of-measure.md#a-named-units-conversion-is-unique` |
