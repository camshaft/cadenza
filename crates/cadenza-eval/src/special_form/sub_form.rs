//! The `-` special form for subtraction.
...
/// Returns the `-` special form for subtraction.
///
/// # Evaluation
/// - Takes exactly 2 arguments
/// - Evaluates both arguments
/// - Subtracts them together (integers, floats, or quantities with compatible dimensions)
///
/// # IR Generation
/// - Generates IR for both operands
/// - Emits a binary sub instruction
///
/// # Examples
/// ```cadenza
/// 1 - 2        # returns 3
...
fn eval_sub(args: &[Expr], ctx: &mut EvalContext<'_>) -> Result<Value> {
    if args.len() != 2 {
    ...
    match (&lhs, &rhs) {
        ...
        (
            ...
            if u1.dimension != u2.dimension {
                return Err(Diagnostic::syntax(format!(
                    "cannot sub quantities with incompatible dimensions: {} and {}",
                    u1.name, u2.name
                )));
            }
        ...
        // Allow subing plain numbers to quantities
        (
            Value::Quantity {
                value,
                unit,
...
fn ir_sub(
    ...
    // Emit binary sub instruction
    let result = block.binop(BinOp::Sub, lhs, rhs, ty.clone(), source);
