//! The `<=` special form for less-than-or-equal comparison.
...
/// Returns the `<=` special form for less-than-or-equal comparison.
...
fn eval_le(args: &[Expr], ctx: &mut EvalContext<'_>) -> Result<Value> {
    if args.len() != 2 {
...
fn ir_le(
    ...
    let result = block.binop(BinOp::Le, lhs, rhs, Type::Bool, source);
