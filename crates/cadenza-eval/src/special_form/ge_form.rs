//! The `>=` special form for greater-than-or-equal comparison.
...
/// Returns the `>=` special form for greater-than-or-equal comparison.
...
fn eval_ge(args: &[Expr], ctx: &mut EvalContext<'_>) -> Result<Value> {
    if args.len() != 2 {
...
fn ir_ge(
    ...
    let result = block.binop(BinOp::Ge, lhs, rhs, Type::Bool, source);
