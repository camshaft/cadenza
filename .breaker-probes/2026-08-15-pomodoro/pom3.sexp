(case "pom3 the merged pomodoro with the guard CASCADE reordered high-to-low — under a hundred counts the session, a hundred-plus counts the break remaining, the same protocol answers survive the encoding, and the shorter session completes two pomodoros where the longer completes one"
  (input  (do
            (effect P
              (op work (-> Int64))
              (op rest (-> Int64)))
