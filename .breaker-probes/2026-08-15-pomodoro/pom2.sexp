(case "pom2 the pomodoro with work-and-break MERGED into one phase field — under a hundred counts the session, a hundred-plus counts the break remaining, the same protocol answers survive the encoding, and the shorter session completes two pomodoros where the longer completes one"
  (input  (do
            (effect P
              (op work (-> Int64))
              (op rest (-> Int64)))
