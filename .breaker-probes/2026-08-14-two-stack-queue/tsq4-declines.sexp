(case "tsq4 a TWO-STACK amortized FIFO queue — enq pushes the in-stack, deq pops the out-stack and reverse-refills from the in-stack only when empty, so an element enqueued between refills never overtakes one already staged"
  (input  (do
            (effect Q
              (op enq (-> Int64 Int64))
              (op deq (-> Int64)))
                  ...
                  (let ((b (Q.enq (+ n 2))))
                    (let ((c (Q.deq)))
                      (let ((d (Q.deq)))
                        (+ (* 100 (+ (* 100 (+ (* 100 a) b)) c)) d)))))))
            (export main)))
  (call   main (: 10 Int64)) (output (: 11121112 Int64))
  (call   main (: 0 Int64)) (output (: 1020102 Int64)))
