(case "sftC shrink — push AND popmin arms both present, one popmin dispatch (siftdn with computed-index double List.update + smallest scan)"
  (input  (do
            ...
            (def (main (: n Int64))
              (handle H (: (list 9 n 7) (List Int64))
                ((push (v) st
                  (match st (h0 (resume (getat h0 0) (List.push h0 v)))))
                 (popmin () st
                  (if (= (List.len st) 0)
                      (resume -99 st)
                      (if (= (List.len st) 1)
