(case "sftD the sft1 arms at TWO dispatches (push then popmin) — push appends then sifts UP by parent swaps answering the new root, popmin moves the last element to the root and sifts DOWN answering the old root, and the seeded pushes force different swap paths on the two runs"
  (input  (do
            (effect H
              (op push (-> Int64 Int64))
              (op popmin (-> Int64)))
              ...
              (handle H (: (list) (List Int64))
                ...
                (let ((a (H.push (+ n 2))))
                  (let ((d (H.popmin)))
                    (+ (* 100 a) d)))))
            (export main)))
  (call   main (: 10 Int64)) (output (: 1212 Int64))
