(case "sftE three dispatches (push push popmin) — push appends then sifts UP by parent swaps answering the new root, popmin moves the last element to the root and sifts DOWN answering the old root, and the seeded pushes force different swap paths on the two runs"
  (input  (do
            (effect H
              (op push (-> Int64 Int64))
              (op popmin (-> Int64)))
                ...
                (let ((a (H.push (+ n 2))))
                  (let ((b (H.push 4)))
                    (let ((d (H.popmin)))
                      (+ (* 100 (+ (* 100 a) b)) d))))))
            (export main)))
  (call   main (: 10 Int64)) (output (: 120404 Int64))
  (call   main (: 0 Int64)) (output (: 20202 Int64)))
