(case "sftH six dispatches confirm (same as sft1) — push appends then sifts UP by parent swaps answering the new root, popmin moves the last element to the root and sifts DOWN answering the old root, and the seeded pushes force different swap paths on the two runs"
  (input  (do
            (effect H
              (op push (-> Int64 Int64))
              (op popmin (-> Int64)))
