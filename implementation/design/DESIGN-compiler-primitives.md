...
- No `Hash.of` in the guest prelude and no `Contract.id` intrinsic (`rcdzc/src/prelude.rs` enumerates the
...
```
import ast decl from "temp-celsius.cdz"          -- (1) decl : Ast — the declaration file's whole AST
let bytes = Ast.encode (canonicalize-contract decl)  -- (2) transform + encode, folded to a compile-time Bytes constant
let temp-celsius-id = Hash.of bytes              -- (3) blake3 → the contract-id, a compile-time constant
...
- At runtime, the SAME `Hash.of` op lets a program hash a declaration it *receives* (e.g. an adapter that
...
`link.rs:542` resolves each name to the sibling module's export). The new surface binds the imported
module's **canonical binary AST** as a single compile-time `Ast` value:

```
import ast decl from "temp-celsius.cdz"    -- decl : Ast, the whole module document as a compile-time value
```

- **Node form (chosen default, D1):** extend the existing import clause with an `ast` binder —
  `(import "path" (ast decl))` alongside the existing `(import "path" (name…))`. It reuses the whole link
  path: `link.rs` already loads `"path"` as a `cadenza-ast` document; instead of (or in addition to)
  splicing its items, it binds `decl` to that document reified as an `Ast` value — the same `Ast` a `quote`
  of the module body would produce. No new resolver, no runtime cost: `decl` is a compile-time constant.
- **`decl` is compile-time-visible**, which is exactly what primitive 2 needs. It is an ordinary `Ast`
  value: `Ast.List`/`Ast.Name`/… — walkable by userspace `Ast` operations, encodable by `Ast.encode`.
...
- **Erasure:** like `Type.of`, an `Ast` value that is only consumed at compile time (fed to a transform +
  `Ast.encode` + `Hash.of` that all fold) leaves no runtime residue. An `Ast` value CAN also cross the
...
- **Guarantee:** when every input to a pure, total transform is compile-time-visible (an imported `Ast`
  ...
  operations, **recursion over the tree**, and the terminal `Ast.encode`/`Hash.of` — to a constant value,
...
- **Gap A — the missing constant kind (`ConstBytes`), chosen default D3:** today there is **no
  ...
  time, exactly as `blake3::hash` will for `Hash.of`). The backend lowers a `ConstBytes` into the program's
...
- **Guest prelude surface (chosen default, D5): `Hash.of : Bytes → Bytes`** (a 32-byte blake3 digest as a
  `Bytes` value), a `Hash` module in `rcdzc/src/prelude.rs` mirroring the `Type`/`Qty` module pattern
  (`Prim::HashOf` + `from_name "hash-of"`). Result is a plain 32-byte `Bytes` — NOT a distinct nominal
  `Hash` type — so it composes with the existing opaque-`Bytes` `msg.contract` (`world.wit` `type
  contract-id = list<u8>`) by ordinary `Bytes.eq`, no wrapping/unwrapping. (D5 alt: a nominal `Hash` type
  — rejected as default because it forces adapters at every `msg.contract` comparison.)
- **Compile-time fold:** `Hash.of` over a compile-time-visible `Bytes` (the output of 3b) **const-folds**
...
- **Runtime op (chosen default, D6): an APPENDED value-heap runtime op** `hash-blake3(bytes: u32) -> u32`
  ...
  codegen`, the sanctioned cost). `Hash.of` lowers to `Core::HashOf` → this op at runtime, and const-folds
...
- **Byte-identity is the load-bearing invariant:** the compile-time fold and the runtime op MUST produce
  ...
  asserts equality.
- **Domain separation:** the current `Hash::of(bytes)` takes raw bytes (no tag). A contract-id per
  `platform.md §1` is `hash(<the contract declaration's canonical bytes>)`. The declaration's canonical
  `cadenza-ast` form is already structurally distinct from a raw payload (it is a `(contract …)` document),
  so **no separate `HashTag` byte is required by this design** — domain separation is carried by the
  encoded declaration's shape, which userspace controls. (If a future need arises to distinguish two
  hashes of the same bytes, userspace prepends a tag before `Hash.of` — still no compiler change.)
...
1. `import ast decl from "temp-celsius.cdz"` → `decl : Ast` (primitive 1).
...
4. `Hash.of` those bytes → the 32-byte contract-id constant (primitive 3, compile-time fold).
...
**P3b — `Hash.of` guest prelude + runtime lowering (`v-inference` + `v-rust-backend`/`v-compiler-ml`).**
Add the `Hash` module (`Prim::HashOf`, `from_name "hash-of"`) to `rcdzc/src/prelude.rs`; infer arm
`(Hash.of e) : Bytes`; lower `Core::HashOf` → runtime op 91 (wasm) + the rust backend arm
(new-`Core`-variant rule). Gate: a corpus case where a runtime `Bytes` routes through op 91 and executes
under wasmtime to `blake3::hash` of the input. (The COMPILE-TIME `Hash.of` fold is deferred to P2, where
...
literal), and a const-fold arm for `Ast.encode`/`Value.encode`/`Hash.of` over a compile-time-visible input.
Gate: a corpus case where `Ast.encode` of a literal `Ast` value folds to its golden canonical bytes AT
COMPILE TIME (no runtime `value-encode`), executing byte-identical; a `Hash.of` of a literal `Bytes` folds
to the golden blake3 digest at compile time. This is the shared foundation P2's transform output and P3b's
compile-time `Hash.of` both need; it is independent of P1 and can land alongside P3.
...
input) and P0 (the `ConstBytes` output), and its terminal `Hash.of` folds via P0/P3b.
...
P0 introduces `ConstBytes`; P2 depends on P1 + P0; the compile-time `Hash.of` fold lands in P0; P4 depends
...
- **D1 — reflection surface: `(import "path" (ast name))` clause vs a standalone `(reflect "path")` /
  `Ast.of-module` prim.** Default: extend the existing import clause with an `ast` binder — it reuses the
  whole `link.rs` module-load path (the AST is already loaded there) and reads naturally alongside the
  name-import form. Alt (a standalone reflection prim) is more general but needs its own path-resolution;
  rejected unless a use case wants reflection outside an import.
...
- **D5 — `Hash.of` result type: plain 32-byte `Bytes` vs a nominal `Hash` type.** Default: plain `Bytes`,
...
- **D7 — domain-separation tag.** Default: none in the compiler — the contract declaration's canonical
  `cadenza-ast` shape is self-distinguishing, and any tag a userspace scheme wants is prepended in
  userspace before `Hash.of`. Recorded so the P4 implementer knows the id is `blake3(declaration-bytes)`
  with no hidden tag byte. (The old nuked platform's `HashTag::Contract` is NOT reintroduced — the new
  `cdz-platform` `Hash::of` takes raw bytes.)
...
- P3b: a `Hash.of` of a runtime `Bytes` executes through op 91 under wasmtime to `blake3::hash` of the
  input; both wasm and rust backends.
- P0: `Ast.encode`/`Hash.of` of a literal input folds to the golden bytes / blake3 digest AT COMPILE TIME
...
- P4: a conformance run (v-platform-itest harness + Checker) drives true distinct-contract dispatch through
  the platform; the interim payload sentinel is retired.
...
## 9. Byte-identity gate — the one invariant worth stating twice

The single property that makes "contract-hash callable at compile time AND at runtime, same hash both
places" true is: **`Hash.of` const-folded at compile time == `hash-blake3` op executed at runtime, for the
same input bytes.** The gate MUST assert this directly (hash a fixture at compile time via the fold and at
runtime via op 91; assert the two 32-byte outputs are equal), not merely test each half in isolation. If
this ever diverges, a guest's compiled-in contract-id would not match a runtime-hashed declaration, and
routing (exact hash equality) silently fails to match. It is the load-bearing correctness property of the
whole design.
...
- **Userspace contract-id library + held reducer (P4):** `v-platform` (owns contracts/codegen/world +
  the `.cdz` library) then `v-platform-itest` (the conformance run).
- The compiler-side owner who will take the codegen/emit slice is **v-platform** once the design settles —
  flag them when P1–P3 shapes are agreed.
