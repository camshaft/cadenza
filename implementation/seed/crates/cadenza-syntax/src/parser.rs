...
mod tests {
    ...
    // `semicolon_sequences_a_function_body` (a `;`-separated body folds to a flat `(do …)`, greedily
    // collecting its run and STOPPING at the next top-level `def`) + `top_level_forms_juxtapose_without_
    // semicolons` (top-level forms are whitespace-separated, no `;` — `def a = 1 def b = 2` is two roots)
    // MIGRATED to the spec/syntax corpus (inc-6 batch-72): ml/431-semicolon-body-stops-at-next-def
    // `def f() = a; b; c`⏎`def g() = 2`→`(do (def (f) (do a b c)) (def (g) 2))` (body stops at `def g`);
    // the def-juxtaposition `def a = 1 def b = 2`→`(do (def a 1) (def b 2))` is subsumed by ml/203-top-level-
    // value-defs-blank-separated. (`top_level_semicolon_folds_and_flattens` stays Rust below — see the GAP note.)

    // STAYS RUST pending a reader GAP (corpus-policy: never pin a compiler gap): `f(); g()` (with `;`) →
    // `(do (f) (g))` correctly (pinned at ml/432-semicolon-top-level-folds), but `f() g()` (juxtaposition,
    // no `;`) parses to `(do ((. Qty of) (f) ((. Unit of) #"g")) unit)` — the quantity sugar wrongly eats the
    // call-result `f()` as a magnitude + `g` as a unit. So the "`;` is optional / identical tree" invariant
    // does NOT hold; this test only passes because it checks `len == 2` (the quantity `do` is also len 2).
    // Spec-correct: `f() g()` should be `(do (f) (g))` (quantity magnitude should require a number literal).
    // Flagged v-syntax + concierge (inc-6 batch-72). Once fixed, migrate `f() g()`→`(do (f) (g))` + retire this.
    #[test]
    ...
    // `semicolon_in_argument_position_needs_parens` (a call argument is a single expression, so a `;` inside
    // must parenthesize: `f((a; b))` is a one-arg call whose argument is the sequence `(do a b)`) +
    // `if_branch_does_not_swallow_the_trailing_sequence` (`if`'s branches parse at `PREC_SEQ + 1`, so a `;`
    // after the `if` belongs to the ENCLOSING sequence) MIGRATED to the spec/syntax corpus (inc-6 batch-72):
