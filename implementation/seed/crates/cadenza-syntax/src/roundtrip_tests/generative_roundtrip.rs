...
/// Assert the DEFAULT ML parser (`read_ml`) and the FROZEN RECURSIVE reference (`read_ml_recursive`)
/// produce BYTE-IDENTICAL results for `ml`: structurally-equal arenas, an identical span table (the
/// guide/LSP depend on it and nothing else pins it byte-for-byte), and identical errors.
fn assert_parsers_agree(label: &str, ml: &str) {
    let new = parser::read_ml(ml);
    let recursive = parser::read_ml_recursive(ml);
    assert!(
        new.arenas.structurally_eq(&recursive.arenas),
        "{label}: parser arena diverged from the recursive reference\n  ml: {ml}\n  new: {}\n  ref: {}",
        sexpr::print(&new.arenas),
        sexpr::print(&recursive.arenas),
    );
    assert_eq!(
        new.spans.len(),
        recursive.spans.len(),
        "{label}: span-table length diverged\n  ml: {ml}"
    );
    for i in 0..recursive.spans.len() as u32 {
        let id = crate::ast::StructId(i);
        assert_eq!(
            new.spans.get(id),
            recursive.spans.get(id),
            "{label}: span[{i}] diverged\n  ml: {ml}"
        );
    }
    assert_eq!(
        new.errors.len(),
        recursive.errors.len(),
        "{label}: error count diverged\n  ml: {ml}\n  new: {:?}\n  ref: {:?}",
        new.errors,
        recursive.errors,
    );
    for (a, b) in new.errors.iter().zip(recursive.errors.iter()) {
        assert_eq!(
            a.message, b.message,
...
/// The DIFFERENTIAL ORACLE for the ML parser's recursive→iterative rewrite (v-syntax-nonrec-reader I2):
/// `read_ml` (the default, iterative once the rewrite lands) MUST agree byte-for-byte with the frozen
/// recursive reference `read_ml_recursive` over a broad program space — arenas + span table + errors.
/// TODAY both call the recursive parser, so this is a GREEN PASSTHROUGH that validates the diff machinery
/// and the sweep. When a later increment makes `read_ml` iterative (adding iterative methods ALONGSIDE the
/// recursive ones, `read_ml_recursive` staying on the recursive path), this becomes the live net that
/// catches ANY divergence — a mis-fired `carry_docs`, a shifted comment slot, a precedence/associativity
/// edge — localized to the offending input, BEFORE it reaches the corpus as a regression.
#[test]
fn differential_oracle_read_ml_agrees_with_the_recursive_reference() {
    // (1) Broad generated sweep — the same generator + s-expr→ML-print derivation as
    // `ml_surface_round_trips_generated_programs`, but DUAL-parsed (read_ml vs read_ml_recursive).
    let seeds: [u64; 4] = [
        0x0bad_c0de_dead_beef,
        0x5eed_1234_5678_9abc,
        0xfeed_face_cafe_babe,
    ...
    for &seed in &seeds {
        ...
        for _ in 0..1000 {
            ...
            let src = gen_expr(&mut rng, depth);
            let program = format!("(def (main) {src})");
            let oracle = sexpr::read(&program).expect("generator produces valid s-expr");
            let ml = printer::print(&oracle, WIDTH);
            assert_parsers_agree("generated", &ml);
            total += 1;
        }
    }
    assert!(
        total >= 4000,
        "swept a meaningful generated space, got {total}"
    );

    // (2) Targeted TRICKY ARMS the I3 expr-cycle rewrite must get exactly right — each authored as an
    // s-expr and ML-printed so it is GUARANTEED-VALID ML that exercises the arm. `carry_docs` order (an
    // `@`/`@!` form carrying a leading doc) is the #1 correctness risk — the class that regressed a prior
    // comment slice — so it leads.
    let tricky_sexprs = [
        // @-annotated def WITH a leading doc — carry_docs BEFORE/AFTER the inner prefix.
        "(comment \"the identity\" (@ inline-never (def (f x) x)))",
        // stacked annotations + a doc.
        "(comment \"doc\" (@ a (@ b (def (g y) y))))",
        // @! pragma stacking.
        "(@! k1 (@! k2 (def (h z) z)))",
        // right-associative arrow chain A -> B -> C -> D (type position).
        "(def (f (: g (-> A (-> B (-> C D))))) g)",
        // double / triple prefix unary-minus run.
        "(def (f x) (- (- x)))",
        "(def (f x) (- (- (- x))))",
        // nested keyword forms: if-in-if, match, stacked let, nested fn.
        "(def (f a b c) (if a (if b 1 2) 3))",
        "(def (f x) (match x ((0) 1) ((_) 2)))",
        "(def (f x) (let ((y (+ x 1))) (let ((z (* y 2))) z)))",
        "(def (f) ((fn (a) ((fn (b) (+ a b)) 2)) 1))",
        // long left-associative infix run (the spine path) + a postfix member chain.
        "(def (f a b c d e) (+ a (+ b (+ c (+ d e)))))",
        "(def (f x) (. (. (. x a) b) c))",
    ];
    for s in tricky_sexprs {
        let oracle = sexpr::read(s).expect("tricky s-expr is valid");
        let ml = printer::print(&oracle, WIDTH);
        assert_parsers_agree("tricky", &ml);
    }
}
