...
/// - **Store layout:** `<blake3hex>.wasm` per component + a `runtime.toml` manifest at the store root.
...
/// - **`REQUIRED_RUNTIME_HASH`** (`rcdzc::backend::wasm::runtime_abi`) IS this BLAKE3 of the built
...
/// - **UNIFIED HASH (operator ruling 2026-08-08):** content addressing is unified tree-wide on BLAKE3
///   (chosen for speed), the same digest as `cdz-kernel::hash::Hash::of` — so a `+<hash>` dep import, a
///   blob-store key, a `CDZ_STORE` `<hash>.wasm` name, and `REQUIRED_RUNTIME_HASH` are ONE interchangeable
///   address space. This SUPERSEDES the earlier ruling (A) (2026-08-05, which had declined a store-wide
///   BLAKE3 move and pinned SHA-256); the operator explicitly reversed it. Kept identical to `Hash::of`
///   (`blake3::hash(bytes)` lowercase hex) so this producer and the kernel verifier never diverge.
