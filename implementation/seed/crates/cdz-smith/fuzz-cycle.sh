...
if [ "$DIFF_COUNT" -gt 0 ]; then
  ...
  if [ -z "$DIFF_CDZ" ] || [ ! -x "$DIFF_CDZ" ]; then
    ...
    if ( cd "$CRATE_DIR" && cargo build -q --release --features differential 2>/dev/null ); then
      ...
      # generate_export_param has 26 shapes (6 scalar + 3 List-entry-borrow + const-sum-field E0282 family
      ...
      # byte-leaf + lpt1 list<tuple> + eot1 option<tuple> + lpr1 list<record> + eor1 option<record> compound
      # list-element/sum-payload boundary-marshal paths). NOTE (measured, corrects the earlier "split imminent"
