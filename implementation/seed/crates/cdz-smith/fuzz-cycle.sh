...
if [ "$DIFF_COUNT" -gt 0 ]; then
  ...
  if [ -z "$DIFF_CDZ" ] || [ ! -x "$DIFF_CDZ" ]; then
    ...
    if ( cd "$CRATE_DIR" && cargo build -q --release --features differential 2>/dev/null ); then
      ...
      # generate_export_param has 32 shapes (6 numeric scalar + chr1 Char + big1 BigInt scalars + ssa1 String.scalar-at + 3 List-entry-borrow + const-sum-field E0282 family
