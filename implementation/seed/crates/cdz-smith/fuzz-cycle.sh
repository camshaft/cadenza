...
if [ "$DIFF_COUNT" -gt 0 ]; then
  ...
  if [ -z "$DIFF_CDZ" ] || [ ! -x "$DIFF_CDZ" ]; then
    ...
    if ( cd "$CRATE_DIR" && cargo build -q --release --features differential 2>/dev/null ); then
      ...
      # generate_export_param has 28 shapes (6 scalar + 3 List-entry-borrow + const-sum-field E0282 family
      ...
      # list<list> nested-heap + rrf1 record-with-record-field nested-product — compound/nested list-element/
      # sum-payload/recursive boundary-marshal paths). NOTE (measured, corrects the earlier "split imminent"
