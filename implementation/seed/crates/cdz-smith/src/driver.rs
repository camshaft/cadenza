...
pub enum GenMode {
    ...
    /// programs (single-handler, nested-handler, effect+collection, multi-op scalar-state fold, a
    /// heap-Map handler-state threaded through resume — the #9522/#9525 handler-state-threading reclaim
    /// family — and a control-flow-join nested-tuple resume new-state — the #9532→#9533 SumPayload-reclaim-
    /// off-a-join-scrutinee invalid-wasm miscompile) that each return a KNOWN Int64. PURE-GUEST (effects lower to guest continuation code, no host boundary), so unlike
