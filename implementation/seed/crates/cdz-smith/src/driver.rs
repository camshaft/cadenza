...
pub enum GenMode {
    ...
    /// programs (single-handler, nested-handler, effect+collection, multi-op scalar-state fold, and a
    /// heap-Map handler-state threaded through resume — the #9522/#9525 handler-state-threading reclaim
    /// family) that each return a KNOWN Int64. PURE-GUEST (effects lower to guest continuation code, no host boundary), so unlike
