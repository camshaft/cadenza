...
/// list corrupts the returned element. Shape 13 is the #9694 ep1 String analog of shape 12: two `String` entry
...
pub fn generate_export_param(entropy: &[u8]) -> ExportParam {
    ...
    // A result<Int64,String> TWO-PAYLOAD sum entry-param value-form for the #9747 rpp21/22 result-field
    // WIT/rebuild path (shape 20). Alternates the variants by e1 parity (decorrelated from shape 19's e2):
    // `(Ok {a})` carries an Int64 payload (a may be NEGATIVE — sign-marshal INSIDE a sum cell) vs `(Err "…")`
    // a String BYTE-LEAF payload in the OTHER discriminant. Unlike shape 19's one-payload option, BOTH arms
    // carry a payload of a DIFFERENT type — the two-payload sum-cell layout #9747 fixed the WIT/rebuild
    // disagreement for. Rendered bare-variant `(Ok …)`/`(Err "…")` (the `(Result.Ok …)`/`#Ok(…)` forms error on rust).
    let result_arg = if e1 % 2 == 0 {
        format!("(Ok {a})")
    } else {
        format!("(Err \"{s0}\")")
    };
    ...
    // An option<tuple<Int64,Int64>> value-form for the eot1 SUM-HOLDING-A-COMPOUND shape (23). Alternates by e2
    // parity: `(Some #tuple({e0} {e1}))` — a sum cell whose PAYLOAD is a TUPLE (not a scalar/byte-leaf) — vs the
    // payload-less `None`. Distinct from shapes 19/20/21 (sum payloads are byte-leaves/scalars) AND from shape 22
    // (compound as a LIST element, not a SUM payload): this crosses a compound cell NESTED inside a sum
    // discriminant cell. A mis-built payload tuple or mis-read discriminant corrupts the field-sum. Value = e0+e1
    // (Some) or 0 (None).
    let opt_tuple_arg = if e2 % 2 == 0 {
        format!("(Some #tuple({e0} {e1}))")
    } else {
        "None".to_string()
    };
    // A list<record<x,y>> value-form for the lpr1 RECORD-list-element shape (24): a two-element list whose
    // elements are RECORDS with NAMED fields (not tuples). The record sibling of shape 22's list<tuple> — a
    // RECORD cell uses a distinct assembler path (named-field resolution/layout) from a positional tuple cell,
    // so it fences the record-assembler-in-a-list-element context; a mis-built element-0 record (wrong field
    ...
    let (source, args) = match shape {
        ...
        // 9 — #9586 CDZ0910 arm-2: a FIVE-field record `Msg` with `(: on (Option Node))` over the scalar
        ...
        // 13 — #9694 ep1 String-consume: two String entry params CONSUMED by String.concat (BytesConcat
        ...
        // 20 — #9747 rpp21/22 result<Int64,String> TWO-PAYLOAD SUM ENTRY PARAM: a `(Result Int64 String)` param
        //      matched to return the Ok Int64 payload directly, or the Err String's byte-len. Distinct from shape
        //      19's ONE-payload option: BOTH discriminant arms carry a payload, of DIFFERENT types (Int64 vs
        //      String) — the exact two-payload sum-cell layout #9747 fixed the result-field WIT/rebuild
        //      disagreement for, now exercised as an ENTRY param crossing the boundary. The Ok payload may be
        ...
        // 23 — eot1 option<tuple<Int64,Int64>> SUM-HOLDING-A-COMPOUND entry param: a `(Option #tuple(Int64 Int64))`
        //      param matched to sum the Some-payload tuple's two fields (or 0 for None). Distinct from shapes
        //      19/20/21 (sum payloads are byte-leaves/scalars) AND shape 22 (compound as a LIST element): here the
        //      compound (tuple cell) is NESTED INSIDE a sum discriminant cell — the sum-cell-holding-a-compound
        //      marshal. A mis-built payload tuple (wrong field offsets / cell nesting) or mis-read discriminant
        //      corrupts the field-sum. Result is a scalar. Arg alternates `(Some #tuple(…))`/`None` by e2 parity.
        23 => (
            "(do (def (f (: o (Option #tuple(Int64 Int64)))) (match o ((Some t) (+ (. t 0) (. t 1))) (None 0))) (export f))"
                .to_string(),
            vec![opt_tuple_arg],
        ),
        // 24 — lpr1 list<record<x,y>> RECORD-LIST-ELEMENT entry param: a `(List (Record (: x Int64) (: y Int64)))`
        //      param, read element 0's record + sum its two NAMED fields (or 0 if empty). The RECORD sibling of
        //      shape 22's list<tuple>: a record cell uses a distinct assembler path (named-field resolution/
...
fn gen_typefuzz_int<C: Choice>(
    ...
    match c.variant(arms) {
        ...
        // An Int64-producing Set op (T1.32), always CONSUMING a set to a scalar (a bare Set main result is
        // a rcdzc capability-gap decline). `(Set.len <set>)`; `(List.len (Set.to-list <set>))`; or
        // `(Set.len (<set-producing-op> …))` over of/insert/remove/union/intersection/difference — each
        // returns `Set α`, funnelled through `Set.len → Int64`. Well-typed (matching elements) → both
        // rcdzc + oracle infer Int64 → agreement.
        9 => {
            let wb = c.variant(2) == 0;
            let s = gen_typefuzz_set(c, iscope, bscope, fresh, wb);
            // depth 0 elements — avoid a nested `(set …)` literal in a Set-op arg (rcdzc capability-gap).
            let elem = |c: &mut C, is: &mut Vec<String>, bs: &mut Vec<String>, f: &mut usize| {
                if wb {
                    gen_typefuzz_bool(c, 0, is, bs, f)
                } else {
                    gen_typefuzz_int(c, 0, is, bs, f)
                }
            };
            match c.variant(7) {
                0 => format!("(Set.len {s})"),
                1 => format!("(List.len (Set.to-list {s}))"),
                2 => {
                    let e = elem(c, iscope, bscope, fresh);
                    format!("(Set.len (Set.insert {s} {e}))")
                }
                3 => {
                    let e = elem(c, iscope, bscope, fresh);
                    format!("(Set.len (Set.remove {s} {e}))")
                }
                4 => {
                    let s2 = gen_typefuzz_set(c, iscope, bscope, fresh, wb);
                    let op = ["union", "intersection", "difference"][c.variant(3)];
                    format!("(Set.len (Set.{op} {s} {s2}))")
                }
                5 => {
                    // `Set.of (List a) → Set a` — build the set from a list instead of a literal.
                    let xs = gen_typefuzz_list(c, iscope, bscope, fresh, wb);
                    format!("(Set.len (Set.of {xs}))")
                }
                _ => {
                    // membership folded into an Int64 via `if` (contains → Bool).
                    let e = elem(c, iscope, bscope, fresh);
                    format!("(if (Set.contains {s} {e}) 1 0)")
                }
            }
        }
        ...
        // A match with a NESTED scalar-LITERAL sub-pattern in a tuple/record/list position (T1.52 inc-6):
        // an element/field sub-pattern is an int LITERAL (fits any int width) at a matching-type position.
        // A literal is REFUTABLE, so the arm needs a COVERING catch-all `_` to be exhaustive ⇒ HOLDS.
        // Returns a bound binder / constant → Int64. Reachability confirmed (all 3 flip skip→Holds on the
        // inc-6 oracle). A literal at a NON-matching type is NOT generated (rcdzc type error / oracle-skip).
        _ => {
            let a = gen_typefuzz_int(c, 0, iscope, bscope, fresh);
            let b = gen_typefuzz_int(c, 0, iscope, bscope, fresh);
            match c.variant(3) {
                0 => format!("(match (tuple {a} 5) ((tuple x 5) x) (_ 0))"),
                1 => format!("(match (record (= x {a})) ((record (= x 0)) 1) (_ 0))"),
                _ => format!("(match (list {a} {b}) ((list 5 .. r) 1) (_ 0))"),
            }
        }
    }
}
...
fn gen_typefuzz_bool<C: Choice>(
    ...
    match c.variant(arms) {
        ...
        // `(Set.contains <set> <elem>)` → Bool (T1.32): membership over a set, the element MATCHING the
        // set's element type (well-typed). Both rcdzc + oracle infer Bool → agreement.
        8 => {
            let wb = c.variant(2) == 0;
            let s = gen_typefuzz_set(c, iscope, bscope, fresh, wb);
            let e = if wb {
...
fn gen_typefuzz_set<C: Choice>(
    ...
    let n = 2 + c.variant(3); // 2..=4 elements
    // DEPTH 0 (leaf: literal / in-scope var) — a set element that itself contains a nested `(set …)` literal
    // is a rcdzc capability-gap decline (nested set-literal not-yet-built), so keep set elements simple.
    let elems: Vec<String> = (0..n)
        .map(|_| {
            if want_bool {
                gen_typefuzz_bool(c, 0, iscope, bscope, fresh)
...
fn gen_typefuzz_illtyped<C: Choice>(
    ...
    match c.variant(31) {
        ...
        // oracle infers IllTyped ⇒ holds; an rcdzc ACCEPT here is a soundness false-accept.
        12 => {
            let xs = gen_typefuzz_list(c, iscope, bscope, fresh, false); // List Int64
            let b = boolean(c, iscope, bscope, fresh); // Bool element — clashes with Int64
            if c.variant(2) == 0 {
                format!("(if (Set.contains (Set.of {xs}) {b}) 1 0)")
            } else {
                format!("(Set.len (Set.insert (Set.of {xs}) {b}))")
            }
        }
        ...
        // an rcdzc ACCEPT here is a soundness false-accept.
        10 => {
            let a = int(c, iscope, bscope, fresh);
            let b = boolean(c, iscope, bscope, fresh);
            if c.variant(2) == 0 {
...
fn gen_typefuzz_value<C: Choice>(
    ...
    match c.variant(15) {
        ...
        // WellTyped agreement. (An EMPTY `(list)` has an unconstrained element type the oracle skips, so
