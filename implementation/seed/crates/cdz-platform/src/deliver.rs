...
//! inject.
//!
//! The deliver contract is a real [`Contract`] like any other — its id is the hash of its declared schema,
//! not of a bare name — and cached once. Its whole declaration is not hand-authored: it lives as real
//! Cadenza in `contracts/deliver.cdz`, whose `@!contract`/`@!input`/`@!output` module pragmas name the
//! contract and its input/output types alongside the type declarations. `cargo xtask codegen` typechecks it
//! with the `cdz` binary and generates `crate::contracts::deliver` (the schema builders and the
//! [`contract`](crate::contracts::deliver::contract) constructor built from those pragmas), so the
//! declaration is provably valid Cadenza and its identity lives only in the source, never restated here.
//!
...
/// (§4). It is a real contract whose id is the hash of its declared schema — generated by `cargo xtask
/// codegen` from `contracts/deliver.cdz` into [`crate::contracts::deliver::contract`]; the contract value is
/// built once and cached, so the id is derived only once.
#[must_use]
...
mod tests {
    ...
    fn the_encoded_value_has_the_shape_the_schema_declares() {
        ...
        let root = crate::contract_value::as_ascribed(&arenas, arenas.root).expect("root ascription");
