...
//! opaque bytes, or a `(program-hash <blob-name>)` reference that resolves at build to the named blob's
...
/// `(program-hash <blob-name>)` payload resolves at [`build`](HarnessSpec::build) to `ProgramHash::of(blob)`'s
...
/// carried inline unchanged; only the `(program-hash …)` head triggers resolution.
#[derive(Clone, Debug, PartialEq, Eq)]
...
pub enum DeliveryEvent {
    ...
    Message {
        ...
        contract: ContractId,
        /// The effect's input value: literal bytes, or a `(program-hash <blob-name>)` reference resolved to
...
impl DeliveryEvent {
    ...
    /// its content-hash bytes — called only for a message payload that is a `(program-hash <name>)` reference.
...
impl HarnessSpec {
    ...
    pub fn build<E>(
        ...
        // Resolve blobs to bytes, and keep each blob's content-hash bytes by name so a `(program-hash <name>)`
        ...
        // A `(program-hash <name>)` payload resolves to that blob's content-hash bytes; an unknown name yields
...
/// Read a message `payload` field as a [`PayloadSource`]: a `(program-hash <blob-name>)` reference — resolved
...
fn read_payload_source(
    ...
    Ok(PayloadSource::Inline(read_bytes(arenas, id).ok_or(
        SpecError::WrongType {
            field,
            want: "a bytes literal or (program-hash <name>)",
        },
    )?))
}
        ...
        DeliveryEvent::Message {
            ...
            let payload = match payload {
                ...
                PayloadSource::ProgramHash(name) => {
                    ...
                    bare_ctor(b, "program-hash", vec![name])
                }
            };
...
mod tests {
    ...
    fn a_program_hash_payload_decodes_and_a_bytes_payload_stays_inline() {
        // `(program-hash "sub")` reads as a ProgramHash reference (resolved to the blob's hash at build);
        // a literal `b"…"` stays Inline — the head is what discriminates, so ordinary payloads are unaffected.
        let arenas = built(|b| {
            let sub = s(b, "sub");
            let phash = bare_ctor(b, "program-hash", vec![sub]);
