...
pub fn build_driver_source(
    ...
    // Both cross a String/Bytes/sum RESULT through the wasm boundary's serialized form, so their result is
    // rendered specially (below); a plain export keeps `cdz_render_expr`.
    ...
    let ret_ty = cdz_return_type(module, export)
        .map(|t| if is_factory { crate::sig::peel_arrow_result(&t) } else { t });
...
mod tests {
    ...
    fn factory_string_result_renders_as_a_byte_list() {
        // A factory returning a String closure result → render __r as the byte-int list, not "…".
        ...
        assert!(d.contains("(__r).bytes()"), "String factory result → byte list: {d}");
    }
