...
async fn a_controller_resolve_alls_a_group_and_multicasts_an_emit_to_each_member() {
    ...
    // Build the two member sessions first so we know their genesis-hash ids (what the group stores + Emit targets).
    let member_a = HostedSession::genesis(
        Hash::of(b"member-a-reducer"),
        Box::new(MemberAgent),
        Box::new(Authorizer::deny_all()),
        CompositeExecutor::new(),
    );
    let member_b = HostedSession::genesis(
    ...
    while let Ok(inbound) = rx.try_recv() {
        ...
        routed_targets.push(inbound.session.as_str().to_string());
    }
    routed_targets.sort();
