...
///
/// The `by` on the op is the controller's SessionId string; the marker records it as a `Hash` (`Hash::of`
/// of the id bytes — the id IS the controller's genesis-hash-hex, an opaque provenance tag on the marker).
    ...
    while let Ok(op) = lifecycle_rx.try_recv() {
        match op {
            LifecycleOp::Terminate { target, by, reason } => {
                let by_hash = Hash::of(by.as_str().as_bytes());
