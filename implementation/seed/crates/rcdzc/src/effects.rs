...
fn node_refs_binder(db: &mut Db, node: StructId, binder: StructId) -> bool {
    match resolved_of(db, node) {
        ...
        Resolved::Ref { value } => {
            ...
            // Ref cycles (`resolve.rs::value_ref_cycle`), and this predicate is called from recursive subtree
            ...
            // `binder` is not referenced → false. Mirrors `value_ref_cycle`'s bounded walk (resolve.rs:562).
