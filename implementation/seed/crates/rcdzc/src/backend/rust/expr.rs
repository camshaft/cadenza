...
fn emit(db: &mut Db, id: StructId, env: &Env, ctx: &Ctx) -> Result<String, Reject> {
    match core_of(db, id) {
        ...
        Core::MapLookup { map, key, .. } => {
            ...
            // the DEFAULT `BTreeMap<i64,i64>`, but the use is `.get(&"k".to_string())` (String key) → a silent
            // MISCOMPILE (rust E0308; wasm runs correct — breaker ms9). RECONSTRUCT the map's type at this
            ...
            let reconstructed = if map_ty.has_free_var() && !key_ty.has_free_var() {
                // The lookup result is `Option<V>`; pull V (the map's value type) from its sole type arg.
                let val_ty = match type_of(db, id).strip_nominal() {
                    Ty::Sum { name, args, .. } if name == "Option" && args.len() == 1 => {
                        args[0].clone()
                    }
                    _ => Ty::Var(0), // no readable payload → leave open, grounds below
                };
