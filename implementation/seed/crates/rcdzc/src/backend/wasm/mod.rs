...
pub mod select;
pub mod serialize;
// Sum-typed runtime-resource emitters (`emit_runtime_sum_resource`, `emit_recursive_sum_resource`),
// peeled out of this file to stay under the 512 KiB source cap; re-imported so their call sites here
// are unchanged.
mod sum_resource;
use sum_resource::{emit_recursive_sum_resource, emit_runtime_sum_resource};
    ...
    for &def in &layout.order {
            ...
            host::first_unrepresentable_host_op(db, body, allow_option_bytes)
        {
            ...
            return Err(Reject::declined(
                crate::diag::DeclineId::WasmHostOpNoBoundaryFormOnBareEffect,
                format!(
