...
fn site_a_record_cell_droppable_when_field_forwarded_borrows_the_param() {
    ...
    // drop. This is the payload-forwarding shape the census reducers take (on_notification/on_response went
    // 3->0 / 5->0 after site-a #9061).
    //
    // POST SITE-B JOINT FIX: the escape query's Proj arm now correctly recognizes a compound-field forward
    // as BORROWING the operand (the child carries its own dup), so the DUP-UNAWARE `param_escapes_body`
    // AGREES with the dup-aware gate here — both say `m` does not escape → droppable. (Before the fix the
    // dup-unaware query over-reported escape and droppability held only via the per-field parent dup_site;
    // that surplus dup was the node#4 census leak, now removed.) The dup-aware gate remains meaningful for
    // genuinely dup-RESCUED consumes elsewhere (a value dup'd for a later use — see
    // `map_lookup_project_escape_is_monotone_in_dup_sites_rules_out_the_4405_gate`); this shape simply no
    // longer needs it. The SAFETY polarity (a MOVED-verbatim cell must NOT be dropped) is pinned separately
    // by `site_a_record_cell_not_droppable_when_moved_out`.
    let ast = crate::testkit::parse(
...
fn site_b_reducer_forward_emits_no_surplus_parent_dups() {
    // SITE-B (node#4 census leak) positive witness for the JOINT fix (escape-query Proj-arm borrow-
    // reclassification + v-core-opt's mark_binder_dups one-liner). The payload-forwarding reducer shape:
    // a param `msg` whose THREE compound (Bytes) fields are each projected + forwarded into a fresh record.
    // Each field emits ONE child dup (the extracted Bytes dup'd into the new record); the param itself is
    // BORROWED (arr-get borrows), so NO per-field PARENT dup of `msg` should be emitted. Before the joint
    // fix the emit had 6 dups (3 child + 3 SURPLUS parent) and the wrapper deep-dropped msg's cell only
    // once → net +N over-retain = the on_message=7 census residual. After: exactly 3 dups (child only), and
    // `msg`'s cell stays droppable (a dead owned temporary). A regression that reintroduces the parent
    // dup_site (or reverts the Proj-arm borrow) flips the count back up and the leak returns.
    let mut db = Db::load(crate::testkit::parse(
    ...
    assert_eq!(
        ...
        "site-b: the 3-field forward must emit exactly 3 dups (one CHILD dup per forwarded field, ZERO \
         surplus PARENT dups) — got {dups} (pre-joint-fix this was 6: 3 child + 3 surplus parent)"
    );
    assert!(
        record_cell_param_droppable(&mut db, body, binder),
