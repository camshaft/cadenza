...
pub(super) fn def_inc1_reclaims_param(db: &mut Db, body: StructId, param_binder: StructId) -> bool {
    ...
    fn go(
            ...
            && matches!(core_of(db, scrutinee), Core::Param { binder } | Core::LocalRef { binder } if binder == pb)
        {
            ...
            if nontail_param_reclaim_kind(db, top_body, scrutinee, &scrut_ty, false, &root)
                .is_some()
            {
                return true;
            }
...
// =============================================================================================
// INC1 reclaim-KIND predicates (moved from select.rs to keep it under the 512 KiB file-size mandate,
// #7349 pushed it over). These are the owned-recursive-sum-param shell-reclaim payload gates; they call
// select.rs privates (cont_rematches_scrutinee, sum_cont_arm_constructs_compound, …) via `use super::*`.
// =============================================================================================
...
pub(super) fn nontail_param_reclaim_kind(
    ...
    // COMPOUND arm — INC1 increment-2: reclaim a RECONSTRUCTING self-recursive fold (recon inc, BST del-min —
    // arms rebuild `(Node …)`/`#tuple(…)`) via is_nontail_spine_param (spine select, needs top_body) +
    // nontail_param_compound_extra_ok (interior-view alias-out + sum_cont_arm_returns_scrutinee fences). The
