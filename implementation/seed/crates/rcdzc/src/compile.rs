...
fn compile_with_opt_inner(
    ...
    if !db.quote_params.is_empty() {
        ...
        // load-time nested-eval fixpoint does not apply). `eval_boundary` is captured AFTER reify (matching
        // the load-time provenance line + this pass's pre-fixpoint behaviour: reified `Ast.*`/original nodes
        // are `< boundary` = caller-origin, a reconstruction is `>= boundary` = template); a fresh
        // `eval_result_nodes` supplies the narrowing that keeps `(eval (eval …))` a correct reject even for an
        // expansion-produced nested eval. The fold count is unused here (single pass).
        let eval_boundary = db.ast.structure.len() as u32;
        let mut eval_result_nodes = std::collections::HashSet::new();
        let _ = crate::eval_ast::desugar_eval(&mut db.ast, eval_boundary, &mut eval_result_nodes);
