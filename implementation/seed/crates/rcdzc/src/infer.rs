...
fn ground_open_var_arms_to_collection(db: &mut Db, bodies: &[StructId], ty: &Ty) {
    // (a) The join must be a determined collection — a solved element, not a hole. Grounding an arm to a
    // `(List Any)`/`(List ?0)` would just move the hole into the arm (still no valtype); require a real
    // element so the arm gets a genuine machine type. `(List Any)` from a lone empty `(list)` is a valid
    // TARGET here though — it emits as an i32 handle (ej1c-proven) — so admit an `Any` ELEMENT for a list/
    // set (the container is the handle; the element type does not change the i32 slot), but a bare-`Var`
    // element (undetermined container) is not a grounding source.
    let is_ground_collection = match ty {
        Ty::List(e) | Ty::Set(e) => !matches!(**e, Ty::Var(_)),
        Ty::Map(k, v) => !matches!(**k, Ty::Var(_)) && !matches!(**v, Ty::Var(_)),
        _ => false,
    };
    if !is_ground_collection {
        return;
    }
