...
fn an_out_of_range_literal_through_a_collection_builder_chain_or_set_rejects_cdz0302() {
    ...
    // collection BUILT by a `Map.insert`/`Set.of`/`Set.insert` builder chain — and the entire
    // `Ty::Set` case — escaped the check on BOTH `check` and `emit`. `(: (Map.insert Map.empty k 200)
    // (Map Int64 Int8))` compiled clean and RAN to a truncated -56. FIX: the `Ty::Map` arm also walks a
    // `MapInsert` chain (check key/val args, recurse the operand), and a new `Ty::Set` arm handles
    // `Set.of`/`Set.insert`. Pins that each builder position now rejects CDZ0302, and that in-range
    // builder chains still compile.
    use crate::abi::Artifact;
    let codes_for = |body: &str| -> Vec<String> {
        let entry = crate::codec::encode(&parse(&format!(
            "(do (def (main (: k Int64)) {body}) (export main))"
        )));
        let out = crate::compile::compile(
            &[
                Artifact::new(Artifact::KIND_AST, "app", entry.clone()),
    ...
    // Map.insert VALUE position, 200 out of range for Int8 → CDZ0302 (was: silent -56 at runtime).
    assert!(
        codes_for("(Map.len (: (Map.insert Map.empty k 200) (Map Int64 Int8)))")
            .contains(&"CDZ0302".to_string()),
        "an out-of-range Int8 VALUE through Map.insert must reject CDZ0302"
    );
    // Map.insert KEY position, 200 out of range for Int8 key → CDZ0302.
    assert!(
        codes_for("(Map.len (: (Map.insert Map.empty 200 k) (Map Int8 Int64)))")
            .contains(&"CDZ0302".to_string()),
        "an out-of-range Int8 KEY through Map.insert must reject CDZ0302"
    );
    // A DEEPER insert chain — the out-of-range literal in an inner insert is reached by recursion.
    assert!(
        codes_for("(Map.len (: (Map.insert (Map.insert Map.empty k 200) k 1) (Map Int64 Int8)))")
            .contains(&"CDZ0302".to_string()),
        "an out-of-range literal in an INNER Map.insert of a chain must reject CDZ0302"
    );
    // Set.of element position → CDZ0302 (there was previously NO Ty::Set arm at all).
    assert!(
        codes_for("(Set.len (: (Set.of (list 200)) (Set Int8)))").contains(&"CDZ0302".to_string()),
        "an out-of-range Int8 element through Set.of must reject CDZ0302"
    );
    // Set.insert element position → CDZ0302.
    assert!(
        codes_for("(Set.len (: (Set.insert Set.empty 200) (Set Int8)))")
            .contains(&"CDZ0302".to_string()),
        "an out-of-range Int8 element through Set.insert must reject CDZ0302"
    );
    // CONTROLS: in-range builder chains still compile (no over-reject).
    assert!(
        !codes_for("(Map.len (: (Map.insert Map.empty k 5) (Map Int64 Int8)))")
            .contains(&"CDZ0302".to_string()),
        "an in-range Int8 VALUE through Map.insert stays legal"
    );
    assert!(
        !codes_for("(Set.len (: (Set.of (list 5)) (Set Int8)))").contains(&"CDZ0302".to_string()),
        "an in-range Int8 element through Set.of stays legal"
    );
    // The map/list LITERAL cases (already covered) still reject — no regression.
    assert!(
        codes_for("(Map.len (: (map (1 200)) (Map Int64 Int8)))").contains(&"CDZ0302".to_string()),
...
fn a_resume_with_the_wrong_number_of_operands_is_cdz0201() {
    ...
    assert!(
        dfew.iter()
            .any(|d| d.code.as_deref() == Some("CDZ0201") && d.message.contains("no next-state")),
