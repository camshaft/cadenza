...
// (a_record_field_key_colliding_with_a_param_stays_beta_immune_through_a_native_field_pair redundant —
// corpus 06-numeric-model "bsi1 β-substitution discriminates KEYS from VALUES in a NATIVE #record — keys
// immune, values substituted" is the native-#record run case ((. #record((= x 5) (= y x)) x) → 5 with the
// KEY x β-immune to the param x, and (. … y) → the arg with the VALUE substituted; runs to 507/498). A green
// value there requires the inlined record to compile WITHOUT the spurious "record field key must be a name"
// CDZ0201 — exactly the M2b FieldPair-key β-immunity this test guarded. (Also 05-compound-types "a record
// field key that coincides with a parameter name survives the call".) Redundant, removed.)
...
// (Migrated to corpus 02-binding-and-control: "a non-Bool guard condition is a type error naming the
// offending type" (CDZ0203, guard condition must be Bool + Int64), "a String guard condition is likewise
// rejected — the Bool check is general" (CDZ0203), "a fault inside a guard condition surfaces — the
// condition is walked" (CDZ0101 unbound name), and "a well-typed Bool guard condition compiles and runs
// clean" (value 5, no false positive). Was a_guard_condition_must_be_bool_and_its_faults_surface.)
...
// (shadowing_a_prelude_payload_type_name_is_a_plain_rebind_not_a_phantom_variant_fault migrated to corpus
// 07-type-system: "a def named after a prelude type name is a plain rebind and compiles" (Int64→value 1) +
// its String sibling, and "a USER variant naming a value-shadowed prelude type name as its payload still
// faults CDZ0203" (the check gated on user nodes is not weakened). The spanless-prelude-payload-node facet
// — the fault used to print with no line:col at the prelude's own payload node — is prose in the corpus
// doc; the code+value observables are the corpus pins.)
