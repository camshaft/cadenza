...
#[test]
fn map_compound_value_key_and_member_access_invariants_reject_cdz0201() {
    // SOUNDNESS guard (05-compound-types), the COMPANION to the mixed-scalar-type / duplicate-scalar-key
    // guard above. It pins the map-type invariants that survive PAST the scalar cases — the ones exercised
    // through the M2 native-compound recognition path (FieldPair map entries + compound VALUES + `.` member
    // access) — in BOTH the name-alias `(map (= k v))` (→ `Apply(MapNew)`) and native `#map((= k v))`
    // (→ `Resolved::Map`) spellings, so a future edit to either arm cannot silently drop the check:
    //   • value-homogeneity by SHAPE (record-value field-set / tuple-value arity mismatch, not just scalar);
    //   • duplicate-key canonicalization for the non-Int/String key kinds (hex-spelling variant, Bool, Unit);
    //   • the cross-KIND `map` vs `record` comparison (a Map and a Record are distinct types);
    //   • `.` member access on a map (a map is not a record — its head is the native `Member` leaf).
    // Every case has an `(error CDZ0201)` sibling in spec/semantics/05-compound-types.sexp; this is the
    // in-process fast-gate mirror that protects the native recognition seam the corpus grade cannot re-run
    // cheaply. (Pre-#5247 the name-alias arm dropped FieldPair entries → these silently built/compiled.)
    for (label, src) in [
        // value-homogeneity by shape — compound values
        (
            "record-value diff fields (alias)",
            "(module m (def (main) (= (map (= \"a\" (record (= x 1))) (= \"b\" (record (= y 2)))) (map (= \"a\" (record (= x 1))) (= \"b\" (record (= y 2)))))) (export main))",
        ),
        (
            "record-value diff fields (native)",
            "(module m (def (main) (= #map((= \"a\" #record((= x 1))) (= \"b\" #record((= y 2)))) #map((= \"a\" #record((= x 1))) (= \"b\" #record((= y 2)))))) (export main))",
        ),
        (
            "tuple-value diff arity (alias)",
            "(module m (def (main) (= (map (= \"a\" (tuple 1 2)) (= \"b\" (tuple 1 2 3))) (map (= \"a\" (tuple 1 2)) (= \"b\" (tuple 1 2 3))))) (export main))",
        ),
        (
            "tuple-value diff arity (native)",
            "(module m (def (main) (= #map((= \"a\" #tuple(1 2)) (= \"b\" #tuple(1 2 3))) #map((= \"a\" #tuple(1 2)) (= \"b\" #tuple(1 2 3))))) (export main))",
        ),
        // duplicate-key canonicalization across key kinds
        (
            "dup int spelling variant 0x1 (alias)",
            "(module m (def (main) (= (map (= 1 10) (= 2 20) (= 0x1 30)) (map (= 1 10) (= 2 20) (= 0x1 30)))) (export main))",
        ),
        (
            "dup bool key (alias)",
            "(module m (def (main) (= (map (= true 1) (= true 2)) (map (= true 1) (= true 2)))) (export main))",
        ),
        (
            "dup bool key (native)",
            "(module m (def (main) (= #map((= true 1) (= true 2)) #map((= true 1) (= true 2)))) (export main))",
        ...
        (
            "dup unit key (alias)",
            "(module m (def (main) (= (map (= () 1) (= () 2)) (map (= () 1) (= () 2)))) (export main))",
        ),
        // cross-KIND comparison + member access
        (
            "map vs record (alias)",
            "(module m (def (main) (= (map (= \"a\" 1) (= \"b\" 2)) (record (= a 1) (= b 2)))) (export main))",
        ),
        (
            "empty map vs empty record (alias)",
            "(module m (def (main) (= (map) (record))) (export main))",
        ),
        (
            "member access on a map (alias)",
            "(module m (def (main) (let ((m (map (= \"a\" 1) (= \"b\" 2)))) (. m a))) (export main))",
        ),
        (
            "member access on a map (native)",
            "(module m (def (main) (let ((m #map((= \"a\" 1) (= \"b\" 2)))) (. m a))) (export main))",
        ),
    ] {
        assert_eq!(
            reject_code(src).as_deref(),
            Some("CDZ0201"),
            "{label}: this map-type invariant must reject CDZ0201, not silently build/project"
        );
    }
    // Guard the OTHER direction: two maps of the SAME type (Map String Int64) with DIFFERENT key sets are
    // comparable (a map's type is its key/value type, NOT its key SET) — they compile and compare, they do
    // NOT false-reject CDZ0201. Order-independent equality likewise compiles.
    for (label, src) in [
        (
            "eq diff key sets, same type",
            "(module m (def (main) (= (map (= \"a\" 1) (= \"b\" 2)) (map (= \"a\" 1) (= \"c\" 2)))) (export main))",
        ),
        (
            "order-independent eq",
            "(module m (def (main) (= (map (= \"a\" 1) (= \"b\" 2)) (map (= \"b\" 2) (= \"a\" 1)))) (export main))",
        ),
    ] {
        assert!(
            reject_code(src).is_none(),
            "{label}: same-typed maps must compile and compare, not false-reject"
        );
    }
}
...
fn an_operator_arg_structural_mismatch_names_the_delta_not_the_raw_unify_message() {
    ...
    let field_ty =
        reject_full("(module m (def (main) (= (record (x 1)) (record (x true)))) (export main))")
    ...
    let tup = reject_full("(module m (def (main) (= (tuple 1 2) (tuple 1 2 3))) (export main))")
...
fn an_annotation_mismatch_with_no_fitting_variant_carries_no_wrap() {
    ...
    let d =
        reject_full("(module m (type Flag On Off) (def (f (: n Int64)) (: n Flag)) (export f))")
            .expect("mismatch must reject");
    assert_eq!(d.code.as_deref(), Some("CDZ0203"), "got: {}", d.message);
    assert!(
...
fn using_an_option_where_its_payload_is_expected_says_to_match_it() {
    ...
    let binop =
        reject_full("(module m (def (g (: xs (List Int64))) (+ ((. List at) xs 0) 1)) (export g))")
...
fn record_project_narrows_to_named_fields_absent_field_is_cdz0212() {
    ...
    assert!(
        !far.message.contains("did you mean") && far.message.contains("closest matches: `alpha`"),
    ...
    assert_eq!(
        reject_code("(module m (def (main) (: (record (a 1)) (Record (a Int64)))) (export main))"),
        None,
