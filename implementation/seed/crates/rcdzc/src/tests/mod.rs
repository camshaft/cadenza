...
// NOTE: a former test asserting a `result<scalar,scalar>` ENTRY param DECLINES was removed when that lift
// landed — a `result<ok,err>` is a STRUCTURAL (anonymous-allowed) WIT former, so the bare-export path now
// synthesizes `result<ok,err>` and admits a scalar Result entry param (crossing as `(disc, join)`). The
// language behavior — a `result<Int64,Int64>` param delivers its Ok/Err payload — is gated by the corpus
// (09-functions `erp2`), the implementation-independent home for an observable behavior, not a host-lang test.
