//! The `cdz-calc` command surface, as an EMBEDDABLE clap `Args` group + a `run` entry point.
//!
//! Factored out of the standalone `main.rs` so the unified `cdz` binary can MOUNT it as `cdz calc …`
//! (the same flatten pattern `cdz` uses for run/corpus) WITHOUT a second binary on the PATH. The
//! standalone `cdz-calc` bin is now a thin shim over [`run`]; both entry points share one implementation
//! and one `--help`. This module owns only the ARG-PARSING + REPL-LOOP surface — the calculator ENGINE
//! (`Calculator`, `Eval`, `eval_program`, `runtime`) is untouched, so its owner's territory is intact.
//! `run` takes the already-parsed [`CalcArgs`] and returns an `ExitCode`, threading a `prog` name so a
//! diagnostic points at the command the user actually typed (`cdz` vs `cdz-calc`).
...
use crate::{Calculator, Eval};

/// The arguments to `cdz calc` / `cdz-calc` — a REPL over the real language, exact by construction.
#[derive(clap::Args)]
pub struct CalcArgs {
...
/// Run the calculator per `args`, returning the process exit code. `prog` names the tool in diagnostics
/// (`cdz-calc` for the standalone bin, `cdz` for the unified one).
pub fn run(args: &CalcArgs, prog: &str) -> ExitCode {
    let surface = if args.sexpr {
        Format::Sexpr
    } else {
        Format::Ml
    };
    let exact = !args.no_exact;
    match &args.once {
        Some(expr) => run_once(expr, surface, exact, args.plain, prog),
        None => run_interactive(surface, exact, args.plain, prog),
    }
}
...
fn run_once(expr: &str, surface: Format, exact: bool, plain: bool, prog: &str) -> ExitCode {
...
fn run_interactive(surface: Format, exact: bool, plain: bool, prog: &str) -> ExitCode {
    let mut calc = Calculator::new_with_exact(surface, exact).with_plain(plain);
