...
// The RESULT-ARTIFACT KIND vocabulary — the artifact `kind` string a delegated reader tags the
// captured bytes with for each `Query` answer (and `KIND_SIDECAR` for the request-list input).
// Part of the compile-boundary wire `cdz` and `rcdzc` agree on, so it lives here alongside the
// `Request`/`Query` vocabulary + codec; `rcdzc::sidecar` `pub use`s these so `sidecar::KIND_*` and
// `crate::sidecar::KIND_*` stay byte-stable there, and the `run_query` producer keeps resolving them.
