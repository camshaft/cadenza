...
fn main() -> ExitCode {
    ...
    match cli.command {
        ...
        Cmd::RunRust(a) => run_rust::run_run_rust(&a),
...
pub(crate) fn read_verdict_source(file: Option<&str>, cmd: &str) -> Result<String, ()> {
    use std::io::Read;
    match file {
...
mod tests {
    ...
    // Run-rust oracle internals (now in the `run_rust` module) the verdict/rlib/signature tests drive.
    use crate::run_rust::{
        emitted_params, emitted_pub_fn_names, marshal_export_args, panic_reason, resolve_deps_dir,
    ...
    fn emitted_params_reads_the_signature() {
        // Nullary → empty list; one param → its `<name>: <type>` slice.
        assert_eq!(
        ...
        // Inner commas in a tuple/generic param type must NOT split the param.
        assert_eq!(
            emitted_params("pub fn g(p: (i64, i64), q: Vec<(A, B)>) -> i64 { 0 }", "g"),
            Some(vec!["p: (i64, i64)", "q: Vec<(A, B)>"])
        );
        // The env plumbing param stays in the raw list (the caller filters it by prefix).
        assert_eq!(
            emitted_params(
        ...
        // Absent export / prefix-only match → None (a bad --call).
    ...
    fn marshal_export_args_is_type_aware() {
        // Scalar passes through verbatim (the fn signature fixes its type); a String crosses as owned.
        assert_eq!(
        ...
        // A bare decimal builds a `Big` ONLY for a `cdz_num::Big` param (else it stays verbatim + fails rustc).
        assert_eq!(
