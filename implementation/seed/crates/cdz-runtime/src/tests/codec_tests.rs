...
fn value_decode_is_frame_tolerant_for_named_and_nested_in_list() {
    ...
    // Named tag 10: [10][name_len=4]"Item"[inner=0].
    ...
    // A real RUNTIME list value (RRB vec).
    let mut v = op_vec_empty();
    for i in 1..=3i64 {
    ...
    // FRAMED element form still decodes (backward compatible).
    ...
    // BARE element form (no `(: n Item)` per element) now decodes against `Named("Item", Int)` — this
    // returned NULL before the frame-tolerance fix.
