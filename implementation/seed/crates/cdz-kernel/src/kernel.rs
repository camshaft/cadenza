...
mod monotonic_now_tests {
    ...
    async fn genesis_hash_is_the_first_event_hash_stable_across_replay_and_reducer_derived_today() {
        ...
        // Pin the properties that name-addressing-as-identity-map depends on. Drive a REAL fold so the
        ...
        s.deliver(inbound(), None, &NowReducer, &now_cap(), &mut StuckClock(1000))
            .await
            .unwrap();
        assert!(
            s.log().len() > 1,
