...
/// Decode a `child-exited` frame encoded by [`encode_child_exited`] → `(child, CloseOutcome)`. Total:
/// non-conforming bytes / a bad hash / an unknown close-outcome head are a clean `Shape` error, never a
/// panic (the bytes ride the durable inbox / an untrusted sender). Reuses `read_close_outcome` so the
/// outcome accepts the SAME shapes the `Closed` event does.
